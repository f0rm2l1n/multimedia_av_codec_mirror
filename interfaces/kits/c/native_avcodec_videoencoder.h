 ...
 * know the exact name of the encoder. The encoder name can be obtained through capability query.
 * @syscap SystemCapability.Multimedia.Media.VideoEncoder
...
/**
 * @brief Set the OH_AVCodecCallback callback function so that the application can respond to the events generated by the
...
/**
 * @brief Set the OH_AVCodecCallback callback function so that the application can respond to the events generated
 * by the video encoder. In encode Surface mode, this interface is used to set frame-specific parameters.
 * If this interface is used, it must be invoked before {@link OH_VideoEncoder_Configure}.
 *
 * @syscap SystemCapability.Multimedia.Media.VideoEncoder
 * @param codec Pointer to an OH_AVCodec instance
 ...
 * otherwise returns a specific error code, refer to {@link OH_AVErrCode}.
 * {@link AV_ERR_NO_MEMORY}, internal errors in the input encode instance, such as an abnormal nullptr.
 * {@link AV_ERR_INVALID_VAL}, 1. the encoder is nullptr or invalid; 2. invalid param in format.
 * {@link AV_ERR_UNKNOWN}, unknown error.
 * {@link AV_ERR_OPERATE_NOT_PERMIT}, internal execution error.
 * {@link AV_ERR_INVALID_STATE}, this interface was called in invalid state, must be called before Prepare.
 * @since 9
 ...
 * you can re-enter the Executing state through Start.
 * @syscap SystemCapability.Multimedia.Media.VideoEncoder
 * @param codec Pointer to an OH_AVCodec instance
 * @return Returns AV_ERR_OK if the execution is successful,
 * otherwise returns a specific error code, refer to {@link OH_AVErrCode}.
...
/**
 * @brief Clear the input and output data buffered and parameters in the encoder, for example, PPS/SPS in H264 format.
 * After this interface is called, all the buffer indexes previously reported through the asynchronous callback will be
 * invalidated, make sure not to access the buffers corresponding to these indexes.
 * @syscap SystemCapability.Multimedia.Media.VideoEncoder
 * @param codec Pointer to an OH_AVCodec instance
 * @return Returns AV_ERR_OK if the execution is successful,
 * otherwise returns a specific error code, refer to {@link OH_AVErrCode}.
...
/**
 * @brief Reset the encoder. The encoder returns to the Initialized state. To continue coding,
 ...
 * 1. the encoder is nullptr or invalid;
