...
void TimeAndIndexConversion::ParseMoov(uint32_t boxSize)
{
    ...
    while (offset_ < parentSize) {
        ...
        FALSE_RETURN_MSG(childBoxSize >= BOX_HEAD_SIZE, "ParseMoov failed due to error box size");
...
void TimeAndIndexConversion::ParseBox(uint32_t boxSize)
{
    ...
    while (offset_ < parentSize) {
        ...
        FALSE_RETURN_MSG(childBoxSize >= BOX_HEAD_SIZE, "ParseBox failed due to error box size");
