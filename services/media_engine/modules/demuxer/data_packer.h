...
public:
    ...
    void PushData(std::shared_ptr<Plugin::Buffer>& bufferPtr, uint64_t offset);

    bool IsDataAvailable(uint64_t offset, uint32_t size, uint64_t &curOffset);

    bool PeekRange(uint64_t offset, uint32_t size, std::shared_ptr<Plugin::Buffer>& bufferPtr);

    bool GetRange(uint64_t offset, uint32_t size, std::shared_ptr<Plugin::Buffer>& bufferPtr);

    bool GetRange(uint32_t size, std::shared_ptr<Plugin::Buffer>& bufferPtr); // For live play
...
private:
    void RemoveBufferContent(std::shared_ptr<Plugin::Buffer> &buffer, size_t removeSize);

    bool PeekRangeInternal(uint64_t offset, uint32_t size, std::shared_ptr<Plugin::Buffer>& bufferPtr, bool isGet);

    void FlushInternal();

    bool FindFirstBufferToCopy(uint64_t offset, int32_t &startIndex, uint64_t &prevOffset);

    size_t CopyFirstBuffer(size_t size, int32_t index, uint8_t *dstPtr, std::shared_ptr<Plugin::Buffer>& dstBufferPtr,
    ...
    Mutex mutex_;
    std::deque<std::shared_ptr<Plugin::Buffer>> que_;
