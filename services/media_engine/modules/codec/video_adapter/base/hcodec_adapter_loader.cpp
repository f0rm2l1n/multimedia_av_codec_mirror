...
const char *HCODEC_LIB_PATH = "libhcodec_adapter.z.so";
} // namespace
std::shared_ptr<CodecBaseAdapter> HCodecAdapterLoader::CreateByName(const std::string &name)
{
    HCodecAdapterLoader &loader = GetInstance();
    CHECK_AND_RETURN_RET_LOG(loader.Init() == AVCS_ERR_OK, nullptr, "Create codec by name failed: init error");
    return loader.Create(name);
}

int32_t HCodecAdapterLoader::GetCapabilityList(std::vector<CapabilityData> &caps)
{
    HCodecAdapterLoader &loader = GetInstance();
...
HCodecAdapterLoader &HCodecAdapterLoader::GetInstance()
{
    static HCodecAdapterLoader loader;
    return loader;
}

int32_t HCodecAdapterLoader::Init()
{
...
std::shared_ptr<CodecBaseAdapter> HCodecAdapterLoader::Create(const std::string &name)
{
    std::shared_ptr<CodecBaseAdapter> codec;
    createFunc_(name, codec);
    return codec;
}

int32_t HCodecAdapterLoader::GetCaps(std::vector<CapabilityData> &caps)
{
