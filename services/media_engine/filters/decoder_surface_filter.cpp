...
void DecoderSurfaceFilter::OnError(MediaAVCodec::AVCodecErrorType errorType, int32_t errorCode)
{
    ...
    eventReceiver_->OnEvent({"DecoderSurfaceFilter", EventType::EVENT_ERROR, MSERR_VIDEO_DEC_FAILED});
}
...
Status DecoderSurfaceFilter::DoInitAfterLink()
{
    ...
    if (ret != Status::OK && eventReceiver_ != nullptr) {
        ...
        eventReceiver_->OnEvent({"decoderSurface", EventType::EVENT_ERROR, MSERR_VIDEO_DEC_UNAVAILABLE});
