...
namespace {
constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, LOG_DOMAIN_SYSTEM_PLAYER, "SeiParser" };
...
constexpr uint8_t NALU_GUIDE_0_LEN = 3;
constexpr uint8_t EMULATION_GUIDE_0_LEN = 2;
constexpr uint8_t EMULATION_PREVENTION_CODE = 0X03;
 
const std::unordered_set<int32_t> SEI_PAYLOAD_TYPE_SUP_SET = { 5 };
}  // namespace
 
namespace OHOS {
namespace Media {
Status SeiParser::ParseSeiPayload(std::string mimeType, std::shared_ptr<AVBuffer> buffer,
    const std::shared_ptr<SeiPayloadInfoGroup> &group, const std::vector<int32_t> &targetTypeVec)
{
    ...
    FALSE_RETURN_V_MSG(mimeType == TYPE_AVC || mimeType == TYPE_HEVC, Status::ERROR_UNSUPPORTED_FORMAT,
        "only support H.264 and H.265");
    ...
    while (FindNextSeiNaluPos(naluStartPtr, maxPointer - 1 - seiNaluPrefixLen, mimeType) == Status::OK) {
        MEDIA_LOG_D("winddraw find nalu pts %{public}ld", buffer->pts_);
        auto naluParseRes = ParseSeiRbsp(naluStartPtr, maxPointer, group, targetTypeVec);
...
Status SeiParser::FindNextSeiNaluPos(uint8_t *&startPtr, uint8_t *maxPtr, std::string mime)
{
    for (uint8_t zeroNum = 0; startPtr < maxPtr; startPtr++) {
        if (*startPtr == 0) {
            zeroNum++;
            continue;
        }
        if (*startPtr != 1 || zeroNum < NALU_GUIDE_0_LEN) {
            zeroNum = 0;
            continue;
        }
        zeroNum = 0;
        uint8_t header = *(++startPtr);
        if (mime == TYPE_AVC) {
            auto naluType = header & AVC_NAL_UNIT_TYPE_FLAG;
            if (naluType == AVC_SEI_TYPE) {
                startPtr += AVC_SEI_HEAD_LEN;
                return Status::OK;
            }
            continue;
        }
        ...
        if (naluType == HEVC_SEI_TYPE_ONE || naluType == HEVC_SEI_TYPE_TWO) {
            startPtr += HEVC_SEI_HEAD_LEN;
            return Status::OK;
        }
    }
    return Status::ERROR_NOT_SEI_FORAMT;
}
 
Status SeiParser::ParseSeiRbsp(uint8_t *&bodyPtr, uint8_t *maxPtr, const std::shared_ptr<SeiPayloadInfoGroup> &group,
    const std::vector<int32_t> &targetTypeVec)
{
    ...
    while (bodyPtr + SEI_UUID_LEN < maxPtr) {
        ...
        if (std::find(targetTypeVec.begin(), targetTypeVec.end(), payloadType) == targetTypeVec.end()) {
            MEDIA_LOG_I("winddraw unsupport type %{public}d", payloadType);
            auto res = IteratorMsgCheckEmuBytes(nullptr, bodyPtr, maxPtr, payloadSize);
        ...
        auto copyRes = IteratorMsgCheckEmuBytes(avBuffer, bodyPtr, maxPtr, payloadSize);
...
int32_t SeiParser::GetSeiTypeOrSize(uint8_t *&bodyPtr, const uint8_t *maxPtr)
{
    int32_t res = 0;
...
Status SeiParser::IteratorMsgCheckEmuBytes(
    const std::shared_ptr<AVBuffer> buffer, uint8_t *&payloadPtr, const uint8_t *maxPtr, const int32_t payloadSize)
{
    ...
    for (int32_t zeroNum = 0; writtenSize < payloadSize && payloadPtr < maxPtr; payloadPtr++) {
        ...
        if (*payloadPtr == EMULATION_PREVENTION_CODE && zeroNum == EMULATION_GUIDE_0_LEN) {
            uint8_t *andOne = payloadPtr + 1;
            if (andOne < maxPtr && *andOne <= EMULATION_PREVENTION_CODE) {
                zeroNum = 0;
                continue;
            }
        }
...
{
    FALSE_RETURN_NOLOG(avBuffer != nullptr);
 
    auto producer = producer_.promote();
    FALSE_RETURN_MSG(producer != nullptr, "report sei failed, buffer queue producer is nullptr");
    ON_SCOPE_EXIT(0)
    {
        producer->ReturnBuffer(avBuffer, true);
    };
 
    auto eventReceiver = eventReceiver_.lock();
    FALSE_RETURN_MSG(eventReceiver != nullptr, "report sei failed, event receiver is nullptr");
    std::shared_ptr<SeiPayloadInfoGroup> group = std::make_shared<SeiPayloadInfoGroup>();
    auto res = SeiParser::ParseSeiPayload(mimeType_, avBuffer, group, payloadTypeVec_);
    ...
    eventReceiver->OnEvent({ "SeiParser", EventType::EVENT_SEI_INFO, seiInfoFormat });
}
}  // namespace Media
}  // namespace OHOS