...
static AutoRegisterFilter<AudioSinkFilter> g_registerAudioSinkFilter("builtin.player.audiosink", FilterType::FILTERTYPE_ASINK, 
    [](const std::string& name, const FilterType type) {return std::make_shared<AudioSinkFilter>(name, FilterType::FILTERTYPE_ASINK); });

AudioSinkFilter::AudioSinkFilter(const std::string& name, FilterType filterType) : Filter(name, FilterType::FILTERTYPE_ASINK)
{
...
Status AudioSinkFilter::Pause()
{
    ...
    auto err = Filter::Pause();
    FALSE_RETURN_V_W(err == Status::OK, err);
    err = audioSink_->Pause();
