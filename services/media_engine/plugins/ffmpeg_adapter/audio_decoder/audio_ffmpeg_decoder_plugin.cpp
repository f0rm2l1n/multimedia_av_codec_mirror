...
Status RegisterAudioDecoderPlugins(const std::shared_ptr<Register> &reg)
{
    ...
    while ((codec = av_codec_iterate(&ite))) {
        ...
        definition.name = "OH.Media.Codec.Decoder.Audio.Mpeg";
        ...
        std::cout << "DecDbg "
                  << "register plugin " << definition.name.c_str() << " succ "
                  << "codec->codec " << codec->name << "  " << codec->long_name << std::endl;
    }
    return Status::OK;
}
...
AudioFfmpegDecoderPlugin::AudioFfmpegDecoderPlugin(std::string name)
    : CodecPlugin(std::move(name)), channels_(0), sampleRate_(0), bit_rate_(0), maxInputSize_(INPUT_BUFFER_SIZE_DEFAULT)
{
}
...
Status AudioFfmpegDecoderPlugin::GetParameter(std::shared_ptr<Meta> &parameter)
{
    ...
    int32_t maxSize = (sampleRate_ / SAMPLE_RATE_RATIO + BUFFER_DIFF) * channels_ * sizeof(short);
    int32_t minSize = MIN_OUTBUF_SIZE * channels_ * sizeof(short);
    if (maxSize < minSize) {
        maxSize = minSize;
    }
    maxOutputSize_ = maxSize;
    // add codec parameter
...
Status AudioFfmpegDecoderPlugin::Prepare()
{
    ...
    auto tmpCtx = std::shared_ptr<AVCodecContext>(context, [](AVCodecContext *ptr) { avcodec_free_context(&ptr); });
    {
        std::lock_guard<std::mutex> lock1(parameterMutex_);
...
Status AudioFfmpegDecoderPlugin::QueueInputBuffer(const std::shared_ptr<AVBuffer> &inputAvBuffer)
{
    ...
    if (inputBuffer->GetSize() == 0 && !(inputAvBuffer->flag_ & BUFFER_FLAG_EOS)) {
        std::cout << "size is 0, but flag is not 1. size:" << inputBuffer->GetSize()
                  << " flag_:" << inputAvBuffer->flag_ << std::endl;
