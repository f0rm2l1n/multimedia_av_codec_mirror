/*
 * Copyright (c) 2024-2025 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
#include "ffmpeg_cook_decoder_plugin.h"
#include "avcodec_codec_name.h"
#include "avcodec_log.h"
#include "plugin/codec_plugin.h"
#include "plugin/plugin_definition.h"
#include "avcodec_mime_type.h"

namespace {
using namespace OHOS::Media;
using namespace OHOS::Media::Plugins;
using namespace Ffmpeg;

constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, LOG_DOMAIN_AUDIO, "AvCodec-AudioFFMpegCookDecoderPlugin"};
constexpr int32_t MIN_CHANNELS = 1;
constexpr int32_t MAX_CHANNELS = 2;
constexpr int32_t SUPPORT_SAMPLE_RATE = 4;
constexpr int32_t INPUT_BUFFER_SIZE_DEFAULT = 8192;
constexpr int32_t OUTPUT_BUFFER_SIZE_DEFAULT = 4 * 1024 * 8;
constexpr int32_t SAMPLE_RATE_PICK[SUPPORT_SAMPLE_RATE] = {8000, 11025, 22050, 44100};
}

namespace OHOS {
namespace Media {
namespace Plugins {
namespace Ffmpeg {
FFmpegCookDecoderPlugin::FFmpegCookDecoderPlugin(const std::string& name)
    : CodecPlugin(name), channels(0), sampleRate(0), blockAlign(0), basePlugin(std::make_unique<FfmpegBaseDecoder>())
{
}

FFmpegCookDecoderPlugin::~FFmpegCookDecoderPlugin()
{
    basePlugin->Release();
    basePlugin.reset();
    basePlugin = nullptr;
}

Status FFmpegCookDecoderPlugin::Init()
{
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::Prepare()
{
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::Reset()
{
    return basePlugin->Reset();
}

Status FFmpegCookDecoderPlugin::Start()
{
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::Stop()
{
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::SetParameter(const std::shared_ptr<Meta> &parameter)
{
    Status ret = basePlugin->AllocateContext("cook");
    Status checkresult = CheckInit(parameter);
    if (checkresult != Status::OK) {
        return checkresult;
    }
    if (ret != Status::OK) {
        AVCODEC_LOGE("Cook init error.");
        return ret;
    }
    ret = basePlugin->InitContext(parameter);
    if (ret != Status::OK) {
        AVCODEC_LOGE("Cook init error.");
        return ret;
    }
    basePlugin->SetBlockAlignContext(blockAlign);
    auto format = basePlugin->GetFormat();
    format->SetData(Tag::AUDIO_MAX_INPUT_SIZE, GetInputBufferSize());
    format->SetData(Tag::AUDIO_MAX_OUTPUT_SIZE, GetOutputBufferSize());
    return basePlugin->OpenContext();
}

Status FFmpegCookDecoderPlugin::GetParameter(std::shared_ptr<Meta> &parameter)
{
    auto format = basePlugin->GetFormat();
    format->SetData(Tag::MIME_TYPE, MediaAVCodec::AVCodecMimeType::MEDIA_MIMETYPE_AUDIO_COOK);
    parameter = format;
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::QueueInputBuffer(const std::shared_ptr<AVBuffer> &inputBuffer)
{
    return basePlugin->ProcessSendData(inputBuffer);
}

Status FFmpegCookDecoderPlugin::QueueOutputBuffer(std::shared_ptr<AVBuffer> &outputBuffer)
{
    return basePlugin->ProcessReceiveData(outputBuffer);
}

Status FFmpegCookDecoderPlugin::GetInputBuffers(std::vector<std::shared_ptr<AVBuffer>> &inputBuffers)
{
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::GetOutputBuffers(std::vector<std::shared_ptr<AVBuffer>> &outputBuffers)
{
    return Status::OK;
}

Status FFmpegCookDecoderPlugin::Flush()
{
    return basePlugin->Flush();
}

Status FFmpegCookDecoderPlugin::Release()
{
    return basePlugin->Release();
}

Status FFmpegCookDecoderPlugin::CheckInit(const std::shared_ptr<Meta> &format)
{
    format->GetData(Tag::AUDIO_CHANNEL_COUNT, channels);
    format->GetData(Tag::AUDIO_SAMPLE_RATE, sampleRate);
    std::vector<uint8_t> extradata;
    format->GetData(Tag::MEDIA_CODEC_CONFIG, extradata);
    if (channels < MIN_CHANNELS || channels > MAX_CHANNELS) {
        AVCODEC_LOGE("check init failed, because channel:%{public}d not support", channels);
        return Status::ERROR_INVALID_PARAMETER;
    }

    for (int32_t i = 0; i < SUPPORT_SAMPLE_RATE; i++) {
        if (sampleRate == SAMPLE_RATE_PICK[i]) {
            break;
        } else if (i == SUPPORT_SAMPLE_RATE - 1) {
            AVCODEC_LOGE("check init failed, because sampleRate:%{public}d not support", sampleRate);
            return Status::ERROR_INVALID_PARAMETER;
        }
    }
    uint32_t minExtradataSize = 8;
    if (extradata.size() < minExtradataSize) {
        AVCODEC_LOGE("Missing or incomplete extradata, size: %{public}" PRIu32,
                     static_cast<uint32_t>(extradata.size()));
        return Status::ERROR_INVALID_PARAMETER;
    }
    if (!format->GetData(Tag::AUDIO_BLOCK_ALIGN, blockAlign)) {
        AVCODEC_LOGE("check init failed, can't get AUDIO_BLOCK_ALIGN");
        return Status::ERROR_INVALID_PARAMETER;
    }
    if (!basePlugin->CheckSampleFormat(format, channels)) {
        AVCODEC_LOGE("check init failed, because CheckSampleFormat failed.");
        return Status::ERROR_INVALID_PARAMETER;
    }
    return Status::OK;
}

int32_t FFmpegCookDecoderPlugin::GetInputBufferSize()
{
    int32_t maxSize = basePlugin->GetMaxInputSize();
    if (maxSize < 0 || maxSize > INPUT_BUFFER_SIZE_DEFAULT) {
        maxSize = INPUT_BUFFER_SIZE_DEFAULT;
    }
    return maxSize;
}

int32_t FFmpegCookDecoderPlugin::GetOutputBufferSize()
{
    return OUTPUT_BUFFER_SIZE_DEFAULT;
}
} // namespace Ffmpeg
} // namespace Plugins
} // namespace Media
} // namespace OHOS
