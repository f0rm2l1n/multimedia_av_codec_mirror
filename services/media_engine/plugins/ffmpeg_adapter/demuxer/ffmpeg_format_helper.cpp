...
{
    MEDIA_LOG_D("CodecID " PUBLIC_LOG_D32 "[" PUBLIC_LOG_S "].", static_cast<int32_t>(codecID), avcodec_get_name(codecID));
...
FileType GetFileTypeByName(const AVFormatContext& avFormatContext)
{
    ...
    MEDIA_LOG_D("file name [" PUBLIC_LOG_S "] file type [" PUBLIC_LOG_D32 "].", fileName, static_cast<int32_t>(fileType));
    return fileType;
}
} // namespace
...
void FFmpegFormatHelper::ParseMediaInfo(const AVFormatContext& avFormatContext, Meta& format)
{
    ...
    if (duration == AV_NOPTS_VALUE) {
        ...
        if (metaDuration != nullptr && (av_parse_time(&us, metaDuration->value, 1) == 0)) {
            if (us > duration) {
                MEDIA_LOG_D("Parse duration from metadata.");
                duration = us;
            }
        }
    }
    if (duration <= 0) {
        for (uint32_t i = 0; i < avFormatContext.nb_streams; ++i) {
            ...
            if (streamDuration > duration) {
                MEDIA_LOG_D("Parse duration from track ." PUBLIC_LOG_U32, i);
    ...
    if (duration > 0) {
        ...
        MEDIA_LOG_W("Parse duration info failed.");
    }
...
void FFmpegFormatHelper::ParseBaseTrackInfo(const AVStream& avStream, Meta &format)
{
    if (g_codecIdToMime.count(avStream.codecpar->codec_id) != 0) {
        ...
        MEDIA_LOG_W("Parse mime type info failed: " PUBLIC_LOG_D32 ".", static_cast<int32_t>(avStream.codecpar->codec_id));
    }
    ...
    if (g_convertFfmpegTrackType.count(mediaType) > 0) {
        ...
        MEDIA_LOG_W("Parse track type info failed: " PUBLIC_LOG_D32 ".", static_cast<int32_t>(avStream.codecpar->codec_type));
    }
}
...
void FFmpegFormatHelper::ParseVideoTrackInfo(const AVStream& avStream, Meta &format)
{
    ...
    if (avStream.codecpar->codec_id == AV_CODEC_ID_HEVC) {
        ParseHDRMetadataInfo(avStream, format);
        ParseColorSpaceInfo(avStream, format);
    }
}
...
void FFmpegFormatHelper::ParseColorSpaceInfo(const AVStream& avStream, Meta &format)
{
