...
{
    MEDIA_LOG_D("block queue " PUBLIC_LOG_S " AddTrackQueue enter, trackIndex: " PUBLIC_LOG_U32 ".", name_.c_str(), trackIndex);
    if (!HasQueue(trackIndex)) {
        uint32_t queIndex = GetValidQueue();
        queMap_[trackIndex] = std::vector<uint32_t>({ queIndex });
        MEDIA_LOG_D("block queue " PUBLIC_LOG_S " AddTrackQueue finish, add track " PUBLIC_LOG_U32 ", get queue " PUBLIC_LOG_U32 "",
...
{
    MEDIA_LOG_D("block queue " PUBLIC_LOG_S " RemoveTrackQueue enter, trackIndex: " PUBLIC_LOG_U32 ".", name_.c_str(), trackIndex);
    if (!HasQueue(trackIndex)) {
...
{
    MEDIA_LOG_D("block queue " PUBLIC_LOG_S " HasCache enter, trackIndex: " PUBLIC_LOG_U32 ".", name_.c_str(), trackIndex);
    for (auto queIndex : queMap_[trackIndex]) {
    ...
    for (auto index = 0; index < static_cast<int32_t>(queVector.size()); ++index) {
        ...
        if (quePool_[queIndex].blockQue->Size() > 0) {
            ...
            if (quePool_[queIndex].blockQue->Empty()) {
                ResetQueue(queIndex);
                MEDIA_LOG_D("track " PUBLIC_LOG_U32 " queue " PUBLIC_LOG_D32 " is empty, will return to pool.", trackIndex, queIndex);
                queVector.erase(queVector.begin() + index);
            }
            MEDIA_LOG_D("block queue " PUBLIC_LOG_S " Pop finish, trackIndex: " PUBLIC_LOG_U32 ".", name_.c_str(), trackIndex);
            sizeMap_[trackIndex] -= 1;
            return block;
        }
    }
...
{
    MEDIA_LOG_D("block queue " PUBLIC_LOG_S " InnerQueueIsFull enter, queueIndex: " PUBLIC_LOG_U32 ".", name_.c_str(), queueIndex);
...
{
    MEDIA_LOG_D("block queue " PUBLIC_LOG_S " HasQueue enter, trackIndex: " PUBLIC_LOG_U32 ".", name_.c_str(), trackIndex);
