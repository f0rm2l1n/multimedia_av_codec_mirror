...
bool CacheMediaChunkBufferImpl::Init(uint64_t totalBuffSize, uint32_t chunkSize)
{
    ...
    MEDIA_LOG_D("Buffer init, totalBuffer: %{public}llu, chunkSize: %{public}u", totalBuffSize, chunkSize);
    std::lock_guard lock(mutex_);
    if (bufferAddr_ != nullptr) {
        return false;
    }
    ...
    FALSE_RETURN_V_MSG_E(sizePerChunk * chunkNum > 0, false, "Invalid sizePerChunk and chunkNum.");
...
size_t CacheMediaChunkBufferImpl::Read(void* ptr, int64_t offset, size_t readSize)
{
    ...
    if (chunkPos != fragmentPos->chunks.end()) {
        ...
        while (hasReadSize < readSize && chunkPos != fragmentPos->chunks.end()) {
            ...
            (void)memcpy_s(dst + hasReadSize, readOne, (*chunkPos)->data + diff, readOne);
            hasReadSize += readOne;
            offsetChunk += readOne;
            chunkPos++;
        }
                                             ...
                                             size_t inWriteSize, size_t& outWriteSize)
{
    ...
    ChunkIterator chunkPos = std::upper_bound(chunkList.begin(), chunkList.end(), offset,
        ...
        if (inputOffset <= chunk->offset + chunk->dataLength) {
            return true;
        }
        return false;
        });
    if (chunkPos == chunkList.end()) {
        DumpInner(0);
        MEDIA_LOG_E("fins chunk happen error. in offset: %lld", offset);
        return false;
    }
    size_t writeSizeTmp = 0;
    auto chunkInfoTmp = *chunkPos;
    if (chunkInfoTmp->offset <= offset && offset < chunkInfoTmp->offset + chunkInfoTmp->dataLength) {
        auto diff = offset - chunkInfoTmp->offset;
        size_t copyLen = (size_t)(chunkInfoTmp->dataLength - diff);
        copyLen = std::min(copyLen, writeSize);
        (void)memcpy_s(chunkInfoTmp->data + diff, copyLen, src, copyLen);
    ...
    writePos_->accessLength = inOffset - writePos_->offsetBegin;
    while (writeSizeTmp < writeSize && chunkPos != chunkList.end()) {
        ...
        (void)memcpy_s(chunkInfoTmp->data, copyLen, src, copyLen);
        IncreaseStep(src, offset, writeSizeTmp, copyLen);
        ++chunkPos;
    }
...
bool CacheMediaChunkBufferImpl::WriteMergerPre(int64_t offset, size_t writeSize, FragmentIterator& nextFragmentPos)
{
    ...
    while (isLoop) {
        if (nextFragmentPos == fragmentCacheBuffer_.end() ||
            offset + (int64_t) writeSize < nextFragmentPos->offsetBegin) {
            nextFragmentPos = fragmentCacheBuffer_.end();
            isLoop = false;
            break;
        }
        if (offset + (int64_t)writeSize < nextFragmentPos->offsetBegin + nextFragmentPos->dataLength) {
            auto endPos = GetOffsetChunkCache(nextFragmentPos->chunks,
                                              offset + (int64_t)writeSize, LeftBoundedRightOpenComp);
            ...
            int64_t newOffset = offset + (int64_t)writeSize;
            size_t moveLen = (size_t)(chunkInfo->offset + chunkInfo->dataLength - newOffset);
            auto mergeDataLen = chunkInfo->dataLength - (int64_t)moveLen;
            (void)memmove_s(chunkInfo->data, moveLen, chunkInfo->data + mergeDataLen, moveLen);
            chunkInfo->offset = newOffset;
            chunkInfo->dataLength = (uint32_t)moveLen;
...
bool CacheMediaChunkBufferImpl::Seek(int64_t offset)
{
    ...
    if (readPos_ != fragmentCacheBuffer_.end()) {
        readPos_ = readPos;
...
inline size_t WriteOneChunkData(CacheChunk& chunkInfo, uint8_t* src, int64_t offset, size_t writeSize)
{
    ...
    size_t writePerOne = (size_t)(chunkInfo.chunkSize - copyBegin);
    writePerOne = std::min(writePerOne, writeSize);
    (void)memcpy_s(chunkInfo.data + copyBegin, writePerOne, src, writePerOne);
    chunkInfo.dataLength = (uint32_t)((size_t)copyBegin + writePerOne);
    return writePerOne;
}
                                             ...
                                             void* ptr, int64_t offset, size_t writeSize)
{
    ...
    if (chunkInfo->chunkSize > chunkInfo->dataLength) {
        wroteTmp += WriteOneChunkData(*chunkInfo, src, offset, writeSize);
        fragmentCacheBuffer.dataLength += wroteTmp;
    }
    while (wroteTmp < writeSize) {
        auto chunkOffset = offset + (int64_t)wroteTmp;
        auto freeChunk = GetFreeCacheChunk(chunkOffset);
        if (freeChunk == nullptr) {
            return wroteTmp;
        }
        auto writePerOne = WriteOneChunkData(*freeChunk, src + wroteTmp, chunkOffset, writeSize - wroteTmp);
        fragmentCacheBuffer.chunks.push_back(freeChunk);
        wroteTmp += writePerOne;
        fragmentCacheBuffer.dataLength += writePerOne;
...
CacheChunk* UpdateFragmentCacheForDelTail(FragmentCacheBuffer& fragment)
{
    ...
    auto dataLength = cacheChunk->dataLength;
    if (fragment.accessLength > fragment.dataLength - dataLength) {
        fragment.accessLength = fragment.dataLength - dataLength;
    }
    fragment.dataLength -= dataLength;
    return cacheChunk;
}
...
void CacheMediaChunkBufferImpl::DeleteHasReadFragmentCacheBuffer(FragmentIterator& fragmentIter, size_t allowChunkNum)
{
    ...
    while (fragmentCacheChunks.chunks.size() >= allowChunkNum) {
        if (fragmentCacheChunks.accessPos != fragmentCacheChunks.chunks.end()) {
            MEDIA_LOG_D("access pos chunk. offset: %{public}lld, length: %{public}u",
                (*fragmentCacheChunks.accessPos)->offset, (*fragmentCacheChunks.accessPos)->dataLength);
        } else {
            MEDIA_LOG_D("access pos is end");
        }
        ...
        if (fragmentCacheChunks.accessPos != fragmentCacheChunks.chunks.begin()) {
            ...
            MEDIA_LOG_D("delete has read chunk. offset: %{public}lld, length:%{public}u",
                tmp->offset, tmp->dataLength);
        } else {
...
void CacheMediaChunkBufferImpl::DeleteUnreadFragmentCacheBuffer(FragmentIterator& fragmentIter, size_t allowChunkNum)
{
    ...
    while (fragmentCacheChunks.chunks.size() > allowChunkNum) {
        if (!fragmentCacheChunks.chunks.empty()) {
            ...
            freeChunks_.push_back(tmp);
            MEDIA_LOG_D("delete unread chunk. offset: %{public}lld, length: %{public}u, list size: " PUBLIC_LOG_ZU,
                tmp->offset, tmp->dataLength, fragmentCacheChunks.chunks.size());
        } else {
            break;
        }
    }
...
CacheChunk* CacheMediaChunkBufferImpl::GetFreeCacheChunk(int64_t offset)
{
    ...
    if (currWritePos != fragmentCacheBuffer_.end()) {
        MEDIA_LOG_D("curr fragment: offset: %{public}lld, dataLen:%{public}lld, accessLen:%{public}lld",
            currWritePos->offsetBegin, currWritePos->dataLength, currWritePos->accessLength);
        ...
        if (currWritePos->chunks.size() >= allowChunkNum) {
            MEDIA_LOG_D("The data has not been read. offset:%{public}lld, allow num: " PUBLIC_LOG_ZU
                ", chunk num:" PUBLIC_LOG_ZU, offset, allowChunkNum, currWritePos->chunks.size());
            return nullptr;
        }
    } else {
...
ChunkIterator CacheMediaChunkBufferImpl::AddFragmentCacheBuffer(int64_t offset)
{
    ...
    uint64_t newReadSizeInit = static_cast<uint64_t>(1 + initReadSizeFactor_ * (double)totalReadSize_);
    MEDIA_LOG_D("Init fragment read size:%{public}llu, totalReadSize:%{public}llu", newReadSizeInit, totalReadSize_);
...
{
    (void)param;
    MEDIA_LOG_I("total buffer size : %{public}llu", totalBuffSize_);
    MEDIA_LOG_I("total chunk size  : %{public}u", chunkSize_);
    MEDIA_LOG_I("total chunk num   : %{public}u", chunkMaxNum_);
    MEDIA_LOG_I("total read size   : %{public}llu", totalReadSize_);
    MEDIA_LOG_I("read size factor  : %{public}.2f", initReadSizeFactor_);
    MEDIA_LOG_I("free chunk num    : %{public}u", freeChunks_.size());
    MEDIA_LOG_I("fragment num      : %{public}u", fragmentCacheBuffer_.size());
    for (auto const & fragment : fragmentCacheBuffer_) {
        MEDIA_LOG_I("fragment offset : %{public}lld", fragment.offsetBegin);
        MEDIA_LOG_I("fragment length : %{public}lld", fragment.dataLength);
        MEDIA_LOG_I("chunk num       : %{public}u", fragment.chunks.size());
        MEDIA_LOG_I("access length   : %{public}lld", fragment.accessLength);
        MEDIA_LOG_I("read size       : %{public}lld", fragment.totalReadSize);
        if (fragment.accessPos != fragment.chunks.end()) {
            auto &chunkInfo = *fragment.accessPos;
            MEDIA_LOG_I("access offset : %{public}lld, len : %{public}u", chunkInfo->offset, chunkInfo->dataLength);
        } else {
            MEDIA_LOG_I("access is end");
        }
        if (!fragment.chunks.empty()) {
            auto &chunkInfo = fragment.chunks.back();
            MEDIA_LOG_I("last chunk offset : %{public}lld, len : %{public}u", chunkInfo->offset, chunkInfo->dataLength);
        }
        MEDIA_LOG_I("");
    }
}
...
void CacheMediaChunkBufferImpl::CheckFragment(const FragmentCacheBuffer& fragment, bool& checkSuccess)
{
    if (fragment.accessPos != fragment.chunks.end()) {
        ...
        if (fragment.accessLength < accessLength ||
            fragment.accessLength > (accessLength + accessChunk->dataLength)) {
            checkSuccess = false;
            MEDIA_LOG_E("fragment access len: %{public}lld, offset: %{public}lld, "
                "access pos offset: %{public}lld, len: %{public}u",
                fragment.accessLength, fragment.offsetBegin, accessChunk->offset, accessChunk->dataLength);
        }
    }
}
...
bool CacheMediaChunkBufferImpl::CheckInner()
{
    ...
    for (auto const& fragment : fragmentCacheBuffer_) {
        ...
        if (!fragment.chunks.empty()) {
            ...
            if ((*prev)->offset != fragment.offsetBegin) {
                checkSuccess = false;
                MEDIA_LOG_E("first chunk offset: %{public}lld, fragment offset: %{public}lld",
                    (*prev)->offset, fragment.offsetBegin);
            }
        }
        while (next != fragment.chunks.end()) {
            auto &chunkPrev = *prev;
            auto &chunkNext = *next;
            dataLength += chunkNext->dataLength;
            if (chunkPrev->offset + chunkPrev->dataLength != chunkNext->offset) {
                checkSuccess = false;
                MEDIA_LOG_E("curr offset: %{public}lld, length: %{public}u, next offset: %{public}lld",
                    chunkPrev->offset, chunkPrev->dataLength, chunkNext->offset);
            }
            ++next;
            ++prev;
        }
        if (dataLength != fragment.dataLength) {
            checkSuccess = false;
            MEDIA_LOG_E("fragment data len :%{public}lld, chunk total len: %{public}lld",
                fragment.dataLength, dataLength);
        }
    ...
    if (chunkNum != chunkMaxNum_) {
        checkSuccess = false;
        MEDIA_LOG_E("stat chunkNum: %{public}llu, chunkMaxNum_: %{public}u", chunkNum, chunkMaxNum_);
    }

    if (totalReadSize != totalReadSize_) {
        checkSuccess = false;
        MEDIA_LOG_E("stat totalReadSize: %{public}llu, totalReadSize_: %{public}llu", totalReadSize, totalReadSize_);
    }
    return checkSuccess;
}
