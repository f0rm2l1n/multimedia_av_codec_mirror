...
constexpr int CACHE_WATER_LINE = 521 * 1024;
...
{
    if (isFirstFrameArrived_ && GetCurrentBufferSize() < PLAY_WATER_LINE) {
        bufferingTimes_++;
...
Status HttpMediaDownloader::ReadRingBuffer(unsigned char* buff, ReadDataInfo& readDataInfo)
{
    ...
    while (buffer_->GetSize() < readDataInfo.wantReadLength_ && !isInterruptNeeded_) {
...
bool HttpMediaDownloader::CheckIsEosBeforeTimeout(unsigned char* buff, ReadDataInfo& readDataInfo)
{
    ...
    if (readDataInfo.isEos_ && downloadRequest_->IsChunkedVod()) {
        readDataInfo.realReadLength_ = cacheMediaBuffer_->Read(buff, readOffset_,
            readDataInfo.wantReadLength_);
        return true;
    }
    return false;
