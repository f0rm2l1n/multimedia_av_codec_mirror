...
constexpr float DEFAULT_CACHE_TIME = 10;
constexpr size_t MAX_BUFFERING_TIME_OUT = 30 * 1000;
constexpr size_t MAX_WATER_LINE_ABOVE = 1 * 1024 * 1024;
}
...
bool HttpMediaDownloader::SaveCacheBufferData(uint8_t* data, uint32_t len)
{
    ...
    while (hasWriteSize < len && !isInterruptNeeded_.load() && !isInterrupt_) {
        ...
        MEDIA_LOGI_LIMIT(SAVE_DATA_LOG_FREQUENCE, "writeOffset "
            PUBLIC_LOG_ZU " res " PUBLIC_LOG_ZU, writeOffset_, res);
