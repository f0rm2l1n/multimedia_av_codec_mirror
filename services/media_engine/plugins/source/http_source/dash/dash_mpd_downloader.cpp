...
static DashSegmentInitValue AddOneSegment(unsigned int segRealDur, int64_t segmentSeq, std::string tempUrl, std::shared_ptr<DashStreamDescription> streamDesc)
{
    ...
    //MEDIA_LOG_I("AddOneSegment:seq:" PUBLIC_LOG_D64 ", segRealDur:" PUBLIC_LOG_U32 ", url:" PUBLIC_LOG_S, segmentSeq, segRealDur, tempUrl.c_str());
...
static DashSegmentInitValue AddOneSegment(const DashSegment& srcSegment, std::shared_ptr<DashStreamDescription> streamDesc)
{
    ...
    //MEDIA_LOG_I("AddOneSegment:seq:" PUBLIC_LOG_D64 ", url:" PUBLIC_LOG_S, segment->numberSeq_, segment->url_.c_str());
    streamDesc->mediaSegments_.push_back(segment);
    return DASH_SEGMENT_INIT_SUCCESS;
}
...
static DashRepresentationInfo* GetRepresentationFromAdptSet(DashAdptSetInfo* adptSet, unsigned int repreIndex)
{
    ...
    for (DashList<DashRepresentationInfo*>::iterator it = adptSet->representationList_.begin(); it != adptSet->representationList_.end(); it++, index++) {
        if (index == repreIndex) {
            return *it;
        }
    }
    ...
    for (auto &streamDescription : streamDescriptions_) {
        if (streamDescription->streamId_ == streamId) {
            if (streamDescription->segsState_ == DASH_SEGS_STATE_FINISH) {
                int64_t segmentIndex = (streamDescription->currentNumberSeq_ == -1) ? 0 : streamDescription->currentNumberSeq_ - streamDescription->startNumberSeq_ + 1;
                MEDIA_LOG_I("get segment index :" PUBLIC_LOG_D64 ", id:" PUBLIC_LOG_D32 ", seq:" PUBLIC_LOG_D64, segmentIndex, streamDescription->streamId_, streamDescription->currentNumberSeq_);
                if (segmentIndex >= 0 && (unsigned int)segmentIndex < streamDescription->mediaSegments_.size()) {
                    ...
                    MEDIA_LOG_I("after get segment index :" PUBLIC_LOG_D64, streamDescription->currentNumberSeq_);
...
DashMpdGetRet DashMpdDownloader::GetBreakPointSegment(int streamId, int64_t breakpoint, std::shared_ptr<DashSegment>& seg)
{
    ...
    for (auto &streamDescription : streamDescriptions_) {
        if (streamDescription->streamId_ == streamId) {
            if (streamDescription->segsState_ == DASH_SEGS_STATE_FINISH) {
                ...
                if (seg != nullptr) {
                    streamDescription->currentNumberSeq_ = seg->numberSeq_;
                    MEDIA_LOG_I("GetBreakPointSegment find segment index :" PUBLIC_LOG_D64, streamDescription->currentNumberSeq_);
                    ret = DASH_MPD_GET_DONE;
                } else {
...
DashMpdGetRet DashMpdDownloader::GetNextVideoStream(const DashMpdBitrateParam& param, int& streamId)
{
    ...
    for (auto stream : streamDescriptions_) {
        ...
        if (stream->bandwidth_ == param.bitrate_) {
            ...
            MEDIA_LOG_I("switch to bandwidth:" PUBLIC_LOG_U32 ", id:" PUBLIC_LOG_D32 ", width:" PUBLIC_LOG_U32, stream->bandwidth_, stream->streamId_, stream->width_);
        }
    ...
    if (destStream->startNumberSeq_ != currentStream->startNumberSeq_) {
        MEDIA_LOG_E("select bitrate:" PUBLIC_LOG_U32 " but seq:" PUBLIC_LOG_D64 " is not equal bitrate:" PUBLIC_LOG_U32 ", seq:" PUBLIC_LOG_D64,
            destStream->bandwidth_, destStream->startNumberSeq_, currentStream->bandwidth_, currentStream->startNumberSeq_);
    }
    ...
    MEDIA_LOG_I("GetNextVideoStream update id:" PUBLIC_LOG_D32 ", seq:" PUBLIC_LOG_D64, destStream->streamId_, destStream->currentNumberSeq_);
        ...
        if (ondemandSegBase_) {
            ...
            if (destStream->indexSegment_ != nullptr) {
                ...
                if (CheckToDownloadSidxWithInitSeg(destStream)) {
                    MEDIA_LOG_I("update range begin from " PUBLIC_LOG_D64 " to " PUBLIC_LOG_D64, startRange, destStream->initSegment_->rangeBegin_);
                    startRange = destStream->initSegment_->rangeBegin_;
                }
                DoOpen(destStream->indexSegment_->url_, startRange, endRange);
            } else {
                MEDIA_LOG_E("GetNextSegmentByBitrate id:" PUBLIC_LOG_D32 " ondemandSegBase_ but indexSegment is null", destStream->streamId_);
            }
        } else {
            // get segment list
            if (GetSegmentsInMpd(destStream) == DASH_SEGMENT_INIT_FAILED) {
                MEDIA_LOG_E("GetNextSegmentByBitrate id:" PUBLIC_LOG_D32 " GetSegmentsInMpd failed", destStream->streamId_);
            }
            // 根据position查找分片
            ret = DASH_MPD_GET_DONE;
        }
...
void DashMpdDownloader::SetCurrentNumberSeqByStreamId(int streamId, int64_t numberSeq)
{
    for (unsigned int index = 0; index < streamDescriptions_.size(); index++) {
        if (streamDescriptions_[index]->streamId_ == streamId) {
            ...
            MEDIA_LOG_I("SetCurrentNumberSeqByStreamId update id:" PUBLIC_LOG_D32 ", seq:" PUBLIC_LOG_D64, streamId, numberSeq);
            break;
        }
    }
}
            ...
            for (auto stream : streamDescriptions_) {
                if (stream->inUse_ && stream->indexSegment_ != nullptr && stream->segsState_ != DASH_SEGS_STATE_FINISH) {
                    ...
                    if (CheckToDownloadSidxWithInitSeg(stream)) {
                        MEDIA_LOG_I("update range begin from " PUBLIC_LOG_D64 " to " PUBLIC_LOG_D64, startRange, stream->initSegment_->rangeBegin_);
...
{
    if (downloadContent_.length() == 0 || currentDownloadStream_ == nullptr || currentDownloadStream_->indexSegment_ == nullptr) {
    ...
    if (CheckToDownloadSidxWithInitSeg(currentDownloadStream_)) {
        ...
        MEDIA_LOG_I("ParseSidx update init segment content size is " PUBLIC_LOG_ZU, currentDownloadStream_->initSegment_->content_.size());
        int64_t initLen = currentDownloadStream_->indexSegment_->indexRangeBegin_ - currentDownloadStream_->initSegment_->rangeBegin_;
        if (initLen >= 0 && (unsigned int)initLen < downloadContent_.size()) {
            sidxContent = downloadContent_.substr((unsigned int)initLen);
            MEDIA_LOG_I("ParseSidx update sidx segment content size is " PUBLIC_LOG_ZU ", " PUBLIC_LOG_C "-" PUBLIC_LOG_C, sidxContent.size(), sidxContent[0], sidxContent[1]);
        }
    }

    std::list<std::shared_ptr<SubSegmentIndex>> subSegIndexList;
    int32_t parseRet = SidxBoxParser::ParseSidxBox(const_cast<char*>(sidxContent.c_str()), sidxContent.length(), currentDownloadStream_->indexSegment_->indexRangeEnd_, subSegIndexList);
    if (parseRet != 0) {
...
void DashMpdDownloader::DoOpen(const std::string& url, int64_t startRange, int64_t endRange)
{
    ...
    MEDIA_LOG_I("DoOpen:start=%{public}lld end=%{public}lld url=%{public}s", (long long)startRange, (long long)endRange, url.c_str());
...
void DashMpdDownloader::SelectBitRate(uint32_t bitRate)
{
    ...
    for (auto &stream : streamDescriptions_) {
        ...
        if (stream->streamId_ == selectVideoStreamId_) {
            ...
            MEDIA_LOG_I("SelectBitRate update id:" PUBLIC_LOG_D32 ", seq:" PUBLIC_LOG_D64, stream->streamId_, stream->currentNumberSeq_);
...
void DashMpdDownloader::SeekToTs(int streamId, int64_t seekTime, std::shared_ptr<DashSegment>& seg)
{
    ...
    for (const auto &index : streamDescriptions_) {
        ...
        if (streamDescription != nullptr && streamDescription->streamId_ == streamId && streamDescription->segsState_ == DASH_SEGS_STATE_FINISH) {
 ...
 */
DashSegmentInitValue DashMpdDownloader::GetSegmentsByPeriodInfo(DashPeriodInfo* periodInfo, DashAdptSetInfo* adptSetInfo, std::string& periodBaseUrl, std::shared_ptr<DashStreamDescription> streamDesc)
{
    ...
    if (periodInfo->periodSegTmplt_ != nullptr && periodInfo->periodSegTmplt_->segTmpltMedia_.length() > 0) {
        ...
        if (adptSetInfo != nullptr && streamDesc->bandwidth_ > 0) {
            DashRepresentationInfo* repInfo = GetRepresentationFromAdptSet(adptSetInfo, streamDesc->representationIndex_);
            if (repInfo != nullptr) {
...
void DashMpdDownloader::GetStreamsInfoInPeriod(DashPeriodInfo* periodInfo, unsigned int periodIndex, std::string mpdBaseUrl)
{
    ...
    if (periodInfo->adptSetList_.size() == 0) {
        ...
        if (ondemandSegBase_ && periodInfo->periodSegBase_ != nullptr && periodInfo->periodSegBase_->indexRange_.size() > 0) {
            desc->indexSegment_ = std::make_shared<DashIndexSegment>();
            desc->indexSegment_->url_ = periodBaseUrl;
            DashParseRange(periodInfo->periodSegBase_->indexRange_, desc->indexSegment_->indexRangeBegin_, desc->indexSegment_->indexRangeEnd_);
        }
    ...
    for (int32_t type = MediaAVCodec::MediaType::MEDIA_TYPE_AUD; type <= MediaAVCodec::MediaType::MEDIA_TYPE_SUBTITLE; type++) {
...
void DashMpdDownloader::GetStreamsInfoInAdptSet(DashAdptSetInfo* adptSetInfo, std::string periodBaseUrl, DashStreamDescription& streamDesc)
{
    ...
    if (repList.size() == 0) {
        ...
        if (ondemandSegBase_ && adptSetInfo->adptSetSegBase_ != nullptr && adptSetInfo->adptSetSegBase_->indexRange_.size() > 0) {
            desc->indexSegment_ = std::make_shared<DashIndexSegment>();
            desc->indexSegment_->url_ = adptSetBaseUrl;
            DashParseRange(adptSetInfo->adptSetSegBase_->indexRange_, desc->indexSegment_->indexRangeBegin_, desc->indexSegment_->indexRangeEnd_);
        }
        streamDescriptions_.push_back(desc);
        return;
    }
    ...
    for (std::list<DashRepresentationInfo*>::iterator it = repList.begin(); it != repList.end(); it++, repIndex++) {
        if (*it != nullptr) {
            ...
            if (ondemandSegBase_ && (*it)->representationSegBase_ != nullptr && (*it)->representationSegBase_->indexRange_.size() > 0) {
                desc->indexSegment_ = std::make_shared<DashIndexSegment>();
                desc->indexSegment_->url_ = repBaseUrl;
                DashParseRange((*it)->representationSegBase_->indexRange_, desc->indexSegment_->indexRangeBegin_, desc->indexSegment_->indexRangeEnd_);
            }
...
bool DashMpdDownloader::ChooseStreamToPlay(MediaAVCodec::MediaType type)
{
    ...
    if (choosedStream != nullptr) {
        ...
        MEDIA_LOG_I("ChooseStreamToPlay type:" PUBLIC_LOG_D32 ", streamId:" PUBLIC_LOG_D32, (int)type, choosedStream->streamId_);
        if (!ondemandSegBase_) {
...
DashSegmentInitValue DashMpdDownloader::GetSegmentsInMpd(std::shared_ptr<DashStreamDescription> streamDesc)
{
    ...
    for (std::list<DashPeriodInfo*>::iterator it = mpdInfo_->periodInfoList_.begin(); it != mpdInfo_->periodInfoList_.end(); it++, currentPeriodIndex++) {
        if (*it != nullptr && currentPeriodIndex == streamDesc->periodIndex_) {
            if (GetSegmentsInPeriod(*it, mpdBaseUrl, streamDesc) == DASH_SEGMENT_INIT_FAILED) {
                MEDIA_LOG_I("GetSegmentsInPeriod" PUBLIC_LOG_U32 " failed, type " PUBLIC_LOG_D32, currentPeriodIndex, streamDesc->type_);
                return DASH_SEGMENT_INIT_FAILED;
            }
...
DashSegmentInitValue DashMpdDownloader::GetSegmentsInPeriod(DashPeriodInfo* periodInfo, std::string mpdBaseUrl, std::shared_ptr<DashStreamDescription> streamDesc)
{
    std::vector<DashAdptSetInfo*> adptSetVector;
...
DashSegmentInitValue DashMpdDownloader::GetSegmentsInAdptSet(DashAdptSetInfo* adptSetInfo, std::string periodBaseUrl, std::shared_ptr<DashStreamDescription> streamDesc)
{
    if (adptSetInfo == nullptr) {
...
DashSegmentInitValue DashMpdDownloader::GetSegmentsInRepresentation(DashRepresentationInfo* repInfo, std::string adptSetBaseUrl, std::shared_ptr<DashStreamDescription> streamDesc)
{
...
DashSegmentInitValue DashMpdDownloader::GetSegmentsWithSegTemplate(const DashSegTmpltInfo* segTmpltInfo, std::string id, std::shared_ptr<DashStreamDescription> streamDesc)
{
    ...
    if (DashSubstituteTmpltStr(media, "$RepresentationID", id) == -1) {
        MEDIA_LOG_E("media " PUBLIC_LOG_S " substitute $RepresentationID error " PUBLIC_LOG_S, media.c_str(), id.c_str());
        return DASH_SEGMENT_INIT_FAILED;
    }

    if (DashSubstituteTmpltStr(media, "$Bandwidth", std::to_string(streamDesc->bandwidth_)) == -1) {
        MEDIA_LOG_E("media " PUBLIC_LOG_S " substitute $Bandwidth error " PUBLIC_LOG_D32, media.c_str(), streamDesc->bandwidth_);
        return DASH_SEGMENT_INIT_FAILED;
    }
...
DashSegmentInitValue DashMpdDownloader::GetSegmentsWithTmpltStatic(const DashSegTmpltInfo* segTmpltInfo, std::string mediaUrl,
 ...
 */
DashSegmentInitValue DashMpdDownloader::GetSegmentsWithTmpltDurationStatic(const DashSegTmpltInfo* segTmpltInfo, std::string mediaUrl,
    unsigned int timeScale, std::shared_ptr<DashStreamDescription> streamDesc)
{
    if (timeScale == 0) {
        return DASH_SEGMENT_INIT_FAILED;
    }
    ...
    unsigned int segmentDuration = (static_cast<uint64_t>(segTmpltInfo->multSegBaseInfo_.duration_) * S_2_MS) / timeScale;
    ...
    while (accumulateDuration < streamDesc->duration_) {
        ...
        if (streamDesc->duration_ - accumulateDuration < segRealDur) {
            segRealDur = streamDesc->duration_ - accumulateDuration;
        }
        ...
        // if the @duration attribute is present, then the time address is determined by replacing the $Time$ identifier with ((k-1) + (kStart-1))* @duration with kStart the value of the @startNumber attribute, if present, or 1 otherwise.
        ...
        if (DashSubstituteTmpltStr(tempUrl, "$Time", std::to_string(startTime)) == -1) {
            MEDIA_LOG_I("" PUBLIC_LOG_S " substitute $Time " PUBLIC_LOG_S " error in static duration", tempUrl.c_str(), std::to_string(startTime).c_str());
            return DASH_SEGMENT_INIT_FAILED;
        }

        if (DashSubstituteTmpltStr(tempUrl, "$Number", std::to_string(segmentSeq)) == -1) {
            MEDIA_LOG_I("" PUBLIC_LOG_S " substitute $Number " PUBLIC_LOG_S " error in static duration", tempUrl.c_str(), std::to_string(segmentSeq).c_str());
            return DASH_SEGMENT_INIT_FAILED;
        }

        if (DASH_SEGMENT_INIT_FAILED == AddOneSegment(segRealDur, segmentSeq, tempUrl, streamDesc)) {
            MEDIA_LOG_I("AddOneSegment failed with [static] [duration]");
            return DASH_SEGMENT_INIT_FAILED;
        }
    }

    MEDIA_LOG_I("GetSegmentsWithTmpltDurationStatic:segment size:" PUBLIC_LOG_ZU, streamDesc->mediaSegments_.size());
    return DASH_SEGMENT_INIT_SUCCESS;
}

DashSegmentInitValue DashMpdDownloader::GetSegmentsWithTmpltTimelineStatic(const DashSegTmpltInfo* segTmpltInfo, std::string mediaUrl,
    unsigned int timeScale, std::shared_ptr<DashStreamDescription> streamDesc)
{
    if (timeScale == 0) {
        return DASH_SEGMENT_INIT_FAILED;
    ...
    for (std::list<DashSegTimeline*>::iterator it = timelineList.begin(); it != timelineList.end(); it++) {
        if (*it != nullptr) {
            int segCount = GetSegCountFromTimeline(it, timelineList.end(), streamDesc->duration_, timeScale, startTime);
            if (segCount < 0) {
            ...
            if (GetSegmentsInOneTimeline(*it, segCount, segDuration, mediaUrl, segmentSeq, startTime, streamDesc) == DASH_SEGMENT_INIT_FAILED) {
                return DASH_SEGMENT_INIT_FAILED;
            }
        }
    }
 ...
 * @param    segCount               segment count in timeline
 * @param    segDuration            segment duration in timeline
 * @param    mediaUrl               SegmentTemplate mediaSegment Url
 ...
 */
DashSegmentInitValue DashMpdDownloader::GetSegmentsInOneTimeline(const DashSegTimeline* timeline, int segCount, unsigned int segDuration, std::string mediaUrl,
    int64_t& segmentSeq, uint64_t& startTime, std::shared_ptr<DashStreamDescription> streamDesc)
{
    int repeat = 0;
    std::string tempUrl;

    while (repeat <= segCount) {
        repeat++;
        if (segmentSeq == -1) {
            segmentSeq = streamDesc->startNumberSeq_;
        } else {
        ...
        tempUrl = mediaUrl;
        if (DashSubstituteTmpltStr(tempUrl, "$Time", std::to_string(startTime)) == -1) {
            MEDIA_LOG_E("" PUBLIC_LOG_S " substitute $Time " PUBLIC_LOG_S " error in static timeline", tempUrl.c_str(), std::to_string(startTime).c_str());
            return DASH_SEGMENT_INIT_FAILED;
        }

        if (DashSubstituteTmpltStr(tempUrl, "$Number", std::to_string(segmentSeq)) == -1) {
            MEDIA_LOG_E("" PUBLIC_LOG_S " substitute $Number " PUBLIC_LOG_S " error in static timeline", tempUrl.c_str(), std::to_string(segmentSeq).c_str());
            return DASH_SEGMENT_INIT_FAILED;
        }

        if (DASH_SEGMENT_INIT_FAILED == AddOneSegment(segDuration, segmentSeq, tempUrl, streamDesc)) {
 ...
 */
int DashMpdDownloader::GetSegCountFromTimeline(DashList<DashSegTimeline*>::iterator& it, const DashList<DashSegTimeline*>::iterator& end, unsigned int periodDuration, unsigned int timeScale, uint64_t startTime)
{
    int segCount = (*it)->r_;
    ...
    if (segCount < 0 && (*it)->d_) {
        ...
        if (it != end && *it != nullptr) {
            ...
            if (nextStartTime <= startTime) {
                MEDIA_LOG_W("r is negative and next S@t " PUBLIC_LOG_U64 " is not larger than startTime " PUBLIC_LOG_U64, nextStartTime, startTime);
                return segCount;
            }
            ...
            if (scaleDuration <= startTime) {
                MEDIA_LOG_W("r is negative, duration " PUBLIC_LOG_U32 " is not larger than startTime " PUBLIC_LOG_U64 ", timeScale " PUBLIC_LOG_U32, periodDuration, startTime, timeScale);
                return segCount;
            }
 ...
 * @return   failed success undo
 */
DashSegmentInitValue DashMpdDownloader::GetSegmentsWithSegList(const DashSegListInfo* segListInfo, std::string baseUrl, std::shared_ptr<DashStreamDescription> streamDesc)
{
    int64_t segmentSeq = -1;
    int segmentCount = 0;
    ...
    for (std::list<DashSegUrl*>::iterator it = segUrlList.begin(); it != segUrlList.end(); it++) {
        if (*it != nullptr) {
            ...
            if (durationList.size() > 0) {
                ...
                // if size of SegmentTimeline smaller than size of SegmentURL，keep the last segment duration in SegmentTimeline
 ...
 * @return   none
 */
void DashMpdDownloader::GetSegDurationFromTimeline(unsigned int periodDuration, unsigned int timeScale, const DashMultSegBaseInfo* multSegBaseInfo, DashList<unsigned int>& durationList)
{
    ...
    if (multSegBaseInfo->duration_ == 0 && multSegBaseInfo->segTimeline_.size() > 0 && mpdInfo_->type_ == DashType::DASH_TYPE_STATIC) {
        uint64_t startTime = 0;
 ...
 * @return   failed success undo
 */
DashSegmentInitValue DashMpdDownloader::GetSegmentsWithBaseUrl(std::list<std::string> baseUrlList, std::shared_ptr<DashStreamDescription> streamDesc)
{
    DashSegment srcSegment;
    srcSegment.streamId_ = streamDesc->streamId_;
    srcSegment.bandwidth_ = streamDesc->bandwidth_;
...
DashRepresentationInfo* DashMpdDownloader::GetRepresemtationFromAdptSet(DashAdptSetInfo* adptSetInfo, unsigned int repIndex)
{
    unsigned int index = 0;
    for (std::list<DashRepresentationInfo*>::iterator it = adptSetInfo->representationList_.begin(); it != adptSetInfo->representationList_.end(); it++, index++) {
        if (repIndex == index) {
            return *it;
        }
    }
...
bool DashMpdDownloader::GetInitSegFromPeriod(std::string periodBaseUrl, std::string repId, std::shared_ptr<DashStreamDescription> streamDesc)
{
    int segTmpltFlag = 0;
    DashUrlType *initSegment = periodManager_->GetInitSegment(segTmpltFlag); // should SetPeriodInfo before GetInitSegment
    if (initSegment != nullptr) {
        ...
        MEDIA_LOG_I("GetInitSegFromPeriod:streamId:" PUBLIC_LOG_D32 ", init seg url " PUBLIC_LOG_S, streamDesc->streamId_, streamDesc->initSegment_->url_.c_str());
        return true;
    }
...
bool DashMpdDownloader::GetInitSegFromAdptSet(std::string adptSetBaseUrl, std::string repId, std::shared_ptr<DashStreamDescription> streamDesc)
{
    int segTmpltFlag = 0;
    DashUrlType *initSegment = adptSetManager_->GetInitSegment(segTmpltFlag); // should SetAdptSetInfo before GetInitSegment
    if (initSegment != nullptr) {
        streamDesc->initSegment_ = std::make_shared<DashInitSegment>();
        UpdateInitSegUrl(streamDesc, initSegment, segTmpltFlag, repId);
        MakeAbsoluteWithBaseUrl(streamDesc->initSegment_, adptSetBaseUrl);
        MEDIA_LOG_I("GetInitSegFromAdptSet:streamId:" PUBLIC_LOG_D32 ", init seg url " PUBLIC_LOG_S, streamDesc->streamId_, streamDesc->initSegment_->url_.c_str());
        return true;
    }
...
bool DashMpdDownloader::GetInitSegFromRepresentation(std::string repBaseUrl, std::string repId, std::shared_ptr<DashStreamDescription> streamDesc)
{
    int segTmpltFlag = 0;
    DashUrlType *initSegment = representationManager_->GetInitSegment(segTmpltFlag); // should SetRepresentationInfo before GetInitSegment
    if (initSegment != nullptr) {
        streamDesc->initSegment_ = std::make_shared<DashInitSegment>();
        UpdateInitSegUrl(streamDesc, initSegment, segTmpltFlag, repId);
        MakeAbsoluteWithBaseUrl(streamDesc->initSegment_, repBaseUrl);
        MEDIA_LOG_I("GetInitSegFromRepresentation:streamId:" PUBLIC_LOG_D32 ", init seg url " PUBLIC_LOG_S, streamDesc->streamId_, streamDesc->initSegment_->url_.c_str());
        return true;
    }
...
void DashMpdDownloader::UpdateInitSegUrl(std::shared_ptr<DashStreamDescription> streamDesc, const DashUrlType* urlType, int segTmpltFlag, std::string representationID)
{
    if (streamDesc != nullptr && streamDesc->initSegment_ != nullptr) {
        ...
        if (segTmpltFlag) {
            ...
            if (DashSubstituteTmpltStr(streamDesc->initSegment_->url_, "$RepresentationID", representationID) == -1) {
                MEDIA_LOG_E("init seg url " PUBLIC_LOG_S " subtitute $RepresentationID error " PUBLIC_LOG_S, streamDesc->initSegment_->url_.c_str(), representationID.c_str());
            }

            if (DashSubstituteTmpltStr(streamDesc->initSegment_->url_, "$Bandwidth", std::to_string(streamDesc->bandwidth_)) == -1) {
                MEDIA_LOG_E("init seg url " PUBLIC_LOG_S " subtitute $Bandwidth error " PUBLIC_LOG_U32, streamDesc->initSegment_->url_.c_str(), streamDesc->bandwidth_);
            }
        }
    }
}
