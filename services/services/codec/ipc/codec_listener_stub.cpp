...
namespace OHOS {
namespace Media {
int32_t CodecBufferCache::ReadFromParcel(uint32_t index, MessageParcel &parcel,
...
std::shared_ptr<AVSharedMemory> CodecBufferCache::ReadFromCaches(uint32_t index)
{
    // std::lock_guard<std::mutex> lock(mutex_);
    ...
    MessageOption &option)
{
    ...
    switch (code) {
        ...
        case CodecListenerMsg::ON_INPUT_BUFFER_AVAILABLE: {
            ...
            std::shared_ptr<AVSharedMemory> memory = nullptr;
            int32_t ret = inputBufferCache_->ReadFromParcel(index, reply, memory);
        ...
        case CodecListenerMsg::ON_OUTPUT_BUFFER_AVAILABLE: {
            ...
            std::shared_ptr<AVSharedMemory> memory = nullptr;
            int32_t ret = outputBufferCache_->ReadFromParcel(index, reply, memory);
