...
void HEncoder::ParseRoiStringValid(const std::string &roiValue, shared_ptr<CodecHDI::OmxCodecBuffer> &omxBuffer)
{
    ...
    while (std::regex_search(temp, match, pattern) && vaildCount < roiNum) {
        ...
        top = std::clamp(static_cast<int>(std::strtol(match[TOP_INDEX].str().c_str(), nullptr, 10)), //Decimal
             0, static_cast<int>(height_));
        left = std::clamp(static_cast<int>(std::strtol(match[LEFT_INDEX].str().c_str(), nullptr, 10)), //Decimal
             0, static_cast<int>(width_));
        bottom = std::clamp(static_cast<int>(std::strtol(match[BOTTOM_INDEX].str().c_str(), nullptr, 10)), //Decimal
             0, static_cast<int>(height_));
        right = std::clamp(static_cast<int>(std::strtol(match[RIGHT_INDEX].str().c_str(), nullptr, 10)), //Decimal
             0, static_cast<int>(width_));
        if (match[QPOFFSET_INDEX].matched) {
            qpOffset = static_cast<int>(std::strtol(match[QPOFFSET_INDEX].str().c_str(), nullptr, 10)); //Decimal
        } else {
