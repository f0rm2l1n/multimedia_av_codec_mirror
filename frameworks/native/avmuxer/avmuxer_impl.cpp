...
std::shared_ptr<AVMuxer> AVMuxerFactory::CreateAVMuxer(int32_t fd, Plugin::OutputFormat format)
{
...
int32_t AVMuxerImpl::Init(int32_t fd, Plugin::OutputFormat format)
{
...
int32_t AVMuxerImpl::SetRotation(int32_t rotation)
{
    ...
    param->Set<Tag::VIDEO_ROTATION>(static_cast<Plugin::VideoRotation>(rotation));
    return StatusConvert(muxerEngine_->SetParameter(param));
}

int32_t AVMuxerImpl::AddTrack(int32_t &trackIndex, std::shared_ptr<Meta> trackDesc)
{
    CHECK_AND_RETURN_RET_LOG(muxerEngine_ != nullptr, AVCS_ERR_INVALID_OPERATION, "AVMuxer Engine does not exist");
...
int32_t AVMuxerImpl::WriteSample(uint32_t trackIndex, std::shared_ptr<AVBuffer> sample)
{
    CHECK_AND_RETURN_RET_LOG(muxerEngine_ != nullptr, AVCS_ERR_INVALID_OPERATION, "AVMuxer Engine does not exist");
...
int32_t AVMuxerImpl::StatusConvert(Media::Status status)
{
    const static std::unordered_map<Media::Status, int32_t> table = {
        ...
        {Status::ERROR_UNSUPPORTED_FORMAT, AVCodecServiceErrCode::AVCS_ERR_UNSUPPORT_CONTAINER_TYPE},
