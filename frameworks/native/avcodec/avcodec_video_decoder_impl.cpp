...
{
    AVCODEC_LOGD_WITH_TAG("AVCodecVideoDecoderImpl:0x%{public}06" PRIXPTR " Instances create", FAKE_POINTER(this));
}
...
AVCodecVideoDecoderImpl::~AVCodecVideoDecoderImpl()
{
    ...
    AVCODEC_LOGD_WITH_TAG("AVCodecVideoDecoderImpl:0x%{public}06" PRIXPTR " Instances destroy", FAKE_POINTER(this));
}
...
int32_t AVCodecVideoDecoderImpl::QueryInputBuffer(uint32_t &index, int64_t timeoutUs)
{
    CHECK_AND_RETURN_RET_LOG_WITH_TAG(codecClient_ != nullptr, AVCS_ERR_INVALID_OPERATION, "Codec service is nullptr");
...
int32_t AVCodecVideoDecoderImpl::QueryOutputBuffer(uint32_t &index, int64_t timeoutUs)
{
    CHECK_AND_RETURN_RET_LOG_WITH_TAG(codecClient_ != nullptr, AVCS_ERR_INVALID_OPERATION, "Codec service is nullptr");
...
std::shared_ptr<AVBuffer> AVCodecVideoDecoderImpl::GetInputBuffer(uint32_t index)
{
...
std::shared_ptr<AVBuffer> AVCodecVideoDecoderImpl::GetOutputBuffer(uint32_t index)
{
    CHECK_AND_RETURN_RET_LOG_WITH_TAG(codecClient_ != nullptr, nullptr, "Codec service is nullptr");
...
int32_t AVCodecVideoDecoderImpl::SetDecryptConfig(const sptr<DrmStandard::IMediaKeySessionService> &keySessionProxy,
    const bool svpFlag)
{
    AVCODEC_LOGI_WITH_TAG("AVCodecVideoDecoderImpl SetDecryptConfig proxy");
    CHECK_AND_RETURN_RET_LOG_WITH_TAG(codecClient_ != nullptr,
        AVCS_ERR_INVALID_OPERATION, "Codec service is nullptr");
    CHECK_AND_RETURN_RET_LOG_WITH_TAG(keySessionProxy != nullptr,
...
int32_t AVCodecVideoDecoderImpl::NotifyMemoryExchange(const bool exchangeFlag)
{
    ...
    AVCODEC_SYNC_TRACE;
    return codecClient_->NotifyMemoryExchange(exchangeFlag);71
}
} // namespace MediaAVCodec
} // namespace OHOS
