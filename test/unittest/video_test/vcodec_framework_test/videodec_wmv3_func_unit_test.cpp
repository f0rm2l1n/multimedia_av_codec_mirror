...
namespace VFTSUIT {
constexpr uint32_t DEFAULT_WIDTH_WMV3 = 352;
constexpr uint32_t DEFAULT_HEIGHT_WMV3 = 288;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_WMV3).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_WMV3).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_WMV3.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_WMV3);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_WMV3);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_PIXEL_FORMAT, static_cast<int32_t>(VideoPixelFormat::NV12));
    format_->PutBuffer(MediaDescriptionKey::MD_KEY_CODEC_CONFIG, WMV3_EXTRADATA, WMV3_EXTRDATA_SIZE);
}

INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_WMV3));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_WMV3.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_WMV3).data());
...
{
    CreateByNameWithParam(SW_WMV3);
    SetFormatWithParam(SW_WMV3);
    PrepareSource(SW_WMV3);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_WMV3 - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_WMV3 - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_WMV3;
