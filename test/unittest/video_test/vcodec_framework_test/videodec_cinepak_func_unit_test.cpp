...
namespace VFTSUIT {
constexpr uint32_t DEFAULT_WIDTH_CINEPAK = 320;
constexpr uint32_t DEFAULT_HEIGHT_CINEPAK = 240;
std::atomic<int32_t> g_vdecCount = 0;
std::string g_vdecName = "";

void TEST_SUIT::SetUpTestCase(void)
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_CINEPAK).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_CINEPAK).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_CINEPAK.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_CINEPAK);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_CINEPAK);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_PIXEL_FORMAT, static_cast<int32_t>(VideoPixelFormat::NV12));
}

INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_CINEPAK));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_CINEPAK.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_CINEPAK).data());
...
{
    CreateByNameWithParam(SW_CINEPAK);
    SetFormatWithParam(SW_CINEPAK);
    PrepareSource(SW_CINEPAK);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_CINEPAK - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_CINEPAK - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_CINEPAK;
