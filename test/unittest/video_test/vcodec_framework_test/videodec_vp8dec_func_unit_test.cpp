...
const int DEFAULT_WIDTH_VP8 = 720;
const int DEFAULT_HEIGHT_VP8 = 480;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_VP8).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_VP8).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_VP8.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_VP8);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_VP8);
...
INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_VP8));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_VP8.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_VP8).data());
...
{
    CreateByNameWithParam(SW_VP8);
    SetFormatWithParam(SW_VP8);
    PrepareSource(SW_VP8);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_VP8 - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_VP8 - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_VP8;
