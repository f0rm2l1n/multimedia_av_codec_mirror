...
class VideoEncSyncSample : public NoCopyable {
public:
    explicit VideoEncSyncSample(std::shared_ptr<VEncSignal> signal);
    virtual ~VideoEncSyncSample();
...
private:
    ...
    void InputLtrParam(int32_t frameInputCount, std::shared_ptr<AVBufferMock> buffer);
