...
const int DEFAULT_WIDTH_DVVIDEO = 720;
const int DEFAULT_HEIGHT_DVVIDEO = 576;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_DVVIDEO).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_DVVIDEO).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_DVVIDEO.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_DVVIDEO);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_DVVIDEO);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_PIXEL_FORMAT, static_cast<int32_t>(VideoPixelFormat::RGBA));
}

INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_DVVIDEO));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_DVVIDEO.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_DVVIDEO).data());
...
HWTEST_P(TEST_SUIT, VideoDecoder_Configure_006, TestSize.Level1)
{
    ...
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_DVVIDEO);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_DVVIDEO);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_FRAME_RATE, 25);
...
{
    CreateByNameWithParam(SW_DVVIDEO);
    SetFormatWithParam(SW_DVVIDEO);
    PrepareSource(SW_DVVIDEO);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_DVVIDEO - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_DVVIDEO - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_DVVIDEO;
