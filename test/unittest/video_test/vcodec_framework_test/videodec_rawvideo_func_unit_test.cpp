...
const int DEFAULT_WIDTH_RAWVIDEO = 720;
const int DEFAULT_HEIGHT_RAWVIDEO = 480;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_RAWVIDEO).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_RAWVIDEO).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_RAWVIDEO.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_RAWVIDEO);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_RAWVIDEO);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_PIXEL_FORMAT, static_cast<int32_t>(VideoPixelFormat::RGBA));
    format_->PutIntValue("rawvideo_input_pix_fmt", 0);
}

INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_RAWVIDEO));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_RAWVIDEO.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_RAWVIDEO).data());
...
{
    CreateByNameWithParam(SW_RAWVIDEO);
    SetFormatWithParam(SW_RAWVIDEO);
    PrepareSource(SW_RAWVIDEO);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_RAWVIDEO - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_RAWVIDEO - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_RAWVIDEO;
