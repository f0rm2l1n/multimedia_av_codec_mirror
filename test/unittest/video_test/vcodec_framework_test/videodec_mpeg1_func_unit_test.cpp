...
const int DEFAULT_WIDTH_MPEG1 = 720;
const int DEFAULT_HEIGHT_MPEG1 = 480;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_MPEG1).data(), false,
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_MPEG1.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_MPEG1);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_MPEG1);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_PIXEL_FORMAT, static_cast<int32_t>(VideoPixelFormat::NV12));
}

INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_MPEG1));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_MPEG1.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_MPEG1).data());
...
{
    CreateByNameWithParam(SW_MPEG1);
    SetFormatWithParam(SW_MPEG1);
    PrepareSource(SW_MPEG1);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
 ...
 * @tc.type: FUNC
 */
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_002, TestSize.Level1)
{
    CreateByNameWithParam(GetParam());
    SetFormatWithParam(GetParam());
    PrepareSource(GetParam());
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_MPEG1 - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_MPEG1 - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_MPEG1;
