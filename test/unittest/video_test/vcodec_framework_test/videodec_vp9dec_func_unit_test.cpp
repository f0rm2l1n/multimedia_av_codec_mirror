...
const int DEFAULT_WIDTH_VP9 = 720;
const int DEFAULT_HEIGHT_VP9 = 480;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_VP9).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_VP9).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_VP9.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
{
    (void)param;
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_VP9);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_VP9);
...
INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_VP9));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_VP9.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_VP9).data());
...
{
    CreateByNameWithParam(SW_VP9);
    SetFormatWithParam(SW_VP9);
    PrepareSource(SW_VP9);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_VP9 - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_VP9 - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_VP9;
