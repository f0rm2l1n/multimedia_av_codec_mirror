...
extern "C" {
#endif
#define AVBuffer FFmpegAVBuffer
...
const int DEFAULT_WIDTH_WVC1 = 720;
const int DEFAULT_HEIGHT_WVC1 = 480;
...
{
    auto capability = CodecListMockFactory::GetCapabilityByCategory((CodecMimeType::VIDEO_WVC1).data(), false,
                                                                    AVCodecCategory::AVCODEC_SOFTWARE);
    ASSERT_NE(nullptr, capability) << (CodecMimeType::VIDEO_WVC1).data() << " can not found!" << std::endl;
...
void TEST_SUIT::CreateByNameWithParam(int32_t param)
{
    ...
    capability_ = CodecListMockFactory::GetCapabilityByCategory(CodecMimeType::VIDEO_WVC1.data(), false,
                                                                AVCodecCategory::AVCODEC_SOFTWARE);
...
void TEST_SUIT::SetFormatWithParam(int32_t param)
{
    ...
    if (ret != 0) {cout << "avformat_open_input error" << endl;}
    ret = avformat_find_stream_info(formatContext, nullptr);
    if (ret != 0) {cout << "avformat_find_stream_info error" << endl;}
    ...
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, DEFAULT_WIDTH_WVC1);
    format_->PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, DEFAULT_HEIGHT_WVC1);
...
INSTANTIATE_TEST_SUITE_P(, TEST_SUIT, testing::Values(SW_WVC1));
...
HWTEST_F(TEST_SUIT, VideoDecoder_Multithread_Create_002, TestSize.Level1)
{
    auto func = []() {
        ...
        std::shared_ptr<VideoDecSample> videoDec = std::make_shared<VideoDecSample>(vdecSignal);
        ASSERT_NE(nullptr, videoDec);
        if (videoDec->CreateVideoDecMockByMime(CodecMimeType::VIDEO_WVC1.data())) {
            g_vdecCount++;
            cout << "create successed, num:" << g_vdecCount.load() << endl;
        } else {
            cout << "create failed, num:" << g_vdecCount.load() << endl;
...
{
    codec_ = OH_VideoDecoder_CreateByMime((CodecMimeType::VIDEO_WVC1).data());
...
{
    CreateByNameWithParam(SW_WVC1);
    SetFormatWithParam(SW_WVC1);
    PrepareSource(SW_WVC1);
    ASSERT_EQ(AV_ERR_OK, videoDec_->Configure(format_));
...
HWTEST_P(TEST_SUIT, VideoDecoder_GetOutputDescription_004, TestSize.Level1)
{
    ...
    EXPECT_GE(pictureWidth, DEFAULT_WIDTH_WVC1 - 1);
    EXPECT_GE(pictureHeight, DEFAULT_HEIGHT_WVC1 - 1);
...
{
    VCodecTestCode param = VCodecTestCode::SW_WVC1;
