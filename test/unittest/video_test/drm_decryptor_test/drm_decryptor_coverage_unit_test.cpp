...
const uint8_t H264_IV[] = { 0x55, 0x44, 0x52, 0x4d, 0x5f, 0x54, 0x45, 0x53,
                            0x54, 0x5f, 0x43, 0x49, 0x44, 0x5f, 0x30, 0x37};
uint32_t H264_ENCRYPTED_BUFFER_SIZE = 420;
...
uint32_t H264_NO_CEI_BUFFER_SIZE = 83;
const uint8_t H264_NO_CEI_BUFFER[] = {
0x00, 0x00, 0x01, 0x61, 0x88, 0x84, 0x00, 0xff, 0x9f, 0x97, 0x12, 0x77, 0x06, 0xff, 0x81, 0x48,
0xf0, 0x84, 0xc3, 0xfb, 0x35, 0x95, 0x4d, 0x40, 0x94, 0x6e, 0x1a, 0xd3, 0x6f, 0xb0, 0xc3, 0xae,
0x35, 0x01, 0x6f, 0x46, 0x26, 0xae, 0x2b, 0x46, 0x6b, 0x81, 0x9f, 0xc1, 0xf9, 0x46, 0x86, 0x5b,
0x62, 0xaa, 0x0d, 0x2a, 0x39, 0xc0, 0xae, 0x2d, 0x52, 0xef, 0x97, 0x6e, 0xd5, 0x78, 0xec, 0xa4,
0x25, 0xee, 0xa7, 0xa6, 0xae, 0xed, 0xf3, 0xe7, 0xdf, 0xef, 0xd8, 0x84, 0xb4, 0x45, 0xd4, 0xfb,
0x67, 0x8c, 0x6c};
...
void CreateH264MediaCencInfo(MetaDrmCencInfo &cencInfo, uint32_t method)
{
    ...
    if (method == 0xfe) {
        cencInfo.subSamples[0].clearHeaderLen = H264_NO_CEI_BUFFER_SIZE;
    } else if (method == 0xff) {
...
void SetH264MediaData(std::shared_ptr<AVBuffer> drmInBuf, MetaDrmCencInfo &cencInfo, uint32_t method)
{
    ...
    switch (method) {
        ...
        case 0x0: // 0x0:NONE
            ...
            break;
        case 0xfe: // 0xfe:NO CEI
            cencInfo.algo = MetaDrmCencAlgorithm::META_DRM_ALG_CENC_UNENCRYPTED;
            drmRes = memcpy_s(drmInBuf->memory_->GetAddr(), H264_NO_CEI_BUFFER_SIZE,
                H264_NO_CEI_BUFFER, H264_NO_CEI_BUFFER_SIZE);
            break;
    ...
    std::shared_ptr<MediaAVCodec::CodecDrmDecryptorMock> decryptorMock, uint32_t method, int32_t flag)
{
    ...
    if (method == 0xfe) {
        decryptorMock->DrmVideoCencDecrypt(drmInBuf, drmOutBuf, H264_NO_CEI_BUFFER_SIZE);
    } else if (method == 0xff) {
...
HWTEST_F(CodecDrmDecryptorUnitTest, Codec_Drm_Decryptor_DrmVideoCencDecrypt_001, TestSize.Level1)
{
    ...
    H264MediaCencDecrypt(drmInBuf, drmOutBuf, decryptorMock_, 0xfe, 1); // 0xfe:NO CEI
...
HWTEST_F(CodecDrmDecryptorUnitTest, Codec_Drm_Decryptor_DrmVideoCencDecrypt_003, TestSize.Level1)
{
    ...
    std::shared_ptr<AVBuffer> testBuf = nullptr;
    uint32_t testBufSize = 0;
    int32_t ret = decryptorMock_->DrmVideoCencDecrypt(testBuf, drmOutBuf, AVS_SM4C_ENCRYPTED_BUFFER_SIZE);
    EXPECT_NE(ret, 0);
    ret = decryptorMock_->DrmVideoCencDecrypt(drmInBuf, testBuf, AVS_SM4C_ENCRYPTED_BUFFER_SIZE);
    EXPECT_NE(ret, 0);
    ret = decryptorMock_->DrmVideoCencDecrypt(drmInBuf, drmOutBuf, testBufSize);
    EXPECT_NE(ret, 0);
}
...
/**
 * @tc.name: Codec_Drm_Decryptor_DrmAudioCencDecrypt_002
 * @tc.desc: DrmAudioCencDecrypt, test for parameter
 */
HWTEST_F(CodecDrmDecryptorUnitTest, Codec_Drm_Decryptor_DrmAudioCencDecrypt_002, TestSize.Level1)
{
    UNITTEST_INFO_LOG("Codec_Drm_Decryptor_DrmAudioCencDecrypt_002");
    decryptorMock_ = std::make_shared<CodecDrmDecryptorMock>();
    EXPECT_NE(decryptorMock_, nullptr);
    ...
    std::shared_ptr<AVBuffer> testBuf = nullptr;
    uint32_t testBufSize = 0;
    int32_t ret = decryptorMock_->DrmAudioCencDecrypt(testBuf, drmOutBuf, DRM_AUDIO_ENCRYPTED_BUFFER_SIZE);
    EXPECT_NE(ret, 0);
    ret = decryptorMock_->DrmAudioCencDecrypt(drmInBuf, testBuf, DRM_AUDIO_ENCRYPTED_BUFFER_SIZE);
    EXPECT_NE(ret, 0);
    ret = decryptorMock_->DrmAudioCencDecrypt(drmInBuf, drmOutBuf, testBufSize);
    EXPECT_NE(ret, 0);
}
} // namespace Media
} // namespace OHOS