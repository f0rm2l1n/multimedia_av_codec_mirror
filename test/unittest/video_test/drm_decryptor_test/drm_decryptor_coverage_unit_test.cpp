...
// for H264
const uint8_t H264_SM4C_KEY_ID[] = { 0x55, 0x44, 0x52, 0x4d, 0x5f, 0x54, 0x45, 0x53,
                                     0x54, 0x5f, 0x43, 0x49, 0x44, 0x5f, 0x30, 0x37};
const uint8_t H264_SM4C_IV[] = { 0x55, 0x44, 0x52, 0x4d, 0x5f, 0x54, 0x45, 0x53,
                                 0x54, 0x5f, 0x43, 0x49, 0x44, 0x5f, 0x30, 0x37};
uint32_t H264_SM4C_ENCRYPTED_BUFFER_SIZE = 418;
...
void CreateH264MediaCencInfo(MetaDrmCencInfo &cencInfo)
{
    ...
    memcpy_s(cencInfo.keyId, sizeof(cencInfo.keyId), H264_SM4C_KEY_ID, sizeof(cencInfo.keyId));
    cencInfo.keyIdLen = KEY_ID_LEN;
    memcpy_s(cencInfo.iv, sizeof(cencInfo.iv), H264_SM4C_IV, sizeof(cencInfo.iv));
    ...
    cencInfo.subSamples[0].clearHeaderLen = H264_SM4C_ENCRYPTED_BUFFER_SIZE;
...
HWTEST_F(CodecDrmDecryptorUnitTest, Codec_Drm_Decryptor_DrmVideoCencDecrypt_001, TestSize.Level1)
{
    ...
    std::shared_ptr<AVBuffer> drmInBuf = AVBuffer::CreateAVBuffer(avAllocator,
        static_cast<int32_t>(H264_SM4C_ENCRYPTED_BUFFER_SIZE));
    EXPECT_NE(drmInBuf, nullptr);
    EXPECT_NE(drmInBuf->memory_, nullptr);
    EXPECT_EQ(drmInBuf->memory_->GetCapacity(), H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    int32_t drmRes = memcpy_s(drmInBuf->memory_->GetAddr(), H264_SM4C_ENCRYPTED_BUFFER_SIZE,
        H264_SM4C_ENCRYPTED_BUFFER, H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    EXPECT_EQ(drmRes, 0);
    ...
    EXPECT_EQ(drmOutBuf->memory_->GetCapacity(), H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    ...
    decryptorMock_->DrmVideoCencDecrypt(drmInBuf, drmOutBuf, H264_SM4C_ENCRYPTED_BUFFER_SIZE);
}
...
HWTEST_F(CodecDrmDecryptorUnitTest, Codec_Drm_Decryptor_DrmAudioCencDecrypt_001, TestSize.Level1)
{
    ...
    std::vector<uint8_t> drmCencVec(reinterpret_cast<uint8_t *>(&cencInfo),
        (reinterpret_cast<uint8_t *>(&cencInfo)) + sizeof(MetaDrmCencInfo));
    drmInBuf->meta_->SetData(Media::Tag::DRM_CENC_INFO, std::move(drmCencVec));
    decryptorMock_->DrmAudioCencDecrypt(drmInBuf, drmOutBuf, H264_SM4C_ENCRYPTED_BUFFER_SIZE);
}
} // namespace Media
} // namespace OHOS