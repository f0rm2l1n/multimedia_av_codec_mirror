...
/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_001
* @tc.desc  : Test audio plugin active and SeekTo returns OK
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_001, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    auto audioPlugin = std::make_shared<Plugins::TestDemuxerPlugin>(Status::OK, Status::OK);
    demuxerPluginManager_->streamInfoMap_[ID_TEST].plugin = audioPlugin;

    int64_t realSeekTime = 0;
    auto ret = demuxerPluginManager_->SeekToKeyFrame(TEST_SEEK_TIME, Plugins::SeekMode::SEEK_NEXT_SYNC, realSeekTime);

    EXPECT_EQ(ret, Status::OK);
    EXPECT_EQ(audioPlugin->GetSeekToCallCount(), 1);
    EXPECT_EQ(audioPlugin->GetLastSeekMode(), Plugins::SeekMode::SEEK_NEXT_SYNC);
}

/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_002
* @tc.desc  : Test audio plugin SeekTo error causes early return
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_002, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    auto audioPlugin = std::make_shared<Plugins::TestDemuxerPlugin>(Status::ERROR_UNKNOWN, Status::OK);
    auto subtitlePlugin = std::make_shared<Plugins::TestDemuxerPlugin>(Status::OK, Status::OK);
    auto videoPlugin = std::make_shared<Plugins::TestDemuxerPlugin>(Status::OK, Status::OK);

    demuxerPluginManager_->streamInfoMap_[ID_TEST].plugin = audioPlugin;
    demuxerPluginManager_->streamInfoMap_[SUBTITLE_STREAM_ID_TEST].plugin = subtitlePlugin;
    demuxerPluginManager_->streamInfoMap_[VIDEO_STREAM_ID_TEST].plugin = videoPlugin;
    ...
    EXPECT_EQ(ret, Status::ERROR_UNKNOWN);
    EXPECT_EQ(audioPlugin->GetSeekToCallCount(), 1);
    EXPECT_EQ(subtitlePlugin->GetSeekToCallCount(), 0);
    EXPECT_EQ(videoPlugin->GetSeekToKeyFrameCallCount(), 0);
}

/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_003
* @tc.desc  : Test subtitle plugin SeekTo OK, no retry
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_003, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    EXPECT_EQ(ret, Status::OK);
    EXPECT_EQ(subtitlePlugin->GetSeekToCallCount(), 1);
    EXPECT_EQ(subtitlePlugin->GetLastSeekMode(), Plugins::SeekMode::SEEK_NEXT_SYNC);
}

/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_004
* @tc.desc  : Test subtitle SeekTo error and mode == SEEK_NEXT_SYNC, no retry
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_004, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    auto subtitlePlugin = std::make_shared<Plugins::TestDemuxerPlugin>(Status::ERROR_UNKNOWN, Status::OK);
    demuxerPluginManager_->streamInfoMap_[SUBTITLE_STREAM_ID_TEST].plugin = subtitlePlugin;
...
/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_005
* @tc.desc  : Test subtitle SeekTo error and mode != SEEK_NEXT_SYNC, retry with SEEK_NEXT_SYNC
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_005, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    int64_t realSeekTime = 0;
    auto ret = demuxerPluginManager_->SeekToKeyFrame(TEST_SEEK_TIME,
        Plugins::SeekMode::SEEK_PREVIOUS_SYNC, realSeekTime);

    EXPECT_EQ(ret, Status::OK);
    EXPECT_EQ(subtitlePlugin->GetSeekToCallCount(), 2);
    EXPECT_EQ(subtitlePlugin->GetLastSeekMode(), Plugins::SeekMode::SEEK_NEXT_SYNC);
}

/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_006
* @tc.desc  : Test video plugin SeekToKeyFrame OK
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_006, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    EXPECT_EQ(ret, Status::OK);
    EXPECT_EQ(videoPlugin->GetSeekToKeyFrameCallCount(), 1);
    EXPECT_EQ(videoPlugin->GetLastSeekMode(), Plugins::SeekMode::SEEK_NEXT_SYNC);
}

/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_007
* @tc.desc  : Test video plugin SeekToKeyFrame error only logs warning and returns OK
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_007, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
    ...
    auto videoPlugin = std::make_shared<Plugins::TestDemuxerPlugin>(Status::OK, Status::ERROR_UNKNOWN);
    demuxerPluginManager_->streamInfoMap_[VIDEO_STREAM_ID_TEST].plugin = videoPlugin;
...
/**
* @tc.name  : Test SeekToKeyFrame  API
* @tc.number: SeekToKeyFrame_008
* @tc.desc  : Test all current stream IDs invalid, no plugin called
*/
HWTEST_F(DemuxerPluginManagerUnittest, SeekToKeyFrame_008, TestSize.Level1)
{
    ASSERT_NE(demuxerPluginManager_, nullptr);
