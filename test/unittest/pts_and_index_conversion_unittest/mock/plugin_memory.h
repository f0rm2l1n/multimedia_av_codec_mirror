...
public:
    Memory(size_t capacity, std::shared_ptr<uint8_t> bufData,
        size_t align = 1, MemoryType type = MemoryType::SHARED_MEMORY) {}
    ~Memory() = default;
    MOCK_METHOD(size_t, GetCapacity, (), ());
    size_t GetSize()
    {
        return 0;
    }
    ...
    size_t Read(uint8_t* out, size_t readSize, size_t position = 0)
    {
        return 0;
    }
    MOCK_METHOD(void, Reset, (), ());
    MOCK_METHOD(MemoryType, GetMemoryType, (), ());
    MOCK_METHOD(uint8_t*, GetRealAddr, (), (const));
    MemoryType memoryType;
    size_t capacity;
