 ...
 * @tc.desc: Test isInSeekContinous_ = true
 */
...
/**
 * @tc.name: Test WriteDfxTimeToVector API
 * @tc.number: WriteDfxTimeToVector_001
 * @tc.desc: Test eventReceiver_!= nullptr && lastRenderTimeNs > 0L
 *                isInSeekContinous_ = false
 */
HWTEST_F(DecoderSurfaceFilterUnittest, WriteDfxTimeToVector_001, TestSize.Level0)
{
    ASSERT_NE(decoderSurfaceFilter_, nullptr);

    decoderSurfaceFilter_->isInSeekContinous_ = false;
    decoderSurfaceFilter_->lastRenderTimeNs_ = RENDER_TIME;

    auto buffer = std::make_shared<AVBuffer>();
    buffer->pts_ = RENDER_TIME;
    buffer->meta_ = std::make_shared<Meta>();

    EXPECT_CALL(*(decoderSurfaceFilter_->videoDecoder_),
        RenderOutputBufferAtTime(_, _, _)).WillRepeatedly(Return(0));
    EXPECT_CALL(*(decoderSurfaceFilter_->videoDecoder_), Release()).WillRepeatedly(Return(Status::OK));
    EXPECT_CALL(*(decoderSurfaceFilter_->videoSink_), GetFrameInterval()).WillRepeatedly(Return(1));
    ...
    buffer->meta_->GetData(Tag::STALLING_TIMESTAMP, tsVec);
    EXPECT_GT(tsVec.size(), 0);
}
}  // namespace Pipeline
}  // namespace Media
}  // namespace OHOS
