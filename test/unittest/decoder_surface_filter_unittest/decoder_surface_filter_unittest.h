 ...
 */
#ifndef DECODER_SURFACE_FILTER_UNITTEST_H
#define DECODER_SURFACE_FILTER_UNITTEST_H
...
class DecoderSurfaceFilterUnittest : public testing::Test {
public:
...
protected:
    std::shared_ptr<DecoderSurfaceFilter> decoderSurfaceFilter_{ nullptr };
    std::shared_ptr<FilterVideoPostProcessorCallback> callbackPtr_{ nullptr };
    std::shared_ptr<DecoderSurfaceFilter> GenerateCommonFilter();
};
...
public:
    ...
    MOCK_METHOD(GSError, AcquireBuffer,
        (sptr<SurfaceBuffer>& buffer, int32_t &fence, int64_t &timestamp, Rect &damage), (override));
...
class MockInputBufferQueueProducer : public AVBufferQueueProducer {
    MOCK_METHOD(uint32_t, GetQueueSize, (), (override));
...
#endif  // DECODER_SURFACE_FILTER_UNITTEST_H
