...
void AudioCodeGsmDecoderUnitTest::StopLoops()
{
    if (signal_ != nullptr) {
        {
...
int32_t AudioCodeGsmDecoderUnitTest::InitFile()
{
    ...
    fd_ = open(GSM_FILE_TODEMUX.data(), O_RDONLY);
    if (fd_ < 0) {
...
int32_t AudioCodeGsmDecoderUnitTest::Reset()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeGsmDecoderUnitTest::CleanUp()
{
    if (pcmOutputFile_.is_open()) {
        pcmOutputFile_.close();
    }
...
HWTEST_F(AudioCodeGsmDecoderUnitTest, audioDecoder_Gsm_loop_test_01, TestSize.Level1)
{
...
HWTEST_F(AudioCodeGsmDecoderUnitTest, audioDecoder_Gsm_loop_test_02, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    for (int i = 0; i < g_loopNumber2000; i++) {
...
HWTEST_F(AudioCodeGsmDecoderUnitTest, audioDecoder_Gsm_loop_test_03, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    EXPECT_EQ(OH_AVErrCode::AV_ERR_OK, Configure());
