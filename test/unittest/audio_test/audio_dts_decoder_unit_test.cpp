...
{
    OH_AVCapability *cap = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_AUDIO_DTS, false, SOFTWARE);
...
{
    audioDec_ = OH_AudioCodec_CreateByName("audio_decoder.dts");
    EXPECT_EQ(nullptr, audioDec_);
}

HWTEST_F(AudioCodeDtsDecoderUnitTest, audioDecoder_Dts_CreateByMime_01, TestSize.Level1)
{
    audioDec_ = OH_AudioCodec_CreateByMime(std::string(AVCodecMimeType::MEDIA_MIMETYPE_AUDIO_DTS).data(), false);
    EXPECT_NE(nullptr, audioDec_);
    Release();
}
...
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile(DTS_FILE_TODEMUX));
