...
{
    inputFile_ = std::make_unique<std::ifstream>(EAC3_FILE_TODEMUX.data(), std::ios::binary);
    if (!inputFile_->is_open()) {
        cout << "Fatal: open input file failed:" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputFile_ = std::make_unique<std::ofstream>(OUTPUT_EAC3_PCM_FILE_PATH.data(), std::ios::out | std::ios::binary);
    if (!outputFile_->is_open()) {
    ...
    int fd = open(EAC3_FILE_TODEMUX.data(), O_RDONLY);
    int64_t size = GetFileSize(EAC3_FILE_TODEMUX.data());
    cout << EAC3_FILE_TODEMUX.data() << "----------------------" << fd << "---------" << size << endl;
...
HWTEST_F(AudioSyncModeCapiUnitTest, eac3_decode_01, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());

    codec_ = OH_AudioCodec_CreateByMime(OH_AVCODEC_MIMETYPE_AUDIO_EAC3, false);
...
HWTEST_F(AudioSyncModeCapiUnitTest, eac3_InvalidSizeTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, eac3_InvalidOffsetTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, eac3_InvalidAttrTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
