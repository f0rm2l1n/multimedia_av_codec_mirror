...
class AudioCodeGsm_Ms_DecoderUnitTest : public testing::Test {
public:
...
void AudioCodeGsm_Ms_DecoderUnitTest::SetUpTestCase(void)
{
    cout << "[SetUpTestCase]: " << endl;
}

void AudioCodeGsm_Ms_DecoderUnitTest::TearDownTestCase(void)
{
    cout << "[TearDownTestCase]: " << endl;
}

void AudioCodeGsm_Ms_DecoderUnitTest::SetUp(void)
{
...
void AudioCodeGsm_Ms_DecoderUnitTest::TearDown(void)
{
...
void AudioCodeGsm_Ms_DecoderUnitTest::Release()
{
    Stop();
...
void AudioCodeGsm_Ms_DecoderUnitTest::HandleEOS(const uint32_t &index)
{
...
void AudioCodeGsm_Ms_DecoderUnitTest::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
...
bool AudioCodeGsm_Ms_DecoderUnitTest::ReadBuffer(OH_AVBuffer *buffer, uint32_t index)
{
...
int64_t AudioCodeGsm_Ms_DecoderUnitTest::GetFileSize(const char *fileName)
{
    int64_t fileSize = 0;
    if (fileName != nullptr) {
...
void AudioCodeGsm_Ms_DecoderUnitTest::InputFunc()
{
...
void AudioCodeGsm_Ms_DecoderUnitTest::OutputFunc()
{
    if (!pcmOutputFile_.is_open()) {
        std::cout << "open " << GSM_MS_FILE_TODEMUX << " failed!" << std::endl;
    }
    while (isRunning_.load()) {
...
int32_t AudioCodeGsm_Ms_DecoderUnitTest::Start()
{
    isRunning_.store(true);
    inputLoop_ = make_unique<thread>(&AudioCodeGsm_Ms_DecoderUnitTest::InputFunc, this);
    if (inputLoop_ == nullptr) {
        cout << "Fatal: No memory" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputLoop_ = make_unique<thread>(&AudioCodeGsm_Ms_DecoderUnitTest::OutputFunc, this);
...
int32_t AudioCodeGsm_Ms_DecoderUnitTest::Stop()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeGsm_Ms_DecoderUnitTest::StopLoops()
{
    if (signal_ != nullptr) {
        {
...
int32_t AudioCodeGsm_Ms_DecoderUnitTest::InitFile()
{
    CleanUp();
    isFirstFrame_ = true;
    frameCount_ = 0;
    ...
    fd_ = open(GSM_MS_FILE_TODEMUX.data(), O_RDONLY);
    if (fd_ < 0) {
...
int32_t AudioCodeGsm_Ms_DecoderUnitTest::CreateCodecFunc()
{
...
int32_t AudioCodeGsm_Ms_DecoderUnitTest::Configure()
{
    if (format_ == nullptr) {
    ...
    OH_AVFormat_SetLongValue(
        format_, MediaDescriptionKey::MD_KEY_CHANNEL_LAYOUT.data(), OH_AudioChannelLayout::CH_LAYOUT_MONO);
...
int32_t AudioCodeGsm_Ms_DecoderUnitTest::Reset()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeGsm_Ms_DecoderUnitTest::CleanUp()
{
    if (pcmOutputFile_.is_open()) {
        pcmOutputFile_.close();
    }
...
HWTEST_F(AudioCodeGsm_Ms_DecoderUnitTest, audioDecoder_Gsm_ms_loop_test_01, TestSize.Level1)
{
...
HWTEST_F(AudioCodeGsm_Ms_DecoderUnitTest, audioDecoder_Gsm_ms_loop_test_02, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    for (int i = 0; i < g_loopNumber2000; i++) {
...
HWTEST_F(AudioCodeGsm_Ms_DecoderUnitTest, audioDecoder_Gsm_ms_loop_test_03, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    EXPECT_EQ(OH_AVErrCode::AV_ERR_OK, Configure());
