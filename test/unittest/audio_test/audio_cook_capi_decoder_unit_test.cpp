...
{
    const size_t maxFrameSize = 1024;
    if (inputFile_.eof()) {
    ...
    inputFile_.read(reinterpret_cast<char*>(OH_AVBuffer_GetAddr(buffer)), maxFrameSize);
    auto bytesRead = inputFile_.gcount();
    if (bytesRead <= 0) {
        buffer->buffer_->memory_->SetSize(0);
        buffer->buffer_->flag_ = AVCODEC_BUFFER_FLAGS_EOS;
        HandleInputEOS(index);
    ...
    buffer->buffer_->memory_->SetSize(bytesRead);
    buffer->buffer_->pts_ = frameCount_ * maxFrameSize;
...
{
    inputFile_.open(inputFilename.data(), std::ios::binary);
    if (!inputFile_.is_open()) {
