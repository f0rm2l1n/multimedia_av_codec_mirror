...
int32_t g_loopNumber2000 = []() {
    const char *env = getenv("GSMMS_LOOP_COUNT");
    if (env != nullptr) {
        int v = atoi(env);
        if (v > 0) {
            return v;
...
class AudioCodeDv_Audio_DecoderUnitTest : public testing::Test {
public:
...
void AudioCodeDv_Audio_DecoderUnitTest::SetUpTestCase(void)
{
    cout << "[SetUpTestCase]: " << endl;
}

void AudioCodeDv_Audio_DecoderUnitTest::TearDownTestCase(void)
{
    cout << "[TearDownTestCase]: " << endl;
}

void AudioCodeDv_Audio_DecoderUnitTest::SetUp(void)
{
...
void AudioCodeDv_Audio_DecoderUnitTest::TearDown(void)
{
...
void AudioCodeDv_Audio_DecoderUnitTest::Release()
{
    Stop();
...
void AudioCodeDv_Audio_DecoderUnitTest::HandleEOS(const uint32_t &index)
{
...
void AudioCodeDv_Audio_DecoderUnitTest::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
...
bool AudioCodeDv_Audio_DecoderUnitTest::ReadBuffer(OH_AVBuffer *buffer, uint32_t index)
{
...
int64_t AudioCodeDv_Audio_DecoderUnitTest::GetFileSize(const char *fileName)
{
    int64_t fileSize = 0;
    if (fileName != nullptr) {
...
void AudioCodeDv_Audio_DecoderUnitTest::InputFunc()
{
...
void AudioCodeDv_Audio_DecoderUnitTest::OutputFunc()
{
    if (!pcmOutputFile_.is_open()) {
        std::cout << "open " << DV_AUDIO_FILE_TODEMUX << " failed!" << std::endl;
    }
    while (isRunning_.load()) {
...
int32_t AudioCodeDv_Audio_DecoderUnitTest::Start()
{
    isRunning_.store(true);
    inputLoop_ = make_unique<thread>(&AudioCodeDv_Audio_DecoderUnitTest::InputFunc, this);
    if (inputLoop_ == nullptr) {
        cout << "Fatal: No memory" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputLoop_ = make_unique<thread>(&AudioCodeDv_Audio_DecoderUnitTest::OutputFunc, this);
...
int32_t AudioCodeDv_Audio_DecoderUnitTest::Stop()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeDv_Audio_DecoderUnitTest::StopLoops()
{
    if (signal_ != nullptr) {
        {
...
int32_t AudioCodeDv_Audio_DecoderUnitTest::InitFile()
{
    CleanUp();
    isFirstFrame_ = true;
    frameCount_ = 0;
    ...
    fd_ = open(DV_AUDIO_FILE_TODEMUX.data(), O_RDONLY);
    if (fd_ < 0) {
...
int32_t AudioCodeDv_Audio_DecoderUnitTest::CreateCodecFunc()
{
...
int32_t AudioCodeDv_Audio_DecoderUnitTest::Configure()
{
    if (format_ == nullptr) {
    ...
    OH_AVFormat_SetLongValue(
        format_, MediaDescriptionKey::MD_KEY_CHANNEL_LAYOUT.data(), OH_AudioChannelLayout::CH_LAYOUT_STEREO);
...
int32_t AudioCodeDv_Audio_DecoderUnitTest::Reset()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeDv_Audio_DecoderUnitTest::CleanUp()
{
    if (pcmOutputFile_.is_open()) {
        pcmOutputFile_.close();
    }
...
HWTEST_F(AudioCodeDv_Audio_DecoderUnitTest, audioDecoder_Dv_Audio_loop_test_01, TestSize.Level1)
{
...
HWTEST_F(AudioCodeDv_Audio_DecoderUnitTest, audioDecoder_Dv_Audio_loop_test_02, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    for (int i = 0; i < g_loopNumber2000; i++) {
...
HWTEST_F(AudioCodeDv_Audio_DecoderUnitTest, audioDecoder_Dv_Audio_loop_test_03, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    EXPECT_EQ(OH_AVErrCode::AV_ERR_OK, Configure());
