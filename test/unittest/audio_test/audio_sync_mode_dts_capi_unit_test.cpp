...
void AudioSyncModeCapiUnitTest::DecoderRun()
{
    ...
    while (isRunning_) {
        if (!inputEnd) {
            uint32_t inIndex;
            OH_AVErrCode ret = OH_AudioCodec_QueryInputBuffer(codec_, &inIndex, inTimeout_);
            ...
            OH_AVBuffer *inputBuf = OH_AudioCodec_GetInputBuffer(codec_, inIndex);
            ASSERT_NE(inputBuf, nullptr);

            inputEnd = DecoderFillInputBuffer(inputBuf);
            ASSERT_EQ(OH_AudioCodec_PushInputBuffer(codec_, inIndex), AV_ERR_OK);
        }

        uint32_t outIndex;
        OH_AVErrCode ret = OH_AudioCodec_QueryOutputBuffer(codec_, &outIndex, outTimeout_);
        if (ret == AV_ERR_TRY_AGAIN_LATER) {
            usleep(SLEEP_TIME);
            continue;
        }
        if (ret == AV_ERR_STREAM_CHANGED) {
            OH_AVFormat *fmt = OH_AudioCodec_GetOutputDescription(codec_);
            OH_AVFormat_GetIntValue(fmt, OH_MD_KEY_AUD_CHANNEL_COUNT, &outputChannels);
            OH_AVFormat_GetIntValue(fmt, OH_MD_KEY_AUD_SAMPLE_RATE,   &outputSampleRate);
            OH_AVFormat_Destroy(fmt);
            continue;
        }
        ASSERT_EQ(ret, AV_ERR_OK);

        OH_AVBuffer *outputBuf = OH_AudioCodec_GetOutputBuffer(codec_, outIndex);
        ...
        if (attr.flags & AVCODEC_BUFFER_FLAGS_EOS) {
            ...
            OH_AudioCodec_FreeOutputBuffer(codec_, outIndex);
            break;
        }
        ++outputFrameCnt_;
...
{
    inputFile_ = std::make_unique<std::ifstream>(DTS_FILE_TODEMUX.data(), std::ios::binary);
    if (!inputFile_->is_open()) {
        cout << "Fatal: open input file failed:" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputFile_ = std::make_unique<std::ofstream>(OUTPUT_DTS_PCM_FILE_PATH.data(), std::ios::out | std::ios::binary);
    if (!outputFile_->is_open()) {
    ...
    int fd = open(DTS_FILE_TODEMUX.data(), O_RDONLY);
    int64_t size = GetFileSize(DTS_FILE_TODEMUX.data());
    cout << DTS_FILE_TODEMUX.data() << "----------------------" << fd << "---------" << size << endl;
...
HWTEST_F(AudioSyncModeCapiUnitTest, dts_decode_01, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());

    codec_ = OH_AudioCodec_CreateByMime(OH_AVCODEC_MIMETYPE_AUDIO_DTS, false);
...
HWTEST_F(AudioSyncModeCapiUnitTest, dts_InvalidSizeTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, dts_InvalidOffsetTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, dts_InvalidAttrTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
