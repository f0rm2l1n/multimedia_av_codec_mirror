...
{
    inputFile_ = std::make_unique<std::ifstream>(DV_AUDIO_FILE_TODEMUX.data(), std::ios::binary);
    if (!inputFile_->is_open()) {
        cout << "Fatal: open input file failed:" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputFile_ = std::make_unique<std::ofstream>(OUTPUT_DV_AUDIO_PCM_FILE_PATH.data(),
...
HWTEST_F(AudioSyncModeCapiUnitTest, av_audio_decode_01, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());

    codec_ = OH_AudioCodec_CreateByMime(OH_AVCODEC_MIMETYPE_AUDIO_DVAUDIO, false);
...
HWTEST_F(AudioSyncModeCapiUnitTest, av_audio_InvalidSizeTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, av_audio_InvalidOffsetTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, av_audio_InvalidAttrTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
