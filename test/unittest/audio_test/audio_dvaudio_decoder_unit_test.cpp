...
constexpr string_view DVAUDIO_FILE_TODEMUX = "/data/test/media/dvaudio_48000_stereo.wav";
constexpr string_view OUTPUT_DVAUDIO_PCM_FILE_PATH = "/data/test/media/test_decoder_dvaudio.pcm";
} // namespace
...
void AudioCodeDvaudioDecoderUnitTest::TearDownTestCase(void)
{
    cout << "[TearDownTestCase]: " << endl;
}

void AudioCodeDvaudioDecoderUnitTest::SetUp(void)
{
...
void AudioCodeDvaudioDecoderUnitTest::TearDown(void)
{
...
void AudioCodeDvaudioDecoderUnitTest::HandleEOS(const uint32_t &index)
{
...
void AudioCodeDvaudioDecoderUnitTest::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
...
bool AudioCodeDvaudioDecoderUnitTest::ReadBuffer(OH_AVBuffer *buffer, uint32_t index)
{
    ...
    if (readRet != OH_AVErrCode::AV_ERR_OK) {
        cout << "Fatal: OH_AVDemuxer_ReadSampleBuffer fail loop=" << loopIndex_ << " frame=" << frameCount_
             ...
             << " buffer size: " << buffer->buffer_->memory_->GetSize() << endl;
        return false;
    }
...
int64_t AudioCodeDvaudioDecoderUnitTest::GetFileSize(const char *fileName)
{
    int64_t fileSize = 0;
    if (fileName != nullptr) {
...
void AudioCodeDvaudioDecoderUnitTest::InputFunc()
{
...
void AudioCodeDvaudioDecoderUnitTest::OutputFunc()
{
    if (!pcmOutputFile_.is_open()) {
        std::cout << "open " << DVAUDIO_FILE_TODEMUX << " failed!" << std::endl;
    }
    while (isRunning_.load()) {
...
int32_t AudioCodeDvaudioDecoderUnitTest::Start()
{
    isRunning_.store(true);
    inputLoop_ = make_unique<thread>(&AudioCodeDvaudioDecoderUnitTest::InputFunc, this);
    if (inputLoop_ == nullptr) {
        cout << "Fatal: No memory" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputLoop_ = make_unique<thread>(&AudioCodeDvaudioDecoderUnitTest::OutputFunc, this);
...
int32_t AudioCodeDvaudioDecoderUnitTest::Stop()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeDvaudioDecoderUnitTest::StopLoops()
{
    if (signal_ != nullptr) {
        {
...
int32_t AudioCodeDvaudioDecoderUnitTest::InitFile(string_view input)
{
    CleanUp();
    isFirstFrame_ = true;
    frameCount_ = 0;
    pcmOutputFile_.open(OUTPUT_DVAUDIO_PCM_FILE_PATH.data(), std::ios::out | std::ios::binary);
    if (!pcmOutputFile_.is_open()) {
...
int32_t AudioCodeDvaudioDecoderUnitTest::InitFile(string_view input, string_view output)
{
    CleanUp();
    isFirstFrame_ = true;
    frameCount_ = 0;
...
int32_t AudioCodeDvaudioDecoderUnitTest::CreateCodecFunc()
{
...
int32_t AudioCodeDvaudioDecoderUnitTest::Configure()
{
...
int32_t AudioCodeDvaudioDecoderUnitTest::Reset()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeDvaudioDecoderUnitTest::CleanUp()
{
    if (pcmOutputFile_.is_open()) {
        pcmOutputFile_.close();
    }
...
HWTEST_F(AudioCodeDvaudioDecoderUnitTest, AudioDemuxAndDecode_Dvaudio_001, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile(DVAUDIO_FILE_TODEMUX, OUTPUT_DVAUDIO_PCM_FILE_PATH));
