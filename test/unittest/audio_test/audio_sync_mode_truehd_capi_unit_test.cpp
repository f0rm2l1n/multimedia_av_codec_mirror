...
namespace {
constexpr int32_t DEFAULT_CHANNELS = 8;
...
{
    inputFile_ = std::make_unique<std::ifstream>(TRUEHD_FILE_TODEMUX.data(), std::ios::binary);
    if (!inputFile_->is_open()) {
        cout << "Fatal: open input file failed:" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputFile_ = std::make_unique<std::ofstream>(OUTPUT_TRUEHD_PCM_FILE_PATH.data(), std::ios::out | std::ios::binary);
    if (!outputFile_->is_open()) {
    ...
    int fd = open(TRUEHD_FILE_TODEMUX.data(), O_RDONLY);
    int64_t size = GetFileSize(TRUEHD_FILE_TODEMUX.data());
    cout << TRUEHD_FILE_TODEMUX.data() << "----------------------" << fd << "---------" << size << endl;
...
HWTEST_F(AudioSyncModeCapiUnitTest, truehd_decode_01, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());

    codec_ = OH_AudioCodec_CreateByMime(OH_AVCODEC_MIMETYPE_AUDIO_TRUEHD, false);
...
HWTEST_F(AudioSyncModeCapiUnitTest, truehd_InvalidSizeTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, truehd_InvalidOffsetTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, truehd_InvalidAttrTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
