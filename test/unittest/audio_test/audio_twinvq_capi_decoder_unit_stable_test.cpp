...
constexpr string_view OUTPUT_TWINVQ_PCM_FILE_PATH = "/data/test/media/twinvq_8000_mono.pcm";
} // namespace
...
void AudioCodeTwinvqDecoderUnitTest::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
...
void AudioCodeTwinvqDecoderUnitTest::HandleInputEOS(const uint32_t index)
{
    std::cout << "end buffer\n";
...
void AudioCodeTwinvqDecoderUnitTest::StopLoops()
{
    if (signal_ != nullptr) {
        {
...
int32_t AudioCodeTwinvqDecoderUnitTest::Reset()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeTwinvqDecoderUnitTest::CleanUp()
{
    if (pcmOutputFile_.is_open()) {
        pcmOutputFile_.close();
    }
