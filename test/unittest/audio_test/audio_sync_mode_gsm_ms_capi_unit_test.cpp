...
{
    inputFile_ = std::make_unique<std::ifstream>(GSM_MS_FILE_TODEMUX.data(), std::ios::binary);
    if (!inputFile_->is_open()) {
        cout << "Fatal: open input file failed:" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputFile_ = std::make_unique<std::ofstream>(OUTPUT_GSM_MS_PCM_FILE_PATH.data(), std::ios::out | std::ios::binary);
    if (!outputFile_->is_open()) {
...
HWTEST_F(AudioSyncModeCapiUnitTest, gsm_ms_decode_01, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());

    codec_ = OH_AudioCodec_CreateByMime(OH_AVCODEC_MIMETYPE_AUDIO_GSM_MS, false);
...
HWTEST_F(AudioSyncModeCapiUnitTest, gsm_ms_InvalidSizeTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, gsm_ms_InvalidOffsetTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
...
HWTEST_F(AudioSyncModeCapiUnitTest, gsm_ms_InvalidAttrTest, TestSize.Level1)
{
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, InitFile());
