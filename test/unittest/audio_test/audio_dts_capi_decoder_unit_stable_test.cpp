...
class AudioCodeDts_DecoderUnitTest : public testing::Test {
public:
...
void AudioCodeDts_DecoderUnitTest::SetUpTestCase(void)
{
    cout << "[SetUpTestCase]: " << endl;
}

void AudioCodeDts_DecoderUnitTest::TearDownTestCase(void)
{
    cout << "[TearDownTestCase]: " << endl;
}

void AudioCodeDts_DecoderUnitTest::SetUp(void)
{
...
void AudioCodeDts_DecoderUnitTest::TearDown(void)
{
...
void AudioCodeDts_DecoderUnitTest::Release()
{
    Stop();
...
void AudioCodeDts_DecoderUnitTest::HandleEOS(const uint32_t &index)
{
...
void AudioCodeDts_DecoderUnitTest::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
...
bool AudioCodeDts_DecoderUnitTest::ReadBuffer(OH_AVBuffer *buffer, uint32_t index)
{
...
int64_t AudioCodeDts_DecoderUnitTest::GetFileSize(const char *fileName)
{
    int64_t fileSize = 0;
    if (fileName != nullptr) {
...
void AudioCodeDts_DecoderUnitTest::InputFunc()
{
...
void AudioCodeDts_DecoderUnitTest::OutputFunc()
{
    if (!pcmOutputFile_.is_open()) {
...
int32_t AudioCodeDts_DecoderUnitTest::Start()
{
    isRunning_.store(true);
    inputLoop_ = make_unique<thread>(&AudioCodeDts_DecoderUnitTest::InputFunc, this);
    if (inputLoop_ == nullptr) {
        cout << "Fatal: No memory" << endl;
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    outputLoop_ = make_unique<thread>(&AudioCodeDts_DecoderUnitTest::OutputFunc, this);
...
int32_t AudioCodeDts_DecoderUnitTest::Stop()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeDts_DecoderUnitTest::StopLoops()
{
    if (signal_ != nullptr) {
        {
...
int32_t AudioCodeDts_DecoderUnitTest::InitFile()
{
    CleanUp();
    isFirstFrame_ = true;
    frameCount_ = 0;
    ...
    fd_ = open(DTS_FILE_TODEMUX.data(), O_RDONLY);
    if (fd_ < 0) {
        cout << "Fatal: open input file failed" << endl;
        CleanUp();
        return OH_AVErrCode::AV_ERR_UNKNOWN;
    }
    int64_t size = GetFileSize(DTS_FILE_TODEMUX.data());
...
int32_t AudioCodeDts_DecoderUnitTest::CreateCodecFunc()
{
...
int32_t AudioCodeDts_DecoderUnitTest::Configure()
{
    if (format_ == nullptr) {
...
int32_t AudioCodeDts_DecoderUnitTest::Reset()
{
    isRunning_.store(false);
    StopLoops();
    if (signal_) {
...
void AudioCodeDts_DecoderUnitTest::CleanUp()
{
    if (pcmOutputFile_.is_open()) {
        pcmOutputFile_.close();
    }
...
HWTEST_F(AudioCodeDts_DecoderUnitTest, audioDecoder_Dts_loop_test_01, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    EXPECT_EQ(OH_AVErrCode::AV_ERR_OK, Configure());
    for (int i = 0; i < g_loopNumber200; i++) {
...
HWTEST_F(AudioCodeDts_DecoderUnitTest, audioDecoder_Dts_loop_test_02, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    for (int i = 0; i < g_loopNumber200; i++) {
...
HWTEST_F(AudioCodeDts_DecoderUnitTest, audioDecoder_Dts_loop_test_03, TestSize.Level1)
{
    isTestingFormat_ = true;
    ASSERT_EQ(OH_AVErrCode::AV_ERR_OK, CreateCodecFunc());
    EXPECT_EQ(OH_AVErrCode::AV_ERR_OK, Configure());
