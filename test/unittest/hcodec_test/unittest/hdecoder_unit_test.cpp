...
HWTEST_F(HDecoderPreparingUnitTest, configure_vrr_without_framerate, TestSize.Level1)
{
    std::shared_ptr<HCodec> testObj = HCodec::Create(GetCodecName(false, "video/avc"));
    ASSERT_TRUE(testObj);
    Media::Meta meta{};
...
/* ============================ GetInputFormat ============================ */
HWTEST_F(HDecoderUserCallingUnitTest, get_input_format_ok, TestSize.Level1)
{
    std::shared_ptr<HCodec> testObj = HCodec::Create(GetCodecName(false, "video/avc"));
    ASSERT_TRUE(testObj);
    Media::Meta meta{};
    ...
    Format inputFormat;
    ret = testObj->GetInputFormat(inputFormat);
    EXPECT_EQ(AVCS_ERR_OK, ret);
    TLOGI("%s", inputFormat.Stringify().c_str());
...
/* ============================ GetInputFormat ============================ */
HWTEST_F(HDecoderUserCallingUnitTest, get_output_format_ok, TestSize.Level1)
{
    std::shared_ptr<HCodec> testObj = HCodec::Create(GetCodecName(false, "video/avc"));
    ASSERT_TRUE(testObj);
    Media::Meta meta{};
    ...
    ret = testObj->GetOutputFormat(outputFormat);
    EXPECT_EQ(AVCS_ERR_OK, ret);
    TLOGI("%s", outputFormat.Stringify().c_str());
...
/* ============================ BACKGROUND ============================ */
HWTEST_F(HDecoderUserCallingUnitTest, set_freeze_when_codec_is_configured_invalid_001, TestSize.Level1)
{
    std::shared_ptr<HCodec> testObj = HCodec::Create(GetCodecName(false, "video/hevc"));
    ASSERT_TRUE(testObj);
    Media::Meta meta{};
