...
HWTEST_F(MediaDemuxerExtUnitTest, HandleHlsRebootPlugin_001, TestSize.Level1)
{
    ...
    auto ret = mediaDemuxer_->HandleHlsRebootPlugin();
    EXPECT_EQ(ret, Status::OK);
}
...
HWTEST_F(MediaDemuxerExtUnitTest, HandleHlsRebootPlugin_002, TestSize.Level1)
{
    ...
    mediaDemuxer_->seekReadyStreamInfo_[static_cast<int32_t>(StreamType::MIXED)] = {NUM_100, false};

    mediaDemuxer_->HandleHlsRebootPlugin();
}
...
HWTEST_F(MediaDemuxerExtUnitTest, HandleHlsRebootPlugin_003, TestSize.Level1)
{
    ...
    EXPECT_CALL(*(mediaDemuxer_->demuxerPluginManager_), RebootPlugin(_, _, _, _)).Times(0);
    mediaDemuxer_->seekReadyStreamInfo_.clear();
    mediaDemuxer_->seekReadyStreamInfo_[static_cast<int32_t>(StreamType::MIXED)] = {NUM_1, true};
    EXPECT_TRUE(mediaDemuxer_->seekReadyStreamInfo_.find(static_cast<int32_t>(StreamType::MIXED)) !=
        mediaDemuxer_->seekReadyStreamInfo_.end());
    auto seekReadyInfo = mediaDemuxer_->seekReadyStreamInfo_[static_cast<int32_t>(StreamType::MIXED)];
...
HWTEST_F(MediaDemuxerExtUnitTest, HandleHlsRebootPlugin_004, TestSize.Level1)
{
    ...
    mediaDemuxer_->seekReadyStreamInfo_[static_cast<int32_t>(StreamType::MIXED)] = {-1, false};
...
HWTEST_F(MediaDemuxerExtUnitTest, HandleHlsRebootPlugin_005, TestSize.Level1)
{
    ...
    EXPECT_CALL(*(mediaDemuxer_->demuxerPluginManager_), RebootPlugin(_, _, _, _)).Times(0);
    mediaDemuxer_->seekReadyStreamInfo_[static_cast<int32_t>(StreamType::MIXED)] = {200, false};
    Status ret = mediaDemuxer_->HandleHlsRebootPlugin();
    EXPECT_EQ(ret, Status::OK);
}
...
HWTEST_F(MediaDemuxerExtUnitTest, MediaDemuxer_HandleVideoSampleQueue_001, TestSize.Level1)
{
    ...
    Status status = mediaDemuxer_->sampleQueueMap_[mediaDemuxer_->videoTrackId_]->Init(sampleQueueConfig);
    EXPECT_EQ(status, Status::OK);
    EXPECT_CALL(*sampleQueue, AddQueueSize(_)).WillRepeatedly(Return(Status::OK));
 ...
 * @tc.type: FUNC
 */
HWTEST_F(MediaDemuxerExtUnitTest, MediaDemuxer_HandleVideoSampleQueue_002, TestSize.Level1)
{
    mediaDemuxer_->videoTrackId_ = NUM_0;
    auto sampleQueue = std::make_shared<SampleQueue>();
    mediaDemuxer_->sampleQueueMap_[mediaDemuxer_->videoTrackId_] = sampleQueue;
...
HWTEST_F(MediaDemuxerExtUnitTest, MediaDemuxer_HandlePushBuffer_001, TestSize.Level1)
{
    ...
    mediaDemuxer_->videoTrackId_ = NUM_1;
    mediaDemuxer_->needReleaseVideoDecoder_ = true;
    ret = mediaDemuxer_->HandlePushBuffer(NUM_0, dstBuffer,
        mediaDemuxer_->bufferQueueMap_[NUM_0], Status::OK);
    EXPECT_EQ(ret, Status::OK);
...
/**
 * @tc.name: MediaDemuxerExt_HandlePushBuffer_003
 * @tc.desc: test HandlePushBuffer
 * @tc.type: FUNC
 */
HWTEST_F(MediaDemuxerExtUnitTest, MediaDemuxer_HandlePushBuffer_003, TestSize.Level1)
{
    mediaDemuxer_->videoTrackId_ = NUM_0;
    auto* mockProducer = new MockAVBufferQueueProducer();
    sptr<AVBufferQueueProducer> sptrProducer(mockProducer);
    ...
    std::shared_ptr<AVBuffer> dstBuffer = std::make_shared<AVBuffer>();
    dstBuffer->pts_ = NUM_0;
    dstBuffer->flag_ = static_cast<uint32_t>(Plugins::AVBufferFlag::NONE);
    mediaDemuxer_->needReleaseVideoDecoder_ = false;
...
/**
 * @tc.name: MediaDemuxerExt_HandlePushBuffer_005
 * @tc.desc: test HandlePushBuffer
 * @tc.type: FUNC
 */
HWTEST_F(MediaDemuxerExtUnitTest, MediaDemuxer_HandlePushBuffer_005, TestSize.Level1)
{
    mediaDemuxer_->videoTrackId_ = NUM_0;
    auto* mockProducer = new MockAVBufferQueueProducer();
    sptr<AVBufferQueueProducer> sptrProducer(mockProducer);
...
HWTEST_F(MediaDemuxerExtUnitTest, MediaDemuxer_GetBufferFromUserQueue_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(mediaDemuxer_->hasSetLargeSize_, false);
    EXPECT_EQ(ret, true);
    ...
    mediaDemuxer_->mediaMetaData_.globalMeta->Set<Tag::MEDIA_DURATION>(duration);
    mediaDemuxer_->isVideoMuted_ = true;
    mediaDemuxer_->lastVideoPts_ = MAX_VIDEO_LEAD_TIME_ON_MUTE_US;
    mediaDemuxer_->lastAudioPtsInMute_ = 0;
    ret = mediaDemuxer_->GetBufferFromUserQueue(NUM_0, NUM_100);
    EXPECT_EQ(ret, false);
