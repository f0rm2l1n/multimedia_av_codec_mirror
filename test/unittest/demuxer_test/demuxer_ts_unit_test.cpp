...
string g_mtsPath3 = TEST_FILE_PATH + string("h264_mp3_ut.mts");
string g_mtsUri3 = TEST_URI_PATH + string("h264_mp3_ut.mts");
...
string g_m2tsPath2 = TEST_FILE_PATH + string("h264_mp3_ut.m2ts");
string g_m2tsUri2 = TEST_URI_PATH + string("h264_mp3_ut.m2ts");
...
HWTEST_F(DemuxerUnitTest, Demuxer_SeekToTime_2402, TestSize.Level1)
{
    ...
    vector<int32_t> videoVals = {103, 103, 103, 16, 16, 16, 12, 12, 12, 19, 20, 19, 28, 28, 28, 25, 26, 25, 1, 1, 1};
    sharedMem_ = AVMemoryMockFactory::CreateAVMemoryMock(bufferSize_);
    EXPECT_NE(sharedMem_, nullptr);
    for (auto toPts = toPtsList.begin(); toPts != toPtsList.end(); toPts++) {
        for (auto mode = seekModes.begin(); mode != seekModes.end(); mode++) {
