...
string g_mpegPath = TEST_FILE_PATH + string("m4v_h264_aac.m4v");
string g_mpegUri = TEST_URI_PATH + string("m4v_h264_aac.m4v");

} //namespace

namespace {
/**
 * @tc.name: Demuxer_ReadSample_4250
 * @tc.desc: Read sample test for timed metadata track,local
 * @tc.type: FUNC
 */
HWTEST_F(DemuxerUnitTest, Demuxer_ReadSample_4250, TestSize.Level1)
{
    ...
    ASSERT_EQ(frames_[0], 454);
    ASSERT_EQ(frames_[1], 653);
    ASSERT_EQ(keyFrames_[0], 2);
    ASSERT_EQ(keyFrames_[1], 653);
    RemoveValue();
}

/**
 * @tc.name: Demuxer_ReadSample_4251
 * @tc.desc: Read sample test for timed metadata track,Uri
 * @tc.type: FUNC
 */
HWTEST_F(DemuxerUnitTest, Demuxer_ReadSample_4251, TestSize.Level1)
{
...
/**
 * @tc.name: Demuxer_SeekToTime_4252
 * @tc.desc: seek to the specified time
 * @tc.type: FUNC
 */
HWTEST_F(DemuxerUnitTest, Demuxer_SeekToTime_4252, TestSize.Level1)
{
    InitResource(g_mpegPath, LOCAL);
    ASSERT_TRUE(initStatus_);
    ASSERT_TRUE(SetInitValue());
    ...
    vector<int32_t> videoVals = {454, 454, 454, 204, 454, 454};
    vector<int32_t> audioVals = {653, 653, 653, 295, 653, 653};
    sharedMem_ = AVMemoryMockFactory::CreateAVMemoryMock(bufferSize_);
    ASSERT_NE(sharedMem_, nullptr);
...
/**
 * @tc.name: Demuxer_SeekToTime_4253
 * @tc.desc: seek to the specified time
 * @tc.type: FUNC
 */
HWTEST_F(DemuxerUnitTest, Demuxer_SeekToTime_4253, TestSize.Level1)
{
    InitResource(g_mpegUri, URI);
    ASSERT_TRUE(initStatus_);
    ASSERT_TRUE(SetInitValue());
