...
#define FUZZ_PROJECT_NAME "av1swdecoderserver_fuzzer"
const size_t EXPECT_SIZE = 8;
OH_AVCapability *cap_av1 = nullptr;
...
void GetDescription()
{
    ...
    vdec_ = OH_VideoDecoder_CreateByName(g_codecNameAv1.c_str());
    ...
    const char *avcodecMmType = OH_AVCODEC_MIMETYPE_VIDEO_AV1;
...
bool Av1SwdecoderServerFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Av1SwdecoderServerFuzzTest(data, size);
    return 0;
}