...
constexpr const char* FLAC_OUTPUT_FILE_PATH = "/data/test/media/muxer_FLAC_outputfile.flac";
...
{
    audioType_ = AudioMuxerFormatType::TYPE_FLAC;
    output_path_ = FLAC_OUTPUT_FILE_PATH;
    output_format_ = OH_AVOutputFormat::AV_OUTPUT_FORMAT_FLAC;
...
AudioTrackParam AVMuxerDemo::InitFormatParam(AudioMuxerFormatType type)
{
    ...
    param.mimeType = OH_AVCODEC_MIMETYPE_AUDIO_FLAC;
...
int32_t AVMuxerDemo::AddTrack(OH_AVMuxer* muxer, int32_t& trackIndex, AudioTrackParam param)
{
    ...
    OH_AVFormat_SetIntValue(trackFormat, OH_MD_KEY_AUDIO_SAMPLE_FORMAT, static_cast<int32_t>(SAMPLE_S16LE));
    //video
...
void AVMuxerDemo::WriteTrackCover(OH_AVMuxer *muxer, int32_t trackIndex)
{
    ...
    size_t cSize = inputdatasize_ < frameBytes ? inputdatasize_ : frameBytes;
    errno_t res = 0;
    res = strncpy_s(reinterpret_cast<char*>(OH_AVBuffer_GetAddr(av_buffer)), cSize, inputdata_.c_str(), cSize);
    if (res != 0) {
        return;
    }
...
bool AVMuxerDemo::UpdateWriteBufferInfoAVBuffer(OH_AVBuffer **buffer, OH_AVCodecBufferAttr *info)
{
    ...
    errno_t res = 0;
    res = strncpy_s(reinterpret_cast<char *>(OH_AVBuffer_GetAddr(*buffer)), info->size, inputdata_.c_str(), info->size);
    if (res != 0) {
        return false;
    }
