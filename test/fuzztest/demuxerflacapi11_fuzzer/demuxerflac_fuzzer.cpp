...
namespace OHOS {
const char *FLAC_PATH = "/data/test/fuzz_create.flac";
...
bool CheckDataValidity(const uint8_t *data, size_t size)
{
    ...
    int32_t fd = open(FLAC_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
