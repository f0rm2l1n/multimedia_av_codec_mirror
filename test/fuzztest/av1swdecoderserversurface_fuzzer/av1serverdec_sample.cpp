...
int32_t Av1VDecServerSample::SetParameter()
{
    Format fmt;
    fmt.PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, WIDTH);
    fmt.PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, HEIGHT);
...
int32_t Av1VDecServerSample::InitDecoder()
{
    ...
    err = GetAv1DecoderCapabilityList(caps);
    if (err != AVCS_ERR_OK) {
        cout << "GetAv1DecoderCapabilityList failed" << endl;
        return err;
    }
...
void Av1VDecServerSample::RunVideoServerSurfaceDecoder()
{
