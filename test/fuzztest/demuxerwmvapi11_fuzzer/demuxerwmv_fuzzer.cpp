...
namespace OHOS {
const char *WMV_PATH = "/data/test/fuzz_create.wmv";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t size)
{
    ...
    int32_t fd = open(WMV_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
