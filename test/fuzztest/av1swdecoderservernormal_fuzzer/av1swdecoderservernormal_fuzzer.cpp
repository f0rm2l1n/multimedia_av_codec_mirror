...
#define FUZZ_PROJECT_NAME "av1swdecoderservernormal_fuzzer"
const size_t EXPECT_SIZE = 26;

namespace OHOS {
bool Av1SwdecoderServerNormalFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Av1SwdecoderServerNormalFuzzTest(data, size);
    return 0;
}
