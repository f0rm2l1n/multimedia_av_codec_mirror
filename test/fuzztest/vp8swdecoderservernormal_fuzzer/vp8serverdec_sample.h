...
class Vp8VDecServerSample : public NoCopyable {
public:
    Vp8VDecServerSample() = default;
    ~Vp8VDecServerSample();

    void RunVideoServerDecoder();
    const char *inpDir = "/data/test/media/test.vp8";
    const char *outDir = "/data/test/media/Vp8VDecTest.yuv";
...
protected:
    ...
    struct CallBack : public MediaCodecCallback {
        explicit CallBack(Vp8VDecServerSample* tester) : tester(tester) {}
