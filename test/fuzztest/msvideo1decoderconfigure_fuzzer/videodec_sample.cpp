...
int32_t VDecFuzzSample::ConfigureVideoDecoder()
{
    ...
    auto source_ = OH_AVSource_CreateWithURI(filename.data());
    ...
    for (int32_t index = 0; index < trackCount; index++) {
        ...
        if (trackType == MEDIA_TYPE_VID) {
            OH_AVFormat_Copy(format, trackFormat.get());
        }
    }
...
int32_t VDecFuzzSample::CreateVideoDecoder(string codeName)
{
    ...
    tmpDec = OH_VideoDecoder_CreateByMime(OH_AVCODEC_MIMETYPE_VIDEO_MSVIDEO1);
    if (tmpDec) {
        OH_VideoDecoder_Destroy(tmpDec);
        tmpDec = nullptr;
    }
