...
#include "audio_decoder_truehd_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr uint32_t TRUEHD_SAMPLE_RATE = 48000;
constexpr uint32_t FRAME_BYTES_DEFAULT = 1024;
constexpr uint32_t FRAME_BYTES_MP3_FLAC_APE = 4096;
constexpr uint32_t FRAME_BYTES_TRUEHD = 8192;
constexpr string_view INPUT_TRUEHD_FILE_PATH = "/data/test/media/truehd.truehd";
} // namespace
...
{
    std::ifstream inputFile(INPUT_TRUEHD_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_TRUEHD) {
        channelCount = CHANNEL_COUNT;
        sampleRate = TRUEHD_SAMPLE_RATE;
    }
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("truehd") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_TRUEHD;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_TRUEHD_NAME).data());
        std::cout << "CreateDec TYPE_TRUEHD!" << endl;
    } else {
...
void ADecBufferDemo::InputFunc()
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_TRUEHD) {
        frameBytes = FRAME_BYTES_TRUEHD;
