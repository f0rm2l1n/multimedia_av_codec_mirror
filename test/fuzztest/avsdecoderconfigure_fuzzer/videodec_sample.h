...
public:
    ...
    const char *inpDir = "/data/test/media/test.avs";
    ...
    uint32_t SendDataAVS(uint32_t bufferSize, uint32_t index, OH_AVBuffer *buffer);
    void CopyStartCode(uint8_t *frameBuffer, uint32_t bufferSize, OH_AVCodecBufferAttr &attr);
    int32_t ReadDataAVS(uint32_t index, OH_AVBuffer *buffer);
    void WaitForEOS();
