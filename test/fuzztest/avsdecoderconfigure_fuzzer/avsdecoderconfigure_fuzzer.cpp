...
#define FUZZ_PROJECT_NAME "avsdecoderconfigure_fuzzer"
const size_t EXPECT_SIZE = 6;
namespace OHOS {

bool AvsDecoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::AvsDecoderConfigureFuzzTest(data, size);
#endif
    return 0;
}
