...
constexpr uint32_t AVS_START_CODE_SIZE = 4;
constexpr uint8_t AVS_START_CODE[AVS_START_CODE_SIZE] = {0x00, 0x00, 0x01, 0xB6}; // AVS帧头
...
int32_t VDecFuzzSample::StartVideoDecoder()
{
    ...
    inputLoop_ = make_unique<thread>(&VDecFuzzSample::InputFuncAVS, this);
...
int32_t VDecFuzzSample::CreateVideoDecoder(string codeName)
{
    ...
    tmpDec = OH_VideoDecoder_CreateByMime(OH_AVCODEC_MIMETYPE_VIDEO_AVS);
    if (tmpDec) {
        OH_VideoDecoder_Destroy(tmpDec);
        tmpDec = nullptr;
    }
...
{
    if (memcpy_s(frameBuffer, bufferSize + AVS_START_CODE_SIZE, AVS_START_CODE, AVS_START_CODE_SIZE) != EOK) {
        cout << "Fatal: memory copy failed" << endl;
    }
    attr.pts = GetSystemTimeUs();
    attr.size = bufferSize + AVS_START_CODE_SIZE;
    attr.offset = 0;
    attr.flags = AVCODEC_BUFFER_FLAGS_NONE;
}

int32_t VDecFuzzSample::ReadDataAVS(uint32_t index, OH_AVBuffer *buffer)
{
    uint8_t ch[4] = {};
    (void)inFile_->read(reinterpret_cast<char *>(ch), AVS_START_CODE_SIZE);
    if (inFile_->eof()) {
        SetEOS(buffer, index);
        return 1;
    }
    ...
    return SendDataAVS(bufferSize, index, buffer);
}

uint32_t VDecFuzzSample::SendDataAVS(uint32_t bufferSize, uint32_t index, OH_AVBuffer *buffer)
{
    ...
    frameBuffer = new uint8_t[bufferSize + AVS_START_CODE_SIZE];
    // 拷贝AVS帧头
    memcpy_s(frameBuffer, bufferSize + AVS_START_CODE_SIZE, AVS_START_CODE, AVS_START_CODE_SIZE);
    (void)inFile_->read(reinterpret_cast<char *>(frameBuffer + AVS_START_CODE_SIZE), bufferSize);
...
void VDecFuzzSample::InputFuncAVS()
{
    frameCount_ = 1;
    errCount = 0;
    while (isRunning_.load()) {
        ...
        lock.unlock();
        if (!inFile_->eof()) {
            int ret = ReadDataAVS(index, buffer);
            if (ret == 1) {
                break;
            }
        }
