...
constexpr uint8_t SPS[SPS_SIZE + START_CODE_SIZE] = {0x00,0x00,0x00,0x01,0x67,0x64,0x00,0x28,0xAC,0xB4,0x03,
                                                     0xC0,0x11,0x3F,0x2E,0x02,0x20,0x00,0x00,0x03,0x00,0x20,
                                                     0x00,0x00,0x07,0x81,0xE3,0x06,0x54};
constexpr uint8_t PPS[PPS_SIZE + START_CODE_SIZE] = {0x00,0x00,0x00,0x01,0x68,0xEF,0x0F,0x2C,0x8B};
bool isSurfMode = true;
...
bool HwdecoderFuzzTest(const uint8_t *data, size_t size)
{
    ...
    if (!vDecSample) {
        ...
        vDecSample->isSurfMode = isSurfMode;
        isSurfMode = !isSurfMode;
        ...
        vDecSample->Start();
        vDecSample->InputFuncFUZZ(SPS, SPS_SIZE + START_CODE_SIZE);
        vDecSample->InputFuncFUZZ(PPS, PPS_SIZE + START_CODE_SIZE);
    }
    OH_AVErrCode ret = vDecSample->InputFuncFUZZ(data, size);
    vDecSample->SetParameter(data_);
