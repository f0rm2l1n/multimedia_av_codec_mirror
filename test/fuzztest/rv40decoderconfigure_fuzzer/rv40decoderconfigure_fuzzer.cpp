...
#define FUZZ_PROJECT_NAME "rv40decoderconfigure_fuzzer"
...
bool Rv40decoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
    ...
    std::string filename = "/data/test/corpus-Rv40decoderConfigureFuzzTest";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Rv40decoderConfigureFuzzTest(data, size);
#else
    (void)data;
    (void)size;
#endif
