...
namespace OHOS {
const char *MOV_PATH = "/data/test/fuzz_create.mov";
...
bool CheckDataValidity(const uint8_t *data, size_t size)
{
    ...
    int32_t fd = open(MOV_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
