...
#define FUZZ_PROJECT_NAME "rv30decoderconfigure_fuzzer"
...
bool Rv30decoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
    ...
    std::string filename = "/data/test/corpus-Rv30decoderConfigureFuzzTest";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Rv30decoderConfigureFuzzTest(data, size);
#else
    (void)data;
    (void)size;
#endif
