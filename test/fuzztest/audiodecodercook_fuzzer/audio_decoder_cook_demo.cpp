...
#include "audio_decoder_cook_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr size_t COOK_MIN_FRAME = 66;
constexpr uint32_t SAMPLE_RATE = 44100;
constexpr uint32_t DEFAULT_AAC_TYPE = 1;
constexpr uint32_t AMRWB_SAMPLE_RATE = 16000;
constexpr uint32_t AMRNB_SAMPLE_RATE = 8000;
constexpr uint32_t FRAME_BYTES_DEFAULT = 1024;
constexpr uint32_t FRAME_BYTES_MP3_FLAC_APE = 4096;
constexpr uint32_t FRAME_BYTES_COOK = 66 * 16;
constexpr string_view INPUT_COOK_FILE_PATH = "/data/test/media/cook.dat";
...
{
    std::ifstream inputFile(INPUT_COOK_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_COOK) {
        int32_t channelCounttmpcook = 2;
        int32_t sampleRatetmpcook = 44100;
        channelCount = channelCounttmpcook;
        sampleRate = sampleRatetmpcook;
        ...
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_BLOCK_ALIGN, BLOCK_ALIGN);
    }
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("cook") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_COOK;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        std::cout << "CreateDec TYPE_COOK!" << endl;
    } else {
...
void ADecBufferDemo::InputFuncInner(size_t frameBytes)
{
    while (isRunning_.load()) {
        ...
        if (audioType_ == AudioBufferFormatType::TYPE_COOK && currentSize < COOK_MIN_FRAME) {
        ...
        auto dst = reinterpret_cast<char*>(OH_AVBuffer_GetAddr(buffer));
        errno_t ret = strncpy_s(dst, currentSize, currentStr.c_str(), currentSize);
        DEMO_CHECK_AND_BREAK_LOG(ret == EOK, "strncpy_s fail");
        buffer->buffer_->memory_->SetSize(currentSize);

        int32_t pushRet;
        if (isFirstFrame_ && IsBufferFormatAacOrVorbis()) {
            buffer->buffer_->flag_ = AVCODEC_BUFFER_FLAGS_CODEC_DATA;
            pushRet = OH_AudioCodec_PushInputBuffer(audioDec_, index);
            isFirstFrame_ = false;
        } else {
...
{
    size_t frameBytes = FRAME_BYTES_DEFAULT;
    if (audioType_ == AudioBufferFormatType::TYPE_COOK) {
        frameBytes = FRAME_BYTES_COOK;
    } else if (audioType_ == AudioBufferFormatType::TYPE_MP3 ||
               audioType_ == AudioBufferFormatType::TYPE_FLAC ||
               audioType_ == AudioBufferFormatType::TYPE_APE) {
        frameBytes = FRAME_BYTES_MP3_FLAC_APE;
    }

    InputFuncInner(frameBytes);
