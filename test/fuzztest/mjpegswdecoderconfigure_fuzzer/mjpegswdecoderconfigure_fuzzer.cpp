...
#define FUZZ_PROJECT_NAME "mjpegswdecoderconfigure_fuzzer"
...
bool MjpegSwdecoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    ...
    vDecSample->inpDir = "";
    int32_t widthMin = 16;
    int32_t widthMax = 4096;
    int32_t heightMin = 16;
    int32_t heightMax = 4096;
    int32_t frameRateMin = 1;
    int32_t frameRateMax = 1000;
    vDecSample->defaultWidth = std::clamp(fdp.ConsumeIntegral<int32_t>(), widthMin, widthMax);
    vDecSample->defaultHeight = std::clamp(fdp.ConsumeIntegral<int32_t>(), heightMin, heightMax);
    ...
    if (vDecSample->CreateVideoDecoder("OH.Media.Codec.Decoder.Video.MJPEG") != AV_ERR_OK) {
        delete vDecSample;
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::MjpegSwdecoderConfigureFuzzTest(data, size);
    return 0;
}
