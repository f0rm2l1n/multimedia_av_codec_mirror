...
// for H264
const uint8_t DRM_H264_SM4C_KEY_ID[] = {
    0x55, 0x44, 0x52, 0x4d, 0x5f, 0x54, 0x45, 0x53,
    0x54, 0x5f, 0x43, 0x49, 0x44, 0x5f, 0x30, 0x37};
const uint8_t DRM_H264_SM4C_IV[] = {
    0x55, 0x44, 0x52, 0x4d, 0x5f, 0x54, 0x45, 0x53,
    0x54, 0x5f, 0x43, 0x49, 0x44, 0x5f, 0x30, 0x37};
uint32_t DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE = 418;
...
// for Audio
const uint8_t DRM_AUDIO_CBC1_KEY_ID[] = {
    0xf3, 0xc5, 0xe0, 0x36, 0x1e, 0x66, 0x54, 0xb2,
    0x8f, 0x80, 0x49, 0xc7, 0x78, 0xb2, 0x39, 0x46};
const uint8_t DRM_AUDIO_CBC1_IV[] = {
    0xa4, 0x63, 0x1a, 0x15, 0x3a, 0x44, 0x3d, 0xf9,
    0xee, 0xd0, 0x59, 0x30, 0x43, 0xdb, 0x75, 0x19};
uint32_t DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE = 65;
const uint8_t DRM_AUDIO_CBC1_ENCRYPTED_BUFFER[] = {
    0x41, 0x55, 0xf7, 0x4f, 0xbb, 0x05, 0x15, 0xec, 0x11, 0xba, 0x98, 0x92, 0xdc, 0x21, 0x72, 0x75,
    0x52, 0xfd, 0x9b, 0xff, 0x6c, 0x13, 0x75, 0x32, 0x11, 0x9a, 0x1c, 0xd4, 0xac, 0x58, 0x2c, 0x9a,
    0x27, 0x4a, 0xab, 0x5c, 0x5b, 0x5e, 0xe1, 0xe8, 0xb9, 0x80, 0x9b, 0x0c, 0x63, 0xd8, 0xd1, 0xd8,
    0xee, 0x8f, 0xce, 0x56, 0x14, 0xb2, 0xad, 0x30, 0x56, 0x7a, 0x22, 0x72, 0x85, 0x21, 0x1e, 0x47,
...
void CreateH264MediaCencInfo(MetaDrmCencInfo &cencInfo)
{
    ...
    memcpy_s(cencInfo.keyId, sizeof(cencInfo.keyId), DRM_H264_SM4C_KEY_ID, sizeof(cencInfo.keyId));
    cencInfo.keyIdLen = KEY_ID_LEN;
    memcpy_s(cencInfo.iv, sizeof(cencInfo.iv), DRM_H264_SM4C_IV, sizeof(cencInfo.iv));
    ...
    cencInfo.subSamples[0].clearHeaderLen = DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE;
...
void CreateAudioCencInfo(MetaDrmCencInfo &cencInfo)
{
    cencInfo.algo = MetaDrmCencAlgorithm::META_DRM_ALG_CENC_AES_CBC;
    memcpy_s(cencInfo.keyId, sizeof(cencInfo.keyId), DRM_AUDIO_CBC1_KEY_ID, sizeof(cencInfo.keyId));
    cencInfo.keyIdLen = KEY_ID_LEN;
    memcpy_s(cencInfo.iv, sizeof(cencInfo.iv), DRM_AUDIO_CBC1_IV, sizeof(cencInfo.iv));
    cencInfo.ivLen = IV_LEN;
    cencInfo.encryptBlocks = 0;
    cencInfo.skipBlocks = 0;
    cencInfo.firstEncryptOffset = 0;
...
bool DrmH264VideoCencDecrypt()
{
    ...
    std::shared_ptr<AVBuffer> drmInBuf = AVBuffer::CreateAVBuffer(avAllocator,
        static_cast<int32_t>(DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE));
    ...
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(
        (drmInBuf->memory_->GetCapacity() == static_cast<int32_t>(DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE)), false);
    int32_t drmRes = memcpy_s(drmInBuf->memory_->GetAddr(), DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE,
        DRM_H264_SM4C_ENCRYPTED_BUFFER, DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(drmRes == 0, false);
    drmInBuf->memory_->SetSize(DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    ...
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(
        (drmOutBuf->memory_->GetCapacity() == static_cast<int32_t>(DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE)), false);
    ...
    decryptor->DrmVideoCencDecrypt(drmInBuf, drmOutBuf, DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    drmOutBuf->memory_->SetSize(DRM_H264_SM4C_ENCRYPTED_BUFFER_SIZE);
    return true;
}
...
bool DrmAudioCencDecrypt()
{
    ...
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(avAllocator != nullptr, false);
    std::shared_ptr<AVBuffer> drmInBuf = AVBuffer::CreateAVBuffer(avAllocator,
        static_cast<int32_t>(DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE));
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(drmInBuf != nullptr, false);
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(drmInBuf->memory_ != nullptr, false);
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(
        (drmInBuf->memory_->GetCapacity() == static_cast<int32_t>(DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE)), false);
    int32_t drmRes = memcpy_s(drmInBuf->memory_->GetAddr(), DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE,
        DRM_AUDIO_CBC1_ENCRYPTED_BUFFER, DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE);
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(drmRes == 0, false);
    drmInBuf->memory_->SetSize(DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE);
    ...
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(drmOutBuf->memory_ != nullptr, false);
    DRM_DECRYPTOR_FUZZ_CHECK_AND_RETURN_RET(
        (drmOutBuf->memory_->GetCapacity() == static_cast<int32_t>(DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE)), false);
    ...
    std::vector<uint8_t> drmCencVec(reinterpret_cast<uint8_t *>(&cencInfo),
        (reinterpret_cast<uint8_t *>(&cencInfo)) + sizeof(MetaDrmCencInfo));
    drmInBuf->meta_->SetData(Media::Tag::DRM_CENC_INFO, std::move(drmCencVec));
    decryptor->DrmVideoCencDecrypt(drmInBuf, drmOutBuf, DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE);
    drmOutBuf->memory_->SetSize(DRM_AUDIO_CBC1_ENCRYPTED_BUFFER_SIZE);
    return true;
}
