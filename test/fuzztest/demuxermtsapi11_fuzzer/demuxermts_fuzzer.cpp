...
namespace OHOS {
const char *MTS_PATH = "/data/test/fuzz_create.mts";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t dataSize)
{
    ...
    int32_t fd = open(MTS_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
