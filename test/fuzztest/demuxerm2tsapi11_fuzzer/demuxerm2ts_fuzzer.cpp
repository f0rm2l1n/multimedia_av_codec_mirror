...
namespace OHOS {
const char *M2TS_PATH = "/data/test/fuzz_create.m2ts";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t size)
{
    ...
    int32_t fd = open(M2TS_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
