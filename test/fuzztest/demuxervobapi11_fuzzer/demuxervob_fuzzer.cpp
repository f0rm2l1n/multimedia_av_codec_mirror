...
namespace OHOS {
const char *VOB_PATH = "/data/test/fuzz_create.vob";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t dataSize)
{
    ...
    int32_t fd = open(VOB_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
