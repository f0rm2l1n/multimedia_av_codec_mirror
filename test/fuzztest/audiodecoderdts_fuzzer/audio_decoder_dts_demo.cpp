...
#include "audio_decoder_dts_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr uint32_t FRAME_BYTES_DTS = 8192;
constexpr string_view INPUT_DTS_FILE_PATH = "/data/test/media/dts.avi";
} // namespace
...
{
    std::ifstream inputFile(INPUT_DTS_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_DTS) {
        int32_t channelCounttmpdts = 1;
        int32_t sampleRatetmpdts = 48000;
        channelCount = channelCounttmpdts;
        sampleRate = sampleRatetmpdts;
    }
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("dts") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_DTS;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_DTS_NAME).data());
        std::cout << "CreateDec TYPE_DTS!" << endl;
    } else {
...
void ADecBufferDemo::InputFunc()
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_DTS) {
        frameBytes = FRAME_BYTES_DTS;
    } else if (audioType_ == AudioBufferFormatType::TYPE_MP3 || audioType_ == AudioBufferFormatType::TYPE_FLAC ||
               audioType_ == AudioBufferFormatType::TYPE_APE) {
