...
#define FUZZ_PROJECT_NAME "avsdecodersetparameter_fuzzer"
...
{
    format = OH_AVFormat_CreateVideoFormat("video/cavs", DEFAULT_WIDTH, DEFAULT_HEIGHT);
...
namespace OHOS {
bool AvsdecoderSetParameterFuzzTest(const uint8_t *data, size_t size)
{
    ...
    if (!vDecSample) {
        ...
        if (vDecSample->CreateVideoDecoder("OH.Media.Codec.Decoder.Video.AVS") != AV_ERR_OK) {
            delete vDecSample;
            vDecSample = nullptr;
            return false;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::AvsdecoderSetParameterFuzzTest(data, size);
#endif
    return 0;
}
