...
#define FUZZ_PROJECT_NAME "mpeg1decoderconfigure_fuzzer"
...
bool Mpeg1decoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
    ...
    std::string filename = "/data/test/mpeg1.avi";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Mpeg1decoderConfigureFuzzTest(data, size);
    return 0;
}
