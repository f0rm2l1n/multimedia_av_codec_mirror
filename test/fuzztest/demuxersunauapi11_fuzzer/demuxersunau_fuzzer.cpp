...
namespace OHOS {
const char *SUNAU_PATH = "/data/test/fuzz_create.au";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t dataSize)
{
    ...
    int32_t fd = open(SUNAU_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
