...
#include "audio_decoder_gsm_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr string_view INPUT_VORBIS_FILE_PATH = "/data/test/media/8000_mono_gsm.gsm";
} // namespace
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_GSM) {
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("gsm") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_GSM;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_GSM_NAME).data());
        std::cout << "CreateDec TYPE_GSM!" << endl;
    } else {
...
OH_AVCodec* ADecBufferDemo::CreateByMime(const char* mime)
{
    if (mime != nullptr) {
        if (strcmp(mime, "audio/mp4a-latm") == 0) {
            ...
            audioType_ = AudioBufferFormatType::TYPE_LBVC;
        } else if (strcmp(mime, OH_AVCODEC_MIMETYPE_AUDIO_GSM) == 0) {
            audioType_ = AudioBufferFormatType::TYPE_GSM;
        } else {
            audioType_ = AudioBufferFormatType::TYPE_VIVID;
        }
