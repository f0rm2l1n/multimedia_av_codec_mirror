...
#include "audio_decoder_eac3_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
{
    std::ifstream inputFile(INPUT_EAC3_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_EAC3) {
        channelCount = CHANNEL_COUNT;
        sampleRate = EAC3_SAMPLE_RATE;
    }
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("eac3") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_EAC3;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_EAC3_NAME).data());
        std::cout << "CreateDec TYPE_EAC3!" << endl;
    } else {
...
void ADecBufferDemo::InputFunc()
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_EAC3) {
        frameBytes = FRAME_BYTES_EAC3;
