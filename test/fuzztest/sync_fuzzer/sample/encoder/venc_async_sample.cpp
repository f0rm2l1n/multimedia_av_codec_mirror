...
int32_t VideoEncAsyncSample::FuzzStart()
{
    if (signal_ == nullptr || videoEnc_ == nullptr) {
        return AV_ERR_UNKNOWN;
    }
...
void VideoEncAsyncSample::FuzzRunInner()
{
    if (signal_ == nullptr) {
        return;
    }
...
int32_t VideoEncAsyncSample::InputFuncFUZZ(const uint8_t *data, size_t size)
{
    unique_lock<mutex> lock(signal_->inMutex_);
    ...
    memcpy_s(bufferAddr, bufferSize, data, size);

    int ret = PushInputData(index, attr);
