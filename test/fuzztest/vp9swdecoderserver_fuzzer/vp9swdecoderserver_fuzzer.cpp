...
#define FUZZ_PROJECT_NAME "vp9swdecoderserver_fuzzer"
const size_t EXPECT_SIZE = 8;

namespace OHOS {
bool Vp9SwdecoderServerFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Vp9SwdecoderServerFuzzTest(data, size);
#endif
    return 0;
}