...
VDecSyncSample *dec_sample = nullptr;
...
void VDecSyncSample::FlushBuffer()
{
...
std::vector<uint8_t> VDecSyncSample::LoadHashFile()
{
...
bool VDecSyncSample::MdCompare(uint8_t source[])
{
...
int32_t VDecSyncSample::RunVideoDecSurface(string codeName)
{
    SF_OUTPUT = true;
    int err = AV_ERR_OK;
    CreateSurface();
...
int32_t VDecSyncSample::RunVideoDec(string codeName)
{
    SF_OUTPUT = false;
...
int32_t VDecSyncSample::SetVideoDecoderCallback()
{
...
void VDecSyncSample::ReleaseInFile()
{
    if (inFile_ != nullptr) {
        if (inFile_->is_open()) {
            inFile_->close();
...
void VDecSyncSample::StopInloop()
{
...
int32_t VDecSyncSample::StartDecoder()
{
    ...
    inputLoop_ = make_unique<thread>(&VDecSyncSample::InputFuncTest, this);
    ...
    outputLoop_ = make_unique<thread>(&VDecSyncSample::OutputFuncTest, this);
...
int32_t VDecSyncSample::StartSyncDecoder()
{
    isRunning_.store(true);
    inFile_ = make_unique<ifstream>();
    if (inFile_ == nullptr) {
    ...
    signal_ = new VDecAPI11Signal();
    inputLoop_ = make_unique<thread>(&VDecSyncSample::SyncInputFunc, this);
    if (inputLoop_ == nullptr) {
        cout << "Failed to create input loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
    ...
    outputLoop_ = make_unique<thread>(&VDecSyncSample::SyncOutputFunc, this);
    if (outputLoop_ == nullptr) {
        cout << "Failed to create output loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
...
int32_t VDecSyncSample::StartVideoDecoder()
{
    isRunning_.store(true);
    if (PREPARE_FLAG) {
...
void VDecSyncSample::WaitForEOS()
{
...
void VDecSyncSample::InFuncTest()
{
...
void VDecSyncSample::SyncInputFunc()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
...
void VDecSyncSample::InputFuncTest()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
...
int32_t VDecSyncSample::PushData(uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr;
    ...
    if (useHDRSource) {
        ...
        uint32_t two = 2;
        uint32_t three = 3;
        bufferSize = static_cast<uint32_t>((((ch[zero] & 0xFF)) | ((ch[one] & 0xFF) << EIGHT) | ((ch[two] & 0xFF) << SIXTEEN) |
...
int32_t VDecSyncSample::CheckAndReturnBufferSize(OH_AVBuffer *buffer)
{
...
uint32_t VDecSyncSample::SendData(uint32_t bufferSize, uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr;
    ...
    uint32_t size = static_cast<uint32_t>(CheckAndReturnBufferSize(buffer));
    if (size < bufferSize + START_CODE_SIZE) {
        delete[] fileBuffer;
        return 0;
    }
...
void VDecSyncSample::AutoSwitchSurface()
{
    if (autoSwitchSurface) {
        switchSurfaceFlag = (switchSurfaceFlag == 1) ? 0 : 1;
...
int32_t VDecSyncSample::CheckAttrFlag(OH_AVCodecBufferAttr attr)
{
    if (IS_FIRST_FRAME) {
        GetStride();
...
void VDecSyncSample::OutputFuncTest()
{
...
void VDecSyncSample::SyncOutputFunc()
{
    ...
    while (isRunning_.load()) {
        ...
        if (ret != AV_ERR_OK) {
            cout << "OH_VideoDecoder_QueryOutputBuffer fail, ret: " << ret << endl;
            continue;
        }
...
int32_t VDecSyncSample::SyncOutputFuncEos(uint32_t &lastIndex, uint32_t &outFrames, uint32_t &index,
    OH_AVBuffer *buffer, OH_AVCodecBufferAttr &attr)
{
    if (getOutputBufferIndexNoExisted) {
        ...
        if (buffer == nullptr) {
            abnormalIndexValue = true;
            cout << "OH_VideoDecoder_GetOutputBuffer noexist buffer nullptr" << endl;
        }
        return AV_ERR_UNKNOWN;
    }
    if (outFrames == 0 && getOutputBufferIndexRepeated) {
        ...
        if (buffer == nullptr) {
            abnormalIndexValue = true;
            cout << "OH_VideoDecoder_GetOutputBuffer repeat buffer nullptr" << endl;
        }
        return AV_ERR_UNKNOWN;
    }
    if (OH_AVBuffer_GetBufferAttr(buffer, &attr) != AV_ERR_OK) {
...
void VDecSyncSample::ProcessOutputData(OH_AVBuffer *buffer, uint32_t index)
{
    if (!SF_OUTPUT) {
        uint8_t *bufferAddr = OH_AVBuffer_GetAddr(buffer);
...
void VDecSyncSample::RenderOutAtTime(uint32_t index)
{
    if (isAPI) {
...
int32_t VDecSyncSample::StateEos()
{
    uint32_t index;
    unique_lock<mutex> lock(signal_->inMutex_);
...
void VDecSyncSample::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr;
    attr.pts = 0;
    attr.size = 0;
...
int32_t VDecSyncSample::Prepare()
{
...
int32_t VDecSyncSample::QueryInputBuffer(uint32_t index, int64_t timeoutUs)
{
    return OH_VideoDecoder_QueryInputBuffer(vdec_, &index, timeoutUs);
}

OH_AVBuffer *VDecSyncSample::GetInputBuffer(uint32_t index)
{
    return OH_VideoDecoder_GetInputBuffer(vdec_, index);
}

int32_t VDecSyncSample::QueryOutputBuffer(uint32_t index, int64_t timeoutUs)
{
    return OH_VideoDecoder_QueryOutputBuffer(vdec_, &index, timeoutUs);
}

OH_AVBuffer *VDecSyncSample::GetOutputBuffer(uint32_t index)
{
    return OH_VideoDecoder_GetOutputBuffer(vdec_, index);
}

int32_t VDecSyncSample::PushInputBuffer(uint32_t index)
{
    return OH_VideoDecoder_PushInputBuffer(vdec_, index);
}

void VDecSyncSample::StopOutloop()
{
...
int32_t VDecSyncSample::SetParameter(OH_AVFormat *format)
{
    return OH_VideoDecoder_SetParameter(vdec_, format);
}

int32_t VDecSyncSample::SwitchSurface()
{
...
int32_t VDecSyncSample::RepeatCallSetSurface()
{
...
int32_t VDecSyncSample::OpenFile()
{
    inFile_ = make_unique<ifstream>();
    if (inFile_ == nullptr) {
        return AV_ERR_UNKNOWN;
...
void VDecSyncSample::SetParameter(int32_t data)
{
    OH_AVFormat *format = OH_AVFormat_Create();
