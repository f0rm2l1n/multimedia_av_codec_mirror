...
const int32_t *pixFormat = nullptr;
...
bool AvcapabilityFuzzTest(const uint8_t *data, size_t size)
{
    ...
    int bitrate_value = fdp.ConsumeIntegralInRange(0, 3);
    OH_BitrateMode mode = static_cast<OH_BitrateMode>(bitrate_value);
    ...
    int feature_value = fdp.ConsumeIntegralInRange(0, 2);
    OH_AVCapabilityFeature feature = static_cast<OH_AVCapabilityFeature>(feature_value);
    ...
    if (cap != nullptr) {
        ...
        OH_AVCapability_GetSupportedLevelsForProfile(cap, profile, &levels, &levelNum);
        ...
        OH_AVCapability_GetVideoSupportedPixelFormats(cap, &pixFormat, &pixFormatNum);
        OH_AVCapability_GetVideoWidthAlignment(cap, widthAlignment);
        OH_AVCapability_GetVideoHeightAlignment(cap, heightAlignment);
