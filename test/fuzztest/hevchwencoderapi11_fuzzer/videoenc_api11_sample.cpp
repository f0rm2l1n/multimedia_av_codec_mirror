...
static void onEncInputBufferAvailable(OH_AVCodec *codec, uint32_t index, OH_AVBuffer *buffer, void *userData)
{
    VEncSignal *signal = static_cast<VEncSignal *>(userData);
    {
        ...
        if (g_vEncSample == nullptr || g_vEncSample->roiInfo == nullptr) {
            cout << "enc_sample or roi is null" << endl;
            return;
        }
        ...
        OH_AVFormat_SetStringValue(parameter, OH_MD_KEY_VIDEO_ENCODER_ROI_PARAMS, g_vEncSample->roiInfo);
...
int32_t VEncAPI11FuzzSample::SetParameter()
{
    if (venc_) {
        OH_AVFormat *format = OH_AVFormat_Create();
        if (format == nullptr) {
        ...
        int32_t sqrfac = 51;
        int64_t biterate = 10000000;
        (void)OH_AVFormat_SetIntValue(format, OH_MD_KEY_SQR_FACTOR, sqrfac);
        (void)OH_AVFormat_SetLongValue(format, OH_MD_KEY_MAX_BITRATE, biterate);
        int ret = OH_VideoEncoder_SetParameter(venc_, format);
        int32_t value = 0;
        int64_t value2 = 0;
        OH_AVFormat_GetIntValue(format, OH_MD_KEY_SQR_FACTOR, &value);
        OH_AVFormat_GetLongValue(format, OH_MD_KEY_MAX_BITRATE, &value2);
...
{
    venc_ = OH_VideoEncoder_CreateByMime(codecName);
    g_vEncSample = this;
