...
constexpr int64_t FRAME_COUNT_WIDTH = 20;
constexpr int64_t FRAME_COUNT_LENGTH = 1000;

constexpr string_view INPUT_VORBIS_FILE_PATH = "/data/test/media/8000_mono_gsm_ms.avi";
} // namespace
...
{
    DEMO_CHECK_AND_RETURN_RET_LOG(InitFileWithDemux(INPUT_VORBIS_FILE_PATH.data()) == AVCS_ERR_OK,
        false, "Fatal: InitFileWithDemux fail");
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_GSM_MS) {
        int32_t channelCounttmpgsm = 1;
        int32_t sampleRatetmpgsm = 8000;
        channelCount = channelCounttmpgsm;
        sampleRate = sampleRatetmpgsm;
    }
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("gsm_ms") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_GSM_MS;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_GSM_MS_NAME).data());
    } else {
...
void ADecBufferDemo::InputFunc()
{
    while (isRunning_.load()) {
        ...
        if (!ReadBufferFromDemux(buffer, index)) {
            signal_->inQueue_.pop();
            signal_->inBufferQueue_.pop();
            break;
        } else {
...
void ADecBufferDemo::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr {};
    attr.pts   = 0;
    attr.size  = 0;
...
bool ADecBufferDemo::ReadBufferFromDemux(OH_AVBuffer *buffer, uint32_t index)
{
    OH_AVErrCode r = OH_AVDemuxer_ReadSampleBuffer(demuxer_, 0, buffer);
    if (r != AV_ERR_OK) {
        SetEOS(index, buffer);
        return false;
    }
    ...
    buffer->buffer_->pts_  = static_cast<int64_t>(frameCount_.load()) * FRAME_COUNT_WIDTH * FRAME_COUNT_LENGTH;
    buffer->buffer_->flag_ = AVCODEC_BUFFER_FLAGS_NONE;
    return true;
}

int32_t ADecBufferDemo::InitFileWithDemux(const std::string& inputFilePath)
{
    ...
    source_ = OH_AVSource_CreateWithFD(fd, 0, static_cast<int64_t>(st.st_size));
    if (source_ == nullptr) {
        close(fd);
        return AVCS_ERR_UNKNOWN;
    }
    ...
    OH_AVErrCode ret = OH_AVDemuxer_SelectTrackByID(demuxer_, 0);
    if (ret != AV_ERR_OK) {
        return AVCS_ERR_UNKNOWN;
    }
...
OH_AVCodec* ADecBufferDemo::CreateByMime(const char* mime)
{
    if (mime != nullptr) {
        if (strcmp(mime, "audio/mp4a-latm") == 0) {
            ...
            audioType_ = AudioBufferFormatType::TYPE_LBVC;
        } else if (strcmp(mime, OH_AVCODEC_MIMETYPE_AUDIO_GSM_MS) == 0) {
            audioType_ = AudioBufferFormatType::TYPE_GSM_MS;
        } else {
            audioType_ = AudioBufferFormatType::TYPE_VIVID;
        }
