...
bool DecoderSyncFuzzTest(const uint8_t *data, size_t size)
{
    ...
    g_vDecSample->DEFAULT_WIDTH = fdp.ConsumeIntegral<int64_t>();
    g_vDecSample->DEFAULT_HEIGHT = fdp.ConsumeIntegral<int64_t>();
    g_vDecSample->DEFAULT_FRAME_RATE = fdp.ConsumeIntegral<int64_t>();
