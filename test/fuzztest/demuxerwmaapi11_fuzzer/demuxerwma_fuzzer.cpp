...
namespace OHOS {
const char *WMA_PATH = "/data/test/fuzz_create.wma";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t size)
{
    ...
    int32_t fd = open(WMA_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
