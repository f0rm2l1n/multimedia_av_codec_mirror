...
#define FUZZ_PROJECT_NAME "vp9swdecoderservermemrecycle_fuzzer"
const size_t EXPECT_SIZE = 26;

namespace OHOS {
bool Vp9SwdecoderServerMemrecycleFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    Vp9VDecServerSample *vDecSample = new Vp9VDecServerSample();
    ...
    vDecSample->inpDir = "/data/test/media/test.vp9";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Vp9SwdecoderServerMemrecycleFuzzTest(data, size);
#endif
    return 0;
}
