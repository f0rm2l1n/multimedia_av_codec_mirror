...
void VEncServerSample::CallBack::OnError(AVCodecErrorType errorType, int32_t errorCode)
{
    cout << "--OnError--" << endl;
...
void VEncServerSample::CallBack::OnOutputFormatChanged(const Format &format)
{
    tester->GetOutputFormat();
}

void VEncServerSample::CallBack::OnInputBufferAvailable(uint32_t index, std::shared_ptr<AVBuffer> buffer)
{
...
int32_t VEncServerSample::SetParameter()
{
    Format fmt;
...
int32_t VEncServerSample::ConfigServerDecoder()
{
    Format fmt;
    fmt.PutIntValue(MediaDescriptionKey::MD_KEY_WIDTH, defaultWidth);
    fmt.PutIntValue(MediaDescriptionKey::MD_KEY_HEIGHT, defaultHeight);
...
int32_t VEncServerSample::SetCallback()
{
    shared_ptr<CallBack> cb = make_shared<CallBack>(this);
    return codec_->SetCallback(cb);
}

void VEncServerSample::RunVideoServerDecoder()
{
    codec_ = HCodec::Create("OMX.hisi.video.encoder.hevc");
    if (codec_ == nullptr) {
    ...
    inputLoop_ = make_unique<thread>(&VEncServerSample::InputFunc, this);
...
void VEncServerSample::GetOutputFormat()
{
    Format fmt;
