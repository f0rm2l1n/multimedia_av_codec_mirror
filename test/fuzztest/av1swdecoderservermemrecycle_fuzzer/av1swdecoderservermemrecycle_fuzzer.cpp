...
#define FUZZ_PROJECT_NAME "av1swdecoderservermemrecycle_fuzzer"
const size_t EXPECT_SIZE = 26;

namespace OHOS {
bool Av1SwdecoderServerMemrecycleFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
    ...
    vDecSample->inpDir = "/data/test/media/test.av1";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Av1SwdecoderServerMemrecycleFuzzTest(data, size);
    return 0;
}
