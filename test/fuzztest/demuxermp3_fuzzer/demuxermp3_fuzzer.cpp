...
namespace OHOS {
const char *MP3_PATH = "/data/test/fuzz_create.mp3";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t size)
{
    ...
    int32_t fd = open(MP3_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
