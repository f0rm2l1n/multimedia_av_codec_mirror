...
namespace OHOS {
const char *DTS_PATH = "/data/test/fuzz_create.dts";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t dataSize)
{
    ...
    int32_t fd = open(DTS_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
