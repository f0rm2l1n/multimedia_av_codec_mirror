...
namespace OHOS {
const char *TRP_PATH = "/data/test/fuzz_create.trp";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t size)
{
    ...
    int32_t fd = open(TRP_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
