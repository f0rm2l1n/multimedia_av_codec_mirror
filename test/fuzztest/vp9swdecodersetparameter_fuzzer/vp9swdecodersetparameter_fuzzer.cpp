...
#define FUZZ_PROJECT_NAME "vp9swdecodersetparameter_fuzzer"
using namespace std;
...
bool DoSomethingInterestingWithMyAPI(const uint8_t *data, size_t size)
{
    ...
    if (!g_vDecSample) {
        ...
        g_vDecSample->CreateVideoDecoder("OH.Media.Codec.Decoder.Video.VP9");
        g_vDecSample->ConfigureVideoDecoder();
        g_vDecSample->SetVideoDecoderCallback();
        g_vDecSample->StartVideoDecoder();

        OH_AVFormat *format = OH_AVFormat_CreateVideoFormat("video/vp9", DEFAULT_WIDTH, DEFAULT_HEIGHT);
