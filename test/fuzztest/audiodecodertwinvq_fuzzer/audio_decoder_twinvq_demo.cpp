...
#include "audio_decoder_twinvq_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr size_t TWINVQ_MIN_FRAME = 66;
constexpr uint32_t SAMPLE_RATE = 44100;
constexpr uint32_t DEFAULT_AAC_TYPE = 1;
constexpr uint32_t AMRWB_SAMPLE_RATE = 16000;
constexpr uint32_t AMRNB_SAMPLE_RATE = 8000;
constexpr uint32_t FRAME_BYTES_TWINVQ = 1056;
constexpr string_view INPUT_TWINVQ_FILE_PATH = "/data/test/media/8000_mono.dat";
} // namespace
...
{
    std::ifstream inputFile(INPUT_TWINVQ_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_TWINVQ) {
        int32_t channelCounttmptwinvq = 1;
        int32_t sampleRatetmptwinvq = 8000;
        channelCount = channelCounttmptwinvq;
        sampleRate = sampleRatetmptwinvq;
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("twinvq") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_TWINVQ;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_TWINVQ_NAME).data());
        std::cout << "CreateDec TYPE_TWINVQ!" << endl;
    } else {
...
bool ADecBufferDemo::IsBufferFormatAacOrVorbis()
{
    return (audioType_ == AudioBufferFormatType::TYPE_AAC || audioType_ == AudioBufferFormatType::TYPE_VORBIS);
}

bool ADecBufferDemo::WaitInputBuffer()
{
    unique_lock<mutex> lock(signal_->inMutex_);
    signal_->inCond_.wait(lock, [this] {
        return !signal_->inQueue_.empty() || !isRunning_.load();
    });
    return isRunning_.load();
}

void ADecBufferDemo::SendEOS(uint32_t index, OH_AVBuffer* buffer)
{
...
bool ADecBufferDemo::FeedOneFrame(uint32_t index, OH_AVBuffer* buffer, size_t frameBytes)
{
    const size_t currentSize = std::min(inputdatasize, frameBytes);
    if (audioType_ == AudioBufferFormatType::TYPE_TWINVQ && currentSize < TWINVQ_MIN_FRAME) {
        SendEOS(index, buffer);
        inputdatasize = 0;
        return false;
    }
    if (currentSize == 0) {
    ...
    const string chunk = inputdata.substr(inputOffset, currentSize);
    char* dst = reinterpret_cast<char*>(OH_AVBuffer_GetAddr(buffer));
    if (strncpy_s(dst, currentSize, chunk.c_str(), currentSize) != EOK) {
        return false;
    }

    buffer->buffer_->memory_->SetSize(currentSize);
    buffer->buffer_->flag_ = (isFirstFrame_ && IsBufferFormatAacOrVorbis())
                                 ? AVCODEC_BUFFER_FLAGS_CODEC_DATA
                                 : AVCODEC_BUFFER_FLAGS_NONE;

    const int32_t pushRet = OH_AudioCodec_PushInputBuffer(audioDec_, index);
    signal_->inQueue_.pop();
    signal_->inBufferQueue_.pop();
    ...
    return pushRet == AVCS_ERR_OK;
}

void ADecBufferDemo::InputFunc()
{
    constexpr size_t frameBytes = FRAME_BYTES_TWINVQ;
    while (isRunning_.load()) {
        ...
        if (!FeedOneFrame(index, buffer, frameBytes)) {
            isRunning_.store(false);
            break;
        }
    }
