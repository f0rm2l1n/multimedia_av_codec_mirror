...
#define FUZZ_PROJECT_NAME "vp8swdecoderconfigure_fuzzer"
const size_t EXPECT_SIZE = 24;
namespace OHOS {
bool Vp8SwdecoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Vp8SwdecoderConfigureFuzzTest(data, size);
#endif
    return 0;
}