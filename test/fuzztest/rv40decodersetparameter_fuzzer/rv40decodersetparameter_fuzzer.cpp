...
#define FUZZ_PROJECT_NAME "rv40decodersetparameter_fuzzer"
...
{
    format = OH_AVFormat_CreateVideoFormat("video/rv40", DEFAULT_WIDTH, DEFAULT_HEIGHT);
...
namespace OHOS {
bool Rv40decoderSetParameterFuzzTest(const uint8_t *data, size_t size)
{
    ...
    if (!vDecSample) {
        ...
        if (vDecSample->CreateVideoDecoder("OH.Media.Codec.Decoder.Video.RV40") != AV_ERR_OK) {
            delete vDecSample;
            vDecSample = nullptr;
            return false;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Rv40decoderSetParameterFuzzTest(data, size);
#else
    (void)data;
    (void)size;
#endif
