...
#define FUZZ_PROJECT_NAME "rawvideodecodersetparameter_fuzzer"
...
{
    format = OH_AVFormat_CreateVideoFormat("video/rawvideo", DEFAULT_WIDTH, DEFAULT_HEIGHT);
...
namespace OHOS {
bool RawvideoDecoderSetParameterFuzzTest(const uint8_t *data, size_t size)
{
    ...
    if (!vDecSample) {
        ...
        if (vDecSample->CreateVideoDecoder("OH.Media.Codec.Decoder.Video.RAWVIDEO") != AV_ERR_OK) {
            delete vDecSample;
            vDecSample = nullptr;
            return false;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::RawvideoDecoderSetParameterFuzzTest(data, size);
    return 0;
}
