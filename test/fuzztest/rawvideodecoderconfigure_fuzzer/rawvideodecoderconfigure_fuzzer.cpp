...
#define FUZZ_PROJECT_NAME "rawvideodecoderconfigure_fuzzer"
...
bool RawvideodecoderConfigureFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
    ...
    std::string filename = "/data/test/corpus-RawvideodecoderConfigureFuzzTest";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::RawvideodecoderConfigureFuzzTest(data, size);
    return 0;
}
