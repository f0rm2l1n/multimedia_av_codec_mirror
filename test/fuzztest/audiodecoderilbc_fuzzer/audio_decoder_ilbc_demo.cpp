...
#include "audio_decoder_ilbc_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr uint32_t FRAME_BYTES_ILBC = 240;
constexpr string_view INPUT_ILBC_FILE_PATH = "/data/test/media/ilbc.ilbc";
} // namespace
...
{
    std::ifstream inputFile(INPUT_ILBC_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_ILBC) {
        int32_t channelCounttmp = 1;
        int32_t sampleRatetmp = 8000;
        channelCount = channelCounttmp;
        sampleRate = sampleRatetmp;
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("ilbc") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_ILBC;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_ILBC_NAME).data());
        std::cout << "CreateDec TYPE_ILBC!" << endl;
    } else {
...
void ADecBufferDemo::InputFunc()
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_ILBC) {
        frameBytes = FRAME_BYTES_ILBC;
    ...
    if (!eosFlag) {
        if (!signal_->inBufferQueue_.empty()) {
            ...
            std::cout << "info.size:" << info.size
                << " ADecBufferDemo::PushInputData : = " << (int32_t)ret << std::endl;
            if (ret != AV_ERR_OK) {
                return ret;
            }
        }
