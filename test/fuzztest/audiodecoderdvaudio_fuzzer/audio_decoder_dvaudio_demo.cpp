...
constexpr uint32_t DVAUDIO_SAMPLE_RATE = 48000;
constexpr uint32_t FRAME_BYTES_DEFAULT = 1024;
constexpr uint32_t FRAME_BYTES_MP3_FLAC_APE = 4096;
constexpr uint32_t FRAME_BYTES_DVAUDIO = 8192;
constexpr string_view INPUT_DVAUDIO_FILE_PATH = "/data/test/media/dvaudio.avi";
} // namespace
...
{
    std::ifstream inputFile(INPUT_DVAUDIO_FILE_PATH, std::ios::binary);
...
bool ADecBufferDemo::InitFormat(OH_AVFormat *format)
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        OH_AVFormat_SetIntValue(format, MediaDescriptionKey::MD_KEY_AUDIO_SAMPLE_FORMAT.data(),
                                OH_BitsPerSample::SAMPLE_S16LE);
    } else if (audioType_ == AudioBufferFormatType::TYPE_DVAUDIO) {
        channelCount = CHANNEL_COUNT;
        sampleRate = DVAUDIO_SAMPLE_RATE;
    }
...
bool ADecBufferDemo::InitFile(const std::string& inputFile)
{
    ...
    } else if (inputFile.find("dvaudio") != std::string::npos) {
        audioType_ = AudioBufferFormatType::TYPE_DVAUDIO;
    } else {
        audioType_ = AudioBufferFormatType::TYPE_AAC;
    }
    return true;
...
int32_t ADecBufferDemo::CreateDec()
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        ...
        audioDec_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_DECODER_DVAUDIO_NAME).data());
        std::cout << "CreateDec TYPE_DVAUDIO!" << endl;
    } else {
...
void ADecBufferDemo::InputFunc()
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_DVAUDIO) {
        frameBytes = FRAME_BYTES_DVAUDIO;
