...
#include "audio_decoder_vivid_demo.h"
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <sys/stat.h>
...
constexpr uint32_t SAMPLE_FORMAT_VALUE = 2;
constexpr uint32_t SAMPLE_RATE = 44100;
constexpr uint32_t DEFAULT_AAC_TYPE = 1;
constexpr uint32_t AMRWB_SAMPLE_RATE = 16000;
constexpr uint32_t AMRNB_SAMPLE_RATE = 8000;
constexpr string_view INPUT_VIVID_FILE_PATH = "/data/test/media/vivid_2c_44100hz_320k_first_frame.dat";
} // namespace
...
{
    std::ifstream inputFile(INPUT_VIVID_FILE_PATH, std::ios::binary);
...
{
    std::cout << "InputFunc Begin" << std::endl;
    // 已处理数据长度
    size_t offset = 0;
    // 当前待处理数据长度
    size_t bytesToCopy = 0;
    // Size of each chunk to copy
    size_t chunkSize = GetFrameBytes();
    while (isRunning_.load()) {
        ...
        if (offset < inputdatasize) {
            std::cout << "InputFunc input data!!!" << std::endl;
            if (isFirstFrame_) {
                // 首帧
                ReadFirstFrame(buffer);
                buffer->buffer_->flag_ = AVCODEC_BUFFER_FLAGS_CODEC_DATA;
                isFirstFrame_ = false;
            } else {
                //  非首帧
                // 获取当前待处理数据长度
                bytesToCopy = (chunkSize < (inputdatasize - offset)) ? chunkSize : (inputdatasize - offset);
                ReadFuzzData(buffer, offset, bytesToCopy);
                buffer->buffer_->memory_->SetSize(bytesToCopy);
                buffer->buffer_->flag_ = AVCODEC_BUFFER_FLAGS_NONE;
                offset += bytesToCopy;
            }
        } else {
            std::cout << "InputFunc Input EOS!!!" << std::endl;
            // 写入最后一帧
        ...
        signal_->inBufferQueue_.pop();
        frameCount_++;
        std::cout << "InputFunc frameCount: " << frameCount_ << std::endl;
        if (ret != AVCS_ERR_OK) {
            isRunning_.store(false);
            break;
        }
    }
    std::cout << "InputFunc End" << std::endl;
    signal_->startCond_.notify_all();
}

size_t ADecBufferDemo::GetFrameBytes()
{
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_OPUS) {
        ...
        size_t gmusize = 1024 * CHANNEL_COUNT * SAMPLE_FORMAT_VALUE;
        frameBytes = gmusize;
    }
    return frameBytes;
}

void ADecBufferDemo::ReadFirstFrame(OH_AVBuffer* buffer)
{
    std::cout << "ReadFirstFrame Read Data!!!" << std::endl;
    // 1.打开文件
    inputFile_.open(INPUT_VIVID_FILE_PATH, std::ios::binary);
    // 2.是否打开成功
    if (!inputFile_.is_open()) {
        std::cout << "Fatal: open input file failed" << std::endl;
        return;
    }
    // 3.读取数据
    int64_t size;
    ...
    DEMO_CHECK_AND_RETURN_LOG(inputFile_.gcount() == sizeof(size), "Fatal: read size fail");
    inputFile_.read(reinterpret_cast<char *>(&buffer->buffer_->pts_), sizeof(buffer->buffer_->pts_));
    DEMO_CHECK_AND_RETURN_LOG(inputFile_.gcount() == sizeof(pts), "Fatal: read pts fail");
    inputFile_.read(reinterpret_cast<char *>(OH_AVBuffer_GetAddr(buffer)), size);
    buffer->buffer_->memory_->SetSize(size);
    DEMO_CHECK_AND_RETURN_LOG(inputFile_.gcount() == size, "Fatal: read buffer fail");
    // 4.关闭文件
    inputFile_.close();
}

int32_t ADecBufferDemo::ReadFuzzData(OH_AVBuffer* buffer, size_t offset, size_t bytesToCopy)
{
    errno_t err = strncpy_s(reinterpret_cast<char*>(OH_AVBuffer_GetAddr(buffer)),
                            bytesToCopy, (inputdata.substr(offset)).c_str(), bytesToCopy);
    if (err != 0) {
        std::cerr << "Error in safe_strncpy: " << err << std::endl;
        return 1;
    }
    return 0;
}
