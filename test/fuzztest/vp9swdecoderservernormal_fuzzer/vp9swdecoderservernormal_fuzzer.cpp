...
#define FUZZ_PROJECT_NAME "vp9swdecoderservernormal_fuzzer"
const size_t EXPECT_SIZE = 26;

namespace OHOS {
bool Vp9SwdecoderServerNormalFuzzTest(const uint8_t *data, size_t size)
{
    if (size < EXPECT_SIZE) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::Vp9SwdecoderServerNormalFuzzTest(data, size);
#else
    (void)data;
    (void)size;
#endif
