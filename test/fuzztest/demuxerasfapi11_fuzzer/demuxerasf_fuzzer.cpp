...
namespace OHOS {
const char *ASF_PATH = "/data/test/fuzz_create.asf";
...
bool CheckDataValidity(FuzzedDataProvider *fdp, size_t dataSize)
{
    ...
    int32_t fd = open(ASF_PATH, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd < 0) {
        return false;
    }
