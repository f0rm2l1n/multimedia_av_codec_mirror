...
constexpr int32_t MAX_WIDTH_RANGE = 3840;
constexpr int32_t MAX_HEIGHT_RANGE = 2160;
constexpr int32_t MIN_FRANGE = 1;
constexpr int32_t MAX_FRANGE = 20;
...
int32_t VDecAPI11Sample::CheckAttrFlag(OH_AVCodecBufferAttr attr)
{
    ...
    if (attr.flags & AVCODEC_BUFFER_FLAGS_EOS) {
        ...
        if (!SF_OUTPUT && needCheckHash) {
            if (!MdCompare(g_md, SHA512_DIGEST_LENGTH, fileSourcesha256_vp9)) {
                errCount++;
            }
        }
        return -1;
