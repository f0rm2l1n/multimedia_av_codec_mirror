...
int32_t g_inputFIleFd = -1;
int32_t g_trackCount = 0;
int32_t g_trackIndex = -1;
...
public:
    ConsumerListenerBuffer(sptr<Surface> cs, std::string_view name) : cs(cs)
    {
        ...
        cout << "in surface buffer open file : " << name.data() << endl;
    };
    ...
    void OnBufferAvailable() override
    {
        ...
        if (buffer == nullptr) {
            ...
            cout << "surface buffer written frameSize:" << frameSize << ", buffer size:" << buffer->GetSize() << endl;
...
{
    cout << "VDecAPI11Sample::~VDecAPI11Sample()" << endl;
    g_yuvSurface = false;
...
void VdecAPI11FormatChanged(OH_AVCodec *codec, OH_AVFormat *format, void *userData)
{
    ...
    cout << "currentWidth:" << currentWidth << ", currentHeight:" << currentHeight
         << ", g_strideSurface:" << g_strideSurface << ", g_sliceSurface:" << g_sliceSurface << endl;
...
int64_t VDecAPI11Sample::GetInputFileSize(const char *inputFileName)
{
    int64_t fileSize = 0;
    if (inputFileName != nullptr) {
        struct stat fileStatus {};
        if (stat(inputFileName, &fileStatus) == 0) {
...
int32_t VDecAPI11Sample::LoadInputFile(const char *inputFileName)
{
    g_inputFIleFd = open(inputFileName, O_RDONLY);
    int64_t size = GetInputFileSize(inputFileName);
    cout << "LoadInputFile() inputFileName is " << inputFileName << ", g_inputFIleFd is " << g_inputFIleFd
         << ", size is " << size << endl;
    if (g_inputFIleFd < 0) {
        cout << "open intputFile failed !" << endl;
        return AV_ERR_UNKNOWN;
    }
    videoSource = OH_AVSource_CreateWithFD(g_inputFIleFd, 0, size);
    if (videoSource == nullptr) {
        cout << "open intputFile failed !" << endl;
        return AV_ERR_UNKNOWN;
    }
    ...
    if (demuxer == nullptr) {
        cout << "OH_AVDemuxer_CreateWithSource failed !" << endl;
        return AV_ERR_UNKNOWN;
    }
    sourceFormat = OH_AVSource_GetSourceFormat(videoSource);
    if (sourceFormat == nullptr) {
        cout << "OH_AVSource_GetSourceFormat failed !" << endl;
        return AV_ERR_UNKNOWN;
    }
    if (!OH_AVFormat_GetIntValue(sourceFormat, OH_MD_KEY_TRACK_COUNT, &g_trackCount)) {
        cout << "Get track_count failed !" << endl;
        return AV_ERR_UNKNOWN;
    }
    if (g_trackCount < 0) {
        cout << "this video have no video track. g_trackCount is " << g_trackCount << endl;
        return AV_ERR_UNKNOWN;
    }
    for (int32_t index = 0; index < g_trackCount; index++) {
        trackFormat = OH_AVSource_GetTrackFormat(videoSource, index);
        int trackType = 0;
        if (trackFormat && OH_AVFormat_GetIntValue(trackFormat, OH_MD_KEY_TRACK_TYPE, &trackType)) {
    ...
    OH_AVErrCode ret = OH_AVDemuxer_SelectTrackByID(demuxer, g_trackIndex);
    if (ret != AV_ERR_OK) {
        cout << "OH_AVDemuxer_SelectTrackByID failed ! by g_trackIndex[" << g_trackIndex << "]" << endl;
        return AV_ERR_UNKNOWN;
    }
    return AV_ERR_OK;
}
...
int32_t VDecAPI11Sample::StartVideoDecoderForH263()
{
    isRunning_.store(true);
    int ret = OH_VideoDecoder_Start(vdec_);
    if (ret != AV_ERR_OK) {
    ...
    inFile_->open(IN_DIR, ios::in | ios::binary);
    if (!inFile_->is_open()) {
        cout << "failed open file " << IN_DIR << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
        inFile_->close();
        inFile_.reset();
    ...
    inputLoop_ = make_unique<thread>(&VDecAPI11Sample::InputFuncTestForH263, this);
    if (inputLoop_ == nullptr) {
        cout << "Failed to create input loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
...
void VDecAPI11Sample::InputFuncTestForH263()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
    ...
    while (flag) {
        ...
        lock.unlock();
        if (!inFile_->eof()) {
            int ret = PushDataForH263(index, buffer);
            if (ret == 1) {
                flag = false;
                break;
            }
...
int32_t VDecAPI11Sample::PushData(uint32_t index, OH_AVBuffer *buffer)
{
    ...
    OH_AVDemuxer_ReadSampleBuffer(demuxer, g_trackIndex, buffer);
...
int32_t VDecAPI11Sample::PushDataForH263(uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr;
...
int32_t VDecAPI11Sample::SetRotationAngle(int32_t angle)
{
    int32_t ret = 0;
    OH_AVFormat *format = OH_AVFormat_Create();
    if (angle > 0) {
    ...
    ret = OH_VideoDecoder_SetParameter(vdec_, format);
    OH_AVFormat_Destroy(format);
    return ret;
}
