...
int32_t VEncNdkSample::ConfigureVideoEncoder()
{
    ...
    (void)OH_AVFormat_SetIntValue(format, OH_MD_KEY_HEIGHT, defaultWidth);
...
int32_t VEncNdkSample::CreateSurface()
{
    ...
    ret = OH_NativeWindow_NativeWindowHandleOpt(nativeWindow_, SET_BUFFER_GEOMETRY, defaultWidth, defaultWidth);
    if (ret != AV_ERR_OK) {
...
uint32_t VEncNdkSample::ReadOneFrameYUV420SP(uint8_t *dst)
{
    ...
    // copy UV
    for (uint32_t i = 0; i < defaultWidth / sampleRatio_; i++) {
        inFile_->read(reinterpret_cast<char *>(dst), defaultWidth);
        if (!ReturnZeroIfEOS(defaultWidth))
            return 0;
        dst += stride_;
...
int32_t VEncNdkSample::PushData(OH_AVMemory *buffer, uint32_t index, int32_t &result)
{
    ...
    if (defaultPixFmt_ == AV_PIXEL_FORMAT_RGBA) {
        if (size < defaultWidth * stride_) {
            return -1;
        }
        ReadOneFrameRGBA8888(fileBuffer);
        attr.size = stride_ * defaultWidth;
    } else {
...
void VEncNdkSample::InputFunc()
{
    ...
    while (runningFlag) {
        ...
        signal->inCond.wait(lock, [this]() {
            ...
            return signal->inIdxQueue.size() > 0 && !isFlushing_.load();
        });
        if (!isRunning.load()) {
            break;
        }
...
void VEncNdkSample::OutputFunc()
{
    ...
    while (true) {
        ...
        signal->outCond.wait(lock, [this]() {
            ...
            return signal->outIdxQueue.size() > 0 && !isFlushing_.load();
        });
        if (!isRunning.load()) {
            break;
        }
