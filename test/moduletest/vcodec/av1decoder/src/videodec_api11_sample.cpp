...
void VdecAPI11FormatChanged(OH_AVCodec *codec, OH_AVFormat *format, void *userData)
{
    ...
    cout << "VdecAPI11FormatChanged errCount: "<< dec_sample->errCount << endl;
}
...
int32_t VDecAPI11Sample::RunVideoDec_SurfaceForAv1(string codeName)
{
    SF_OUTPUT = true;
    int err = AV_ERR_OK;
    CreateSurface();
...
int32_t VDecAPI11Sample::RunVideoDecForAv1(string codeName)
{
    SF_OUTPUT = false;
    int err = CreateVideoDecoder(codeName);
    if (err != AV_ERR_OK) {
...
void VDecAPI11Sample::SyncInputFuncForAv1()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
...
int32_t VDecAPI11Sample::StartSyncVideoDecoderForAv1()
{
    isRunning_.store(true);
    int ret = OH_VideoDecoder_Start(vdec_);
    if (ret != AV_ERR_OK) {
    ...
    signal_ = new VDecAPI11Signal();
    return InputOutputSyncLoopForAv1();
}

int32_t VDecAPI11Sample::InputOutputSyncLoopForAv1()
{
    inputLoop_ = make_unique<thread>(&VDecAPI11Sample::SyncInputFuncForAv1, this);
    if (inputLoop_ == nullptr) {
        cout << "Failed to create input loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
