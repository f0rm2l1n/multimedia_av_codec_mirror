...
static bool fileCompare(FILE *fp1, FILE *fp2)
{
    fseek(fp1, 0, SEEK_END);
    fseek(fp2, 0, SEEK_END);
    size_t fp1_size = ftell(fp1);
    size_t fp2_size = ftell(fp2);
    if (fp1_size != fp2_size) {
        cout << "fp1_size != fp2_size" << endl;
        return false;
    }
    ...
    uint8_t buffer_2[4] = {};
    while (true) {
        int len_1 = fread(buffer_1, 1, 4, fp1);
        int len_2 = fread(buffer_2, 1, 4, fp2);
        if ((len_1 == 0) && (len_2 == 0)) {
            break;
        }
        if (len_1 != len_2) {
        ...
        if (memcmp(buffer_1, buffer_2, len_1) != 0) {
            cout << "memcmp" << endl;
            return false;
        }
    }
...
/**
 * @tc.number    : DEMUXER_FUNCTION_0600
 * @tc.name      : create source with fd and  read it at intervals but deleted halfway
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_0600, TestSize.Level1)
{
    int tarckType = 0;
    OH_AVCodecBufferAttr attr;
    ...
    int a_frame = 0;
    int v_frame = 0;
    const char *file = "/data/test/media/avcc_10min_1080p.mp4";
    int fd = open(file, O_RDONLY);
    int64_t size = GetFileSize(file);
    cout << file << "----------------------" << fd << "---------" << size << endl;
    source = OH_AVSource_CreateWithFD(fd, 0, size);
...
{
    const char *tmpFile1 = "/data/test/media/compare/avcc_10sec_video.bin";
    const char *tmpFile0 = "/data/test/media/compare/avcc_10sec_audio.bin";
    FILE *fp1 = fopen(tmpFile1, "wb");
    cout << tmpFile1 << "-------------tmpFile1-------------" << endl;
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
        ...
        for (int32_t index = 0; index < trackCount; index++) {
            ...
            if (tarckType == 0) {
                ...
                if (attr.flags == OH_AVCodecBufferFlags::AVCODEC_BUFFER_FLAGS_EOS) {
                    ...
                    fwrite(&attr.size, 1, sizeof(attr.size), fp1);
                }
            }
        }
    }
    ...
    FILE *src_v = fopen("/data/test/media/compare/avcc_video_600.bin", "rb");
    FILE *src_a = fopen("/data/test/media/compare/avcc_audio_431.bin", "rb");
    fseek(fp1, 0, SEEK_SET);
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp1, src_v), true);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp1);
    fclose(fp0);
    fclose(src_v);
    fclose(src_a);
...
{
    const char *tmpFile1 = "/data/test/media/compare/hvcc_video.bin";
    const char *tmpFile0 = "/data/test/media/compare/hvcc_audio.bin";
    FILE *fp1 = fopen(tmpFile1, "wb");
    cout << tmpFile1 << "-------------tmpFile1-------------" << endl;
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_v = fopen("/data/test/media/compare/hvcc_video_602.bin", "rb");
    FILE *src_a = fopen("/data/test/media/compare/hvcc_audio_433.bin", "rb");
    fseek(fp1, 0, SEEK_SET);
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp1, src_v), true);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
...
{
    const char *tmpFile1 = "/data/test/media/compare/mpeg2_video.bin";
    const char *tmpFile0 = "/data/test/media/compare/mpeg2_audio.bin";
    FILE *fp1 = fopen(tmpFile1, "wb");
    cout << tmpFile1 << "-------------tmpFile1-------------" << endl;
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_v = fopen("/data/test/media/compare/mpeg2_video_303.bin", "rb");
    FILE *src_a = fopen("/data/test/media/compare/mpeg2_audio_433.bin", "rb");
    fseek(fp1, 0, SEEK_SET);
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp1, src_v), true);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
...
{
    const char *tmpFile0 = "/data/test/media/compare/m4a.bin";
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_a = fopen("/data/test/media/compare/m4a_10293.bin", "rb");
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp0);
    fclose(src_a);
...
{
    const char *tmpFile0 = "/data/test/media/compare/aac.bin";
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_a = fopen("/data/test/media/compare/aac_9457.bin", "rb");
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp0);
    fclose(src_a);
...
{
    const char *tmpFile0 = "/data/test/media/compare/mp3.bin";
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_a = fopen("/data/test/media/compare/mp3_9150.bin", "rb");
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp0);
    fclose(src_a);
...
{
    const char *tmpFile0 = "/data/test/media/compare/ogg.bin";
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_a = fopen("/data/test/media/compare/ogg_11439.bin", "rb");
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp0);
    fclose(src_a);
...
{
    const char *tmpFile0 = "/data/test/media/compare/flac.bin";
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_a = fopen("/data/test/media/compare/flac_2288.bin", "rb");
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp0);
    fclose(src_a);
...
{
    const char *tmpFile0 = "/data/test/media/compare/wav.bin";
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    FILE *src_a = fopen("/data/test/media/compare/wav_5146.bin", "rb");
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
    fclose(fp0);
    fclose(src_a);
...
{
    const char *tmpFile1 = "/data/test/media/compare/ts_video.bin";
    const char *tmpFile0 = "/data/test/media/compare/ts_audio.bin";
    FILE *fp1 = fopen(tmpFile1, "wb");
    cout << tmpFile1 << "-------------tmpFile1-------------" << endl;
    FILE *fp0 = fopen(tmpFile0, "wb");
    cout << tmpFile0 << "-------------tmpFile0-------------" << endl;
    ...
    ASSERT_EQ(v_frame, 602);
    FILE *src_v = fopen("/data/test/media/compare/ts_video_602.bin", "rb");
    FILE *src_a = fopen("/data/test/media/compare/ts_audio_384.bin", "rb");
    fseek(fp1, 0, SEEK_SET);
    fseek(fp0, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp1, src_v), true);
    ASSERT_EQ(fileCompare(fp0, src_a), true);
 ...
 * @tc.name      : demux mp4, max tracks
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_1900, TestSize.Level2) {}
...
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_2000, TestSize.Level1)
{
    ...
    ASSERT_EQ(AV_ERR_OPERATE_NOT_PERMIT, OH_AVDemuxer_SelectTrackByID(demuxer, 0));
    close(fd);
}
...
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_9800, TestSize.Level0)
{
    ...
    FILE *src = fopen("/data/test/media/wav_info.bin", "rb");
    fseek(fp, 0, SEEK_SET);
    ASSERT_EQ(fileCompare(fp, src), true);
    close(fd);
    fclose(fp);
    fclose(src);
}
...
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_3100, TestSize.Level1)
{
    ...
    ASSERT_EQ(AV_ERR_OK, OH_AVDemuxer_SeekToTime(demuxer, (curr_pts - start_pts) / 1000, SEEK_MODE_PREVIOUS_SYNC));
...
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_3700, TestSize.Level0)
{
    ...
    int64_t prev_i_pts = prev_i_pos * 16666;
    ret = OH_AVDemuxer_SeekToTime(demuxer, (to_ms - start_pts) / 1000, SEEK_MODE_PREVIOUS_SYNC);
    ASSERT_EQ(ret, AV_ERR_OK);
    ret = OH_AVDemuxer_ReadSample(demuxer, trackIndex, memory, &attr);
    ASSERT_EQ(ret, AV_ERR_OK);
    bool ans = abs(prev_i_pts - attr.pts) < 16666 ? true : false;
    ASSERT_EQ(ans, true);
    close(fd);
}
...
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_3800, TestSize.Level0)
{
    ...
    int64_t next_i_pts = next_i_pos * 16666;
    ret = OH_AVDemuxer_SeekToTime(demuxer, (to_ms - start_pts) / 1000, SEEK_MODE_NEXT_SYNC);
    ASSERT_EQ(ret, AV_ERR_OK);
    ret = OH_AVDemuxer_ReadSample(demuxer, trackIndex, memory, &attr);
    ASSERT_EQ(ret, AV_ERR_OK);
    bool ans = abs(next_i_pts - attr.pts) < 16666 ? true : false;
    ASSERT_EQ(ans, true);
    close(fd);
}
...
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_3900, TestSize.Level0)
{
    ...
    int64_t closest_i_pts = closest_i_pos * 16666;
    ret = OH_AVDemuxer_SeekToTime(demuxer, (to_ms - start_pts) / 1000, SEEK_MODE_CLOSEST_SYNC);
    ASSERT_EQ(ret, AV_ERR_OK);
    ret = OH_AVDemuxer_ReadSample(demuxer, trackIndex, memory, &attr);
    ASSERT_EQ(ret, AV_ERR_OK);
    bool ans = abs(closest_i_pts - attr.pts) < 16666 ? true : false;
    ASSERT_EQ(ans, true);
    close(fd);
}
 ...
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5000, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5100
 * @tc.name      : create source with uri,flac
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5100, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5200
 * @tc.name      : create source with uri,m4a
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5200, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5300
 * @tc.name      : create source with uri,mp3
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5300, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5400
 * @tc.name      : create source with uri,ogg
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5400, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5500
 * @tc.name      : create source with uri,wav
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5500, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5600
 * @tc.name      : create source with uri,mp4
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5600, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5700
 * @tc.name      : create source with uri,hvcc mp4
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5700, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5800
 * @tc.name      : create source with uri,wav
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5800, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_5900
 * @tc.name      : create source with uri,wav
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_5900, TestSize.Level0)
{
    OH_AVCodecBufferAttr attr;
    bool isEnd = false;
...
/**
 * @tc.number    : DEMUXER_FUNCTION_6000
 * @tc.name      : create source with uri,wav
 * @tc.desc      : function test
 */
HWTEST_F(DemuxerFuncNdkTest, DEMUXER_FUNCTION_6000, TestSize.Level0)
{
